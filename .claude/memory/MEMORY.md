...
- [project_cell_interactive_overlay_plan.md](project_cell_interactive_overlay_plan.md) — D 路徑 future plan:`<CellInteractiveOverlay>` primitive(寬=cell.contentWidth,absolute pointer-events:none,hover/focus ring 全集中)解 P1 hover/focus 1px diff。短期 A 已接受(1px diff)。實作時走 codex-collab dual-track + **joint test case planning**(Claude+codex 兩邊獨立列 test scenarios → 比稿 union),最低 coverage:4 大 stories(VirtualScroll / RowAutoHeightInlineEdit / InlineEdit / RowDragInteractive)+ state combinations + visual + hit test + z-index portal。Trigger:「pixel-perfect / overlay primitive / cell-interactive layer / D 路徑」
