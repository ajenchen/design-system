...
| **Protected solo workflow**(1 task = 1 branch + 1 PR / required CI / standing engineering delegation)| `infra/governance/release-workflow.json` + `feedback_solo_dev_workflow.md` + AGENTS.md「# Git / release canonical」 | protected main 禁 direct push；PR 建立正常；required CI/conversations resolved **且 user 對他看過的預覽內容說過「發版」**(綁預覽內容指紋,不綁 commit 也不綁分支;一份同意 = 一次 final release)後,由 `release:auto` 走 `pr-checks → merge → publish → readback → consumer`(缺同意收據會停在 `AWAITING_USER_RELEASE_CONSENT`;規則 SSOT = AGENTS.md「# Git / release canonical」1.5)；preview/canary/attestation 非 blocking | ✅ aligned(M28 + hooks + GitHub ruleset)| 2026-07-20 migration / 2026-08-02 five-step reconciliation |
...
- `check_ds_anchor_preflight.sh`(live,P0 BLOCKER exit 2,2026-07-07 升級)— 偵測 production tsx / apps consumer 內 DS primitive wrap 而無 canonical read trace → block;bypass `GOVERNANCE_BYPASS_DS_ANCHOR=1`(audit-logged)
