...
- ✅ **1 Tabs axe-critical(user 拍板「正規修:移出 tablist + aria-owns」)**:先跑 axe **實測確認** critical `aria-required-children`(role=tablist 有 `button[aria-haspopup]` 子代 = inlineAction「更多 ▾」dropdown 在 tablist 內)。**技術更正**:aria-owns **不可行**(axe required-children 計 DOM-owned 子代,不因 aria-owns 排除)+ Radix 要求 trigger 為 List React 子代 → 唯一正規解 = action **portal 到 TabsList overlay 層(tablist 外)**,量測 trigger 座標定位(viewport-relative,涵蓋 none/scroll/menu)。**驗證**:axe 重跑 aria-required-children 2→1(剩 1 = appshell DataTable,非 Tabs);pixel 量測 action 右緣=「更多」tab 右緣(183=183);screenshot 視覺零變;buttonInsideTablist=false。tsc -b ✓ / typecheck:stories ✓ / build:lib ✓。**代價(user 接受)**:鍵盤 Tab 序 action 排 tab 後。canonical:tabs.spec.md 補 portal 架構 + 泛化「ARIA required-children 鐵律」+ 誠實標註殘留非-Tabs 例外(Radix aria-controls unmount / story contrast)。

- ✅ **2 asChild/children 型別收窄**:3-agent 並行 ground-truth 每元件 render body。**narrow 10 介面**:Switch(`asChild|children`,固定 Thumb 覆蓋)/ Tabs Trigger(`asChild`,多節點恆 Children.only crash)/ SegmentedControl Item(`asChild`)/ RadioGroup Root(`asChild`,多 item)/ RadioGroup Item(`asChild|children`,固定 Indicator dot)/ DropdownMenu Content(`asChild`,固定 RowSizeProvider wrapper)/ DropdownMenu RadioItem(`children` required 化,對齊 sibling Item/CheckboxItem)/ ScrollArea(`asChild`,固定 Viewport+ScrollBar+Corner 三子)/ Dialog Content(`asChild`)/ Sheet Content(`asChild`)。**KEEP(不收窄)**:Tabs Root/List(真容器 forwards children)、Tooltip(決策指定保彈性,children 走 contents div)。
  - ⚠️ **框架更正(ground-truth 抓)**:`BreadcrumbLink.asChild` **load-bearing 不可收窄** — 它正是 consumer 傳 `<RouterLink>` 的 Slot 機制(merge DS 樣式 + 保 href)。codex finding「麵包屑最慘:路由連結拿不到網址」框架反了:Omit asChild 會**造成**該 bug 非修它。同 18/1/5 框架誤判類。(真 href-loss 風險只在 collapsed-into-dropdown 的 overflow extractor 只認 `href` 不認 `to`,屬邏輯 gap 非型別面,另記。)
  - Switch 加 `props as SwitchProps & {asChild?;children?}` cast 保 runtime strip(同 RadioGroup 防禦)。驗:tsc -b ✓ / typecheck:stories ✓ / build:lib ✓(無 TS4023,10 介面 .d.ts emit clean)/ 0 內部 consumer 傳 asChild 到收窄介面 / spec 無 asChild 假宣稱(DropdownMenu spec asChild 提及是 Trigger,未收窄)。純型別改動 runtime 零變。
...
- ✅ **11-Select creatable(完成,兌現 user「Select 也要」)**:ground-truth 確認 Select searchable=trigger 內嵌 input(SelectMenu searchable=false),creatable 需外部 search 驅動 SelectMenu create-row(架構整合,非 prop forward——**更正 resolution「接線」前提**)。**正解**:SelectMenu 加**選配受控 `search`/`onSearchChange`**(default uncontrolled = 既有 consumer 零影響),Select 把 trigger `search` 受控傳入 + forward creatable/onCreate/createLabel。加 Creatable 展示 story(標籤 picker 真實情境)。**驗證(playwright 互動)**:非匹配搜尋 → create row 出現 ✓;鍵盤 Enter + 滑鼠 click 皆觸發 onCreate → 選項 4→5 新增+選取 ✓;exact match(Bug)無 create row ✓;0 pageerror。tsc -b ✓ / typecheck:stories ✓ / build:lib ✓。Combobox 不受影響(additive,走 uncontrolled 原路徑)。canonical:select.spec.md 補「建立新選項」段。
