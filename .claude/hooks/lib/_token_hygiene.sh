...
#
# Detects (ALL are silent-fail or cross-OS drift bug classes per canonical token/rule corpus):
...
source "$(dirname "$0")/_provider_paths.sh" 2>/dev/null || {
  printf 'GOVERNANCE_INTEGRITY: token hygiene corpus resolver unavailable\n' >&2
  exit 70
}
...
if grep -qE '\.stories\.tsx$' <<<"$FILE_PATH"; then
  exit 0
fi
if [ ! -f "$FILE_PATH" ]; then
  exit 0
...
# SSOT-drive(#87 2026-07-11):deny-list SSOT = the runner-authenticated utility registry.
# Missing/malformed policy is an integrity failure; a hardcoded fallback would silently create a
# second authority and let a mutable project installation weaken the canonical verdict.
_CORPUS_ROOT=$(governance_corpus_root 2>/dev/null) || {
  printf 'GOVERNANCE_INTEGRITY: token hygiene corpus root unavailable\n' >&2
  exit 70
}
_REG="$_CORPUS_ROOT/packages/design-system/src/tokens/utility-registry.json"
if [ -L "$_REG" ] || [ ! -f "$_REG" ] || [ ! -r "$_REG" ]; then
  printf 'GOVERNANCE_INTEGRITY: token hygiene utility registry unavailable or unsafe\n' >&2
  exit 70
fi
_SHADCN_LIST=$(jq -er '
  select(
    .schemaVersion == 1
    and ._meta.owner == "packages/design-system/src/tokens/utility-registry.json"
    and (._meta.provider_view_policy | type == "string" and length > 0)
  )
  | .shadcn_alias.block.color_alias
  | select(type == "array" and length > 0 and (unique | length) == length)
  | join("|")
' "$_REG" 2>/dev/null) || {
  printf 'GOVERNANCE_INTEGRITY: token hygiene utility registry invalid\n' >&2
  exit 70
}
SHADCN_PATTERN="\\b(${_SHADCN_LIST})\\b"
SHADCN_HITS=$(grep -m 5 -nE "$SHADCN_PATTERN" "$FILE_PATH" 2>/dev/null)
...
TWV4_HITS=$(grep -nE "$TWV4_PATTERN" "$FILE_PATH" 2>/dev/null | grep -v '\[&' | awk 'NR <= 5')
if [ -n "$TWV4_HITS" ]; then
...
SHADOW_HITS=$(grep -m 5 -nE "$SHADOW_PATTERN" "$FILE_PATH" 2>/dev/null)
...
PRIMITIVE_HITS=$(grep -m 5 -nE "$PRIMITIVE_PATTERN" "$FILE_PATH" 2>/dev/null)
...
if [ -n "$VIOLATIONS" ]; then
  CTX=$(printf 'Token hygiene 檢查發現違規 (provider-neutral canonical 禁止項):%b\n\n這些是 silent-fail bug classes——不會造成 compile error,但 runtime 視覺或行為會壞。下個 Edit 回去修。' "$VIOLATIONS")
