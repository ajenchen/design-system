...
HOOK="$SCRIPT_DIR/../stop_extract_canonical_rules.sh"
ORIGINAL_PATH="$PATH"
TMP_ROOT=$(mktemp -d)
...
printf '%s\n' '#!/bin/bash' \
  ...
  '  message) printf "noise\\nGAPS: synthetic rule gap\\nkeywords missing: synthetic\\n" ;;' \
  '  silent) printf "ordinary output\\n" ;;' \
  '  failure) printf "synthetic extraction failure\\n" >&2; exit 23 ;;' \
  '  *) exit 29 ;;' \
  'esac' > "$FAKE_NODE"
chmod +x "$FAKE_NODE"
...
if [ "$EXIT_CODE" = 0 ] && [ -z "$STDERR_TEXT" ] \
  && printf '%s' "$STDOUT_TEXT" | jq -e '.governanceContext.message | contains("Canonical-rule extraction Stop check:") and contains("GAPS: synthetic rule gap")' >/dev/null; then
...
run_hook failure
if [ "$EXIT_CODE" = 0 ] && [ -z "$STDERR_TEXT" ] \
  && printf '%s' "$STDOUT_TEXT" | jq -e '.governanceContext.message | contains("execution failed (exit 23)") and contains("synthetic extraction failure")' >/dev/null; then
