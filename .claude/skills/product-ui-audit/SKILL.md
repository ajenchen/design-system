...
<!-- _generated: scripts/gen-codex-adapter.mjs; source: packages/design-system/ds-canonical/skills/product-ui-audit/SKILL.md; provider: claude; do not edit this adapter view. -->
...
`node scripts/repository-hygiene-invariant.mjs --profile=product-consumer --check`；否則依同一
...
- **mindset #5 先證據、再分權**:code 裡有 TODO-未確認留白,且屬產品／UI／UX SSOT 真取捨(純工程不確定性應依證據自主收斂,不留 TODO)?
