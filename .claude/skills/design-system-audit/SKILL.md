...
> **Budget note**:本檔 = N-dim **registry SSOT**(N 以 `node scripts/dispatch-audit-dims.mjs` 輸出為準,禁 hardcode;dispatch-audit-dims.mjs / deep-audit parse 此檔,`check_dim_count_drift.sh` enforce 下游禁 hardcode — 不 parse 本檔),per CLAUDE.md `# 治理 canonical`「foundational SSOT 例外 ≤ 800-1200」,**不受 250 SKILL cap**(dim table 本質就是 registry,拆出會 break dispatch parser SSOT)。
...
| 54 | **M35 Nearest same-purpose canonical compliance**(2026-05-20 codify per codex Layer B D4)| 對每 `*.stories.tsx` wrap 既有 primitive(Sidebar / DataTable / ChromeHeader / Dialog / Sheet / Popover)的 file 跑:(a) 檔頭含 `@story-baseline:` cite marker?(b) `.claude/references/story-baseline-registry.json` 內 primitive 的 `requiredHelpers` 全 import?(c) `antiPatterns` regex 任一 match → fail?(d) `buttonCanonical` button variant + size + iconOnly + pressed 全 satisfy?Hook `check_story_invariants.sh R8` write-time enforce(block-severity antiPattern = P0 BLOCKER,2026-06-02 升級;warn-severity = stderr only),本 dim batch verify。錨例:2026-05-20 AppShell stories 連 5 round drift 後 codex Layer B 抓 root cause = SSOT 消費被當引用儀式 |
...
| 57 | **M29 DS Anchor Preflight enforcement coverage**(2026-05-26 codify per user verbatim「該程式化的都沒程式化」)| 對每 `*.tsx`(production code,非 stories / test)grep wrap DS primitive(`<Sidebar>` / `<AppShell>` / `<DataTable>` 等)→ verify 過去 30 turns transcript 含 `Grep`/`Read` tool call hit `packages/design-system/src/**/*.spec.md` 或 `*.stories.tsx`,OR 檔頭含 `@story-baseline:` marker 或 inline 3-column owner table。Hook `check_ds_anchor_preflight.sh` write-time P0 BLOCKER(exit 2,2026-07-07 升級;escape `CLAUDE_BYPASS_DS_ANCHOR=1` audit-logged)。對應 meta-patterns.md M29 + self-verify.md Pre-edit phase。錨例:2026-05-26 App.tsx 漏 SidebarTrigger / collapsible / startIcon mock-drift = M29 hook 不存在使 infra 沒攔 |
| 58 | **Fork-user 治理鏈 enforcement**(2026-05-26 codify per user「我們做那麼多 plugin 不就是要避免這件事?」;**2026-06-17 C-prime 改版:主路徑 = committed-config + npm,免 plugin**〔beta.70 真機蓋章;plugin 路線不可靠 per memory `reference_cloud_governance_loading`〕)| SessionStart hook `check_plugin_fork_health.sh` 偵測 fork repo(cwd 非 DS repo + `package.json` 含 `@qijenchen/design-system` dep)治理鏈缺失 → 印 npm 修法提示(r1)+ local vs published 版本 stale 比對提示 `npm run sync-all`(r2)。**Chicken-egg 層(C-prime,取代 2026-05-30 bootstrap hooks — `check_plugin_bootstrap.sh` / `block_production_edit_without_plugin.sh` 已 retired,`refresh-fork-launchers.mjs` OBSOLETE_HOOKS 主動移除避免 brick)**:fork 自帶 committed 啟動器 `fork-governance-dispatcher.sh`(Pre/Post/UserPromptSubmit 讀 `node_modules/@qijenchen/design-system/ds-canonical/fork/manifest.json` 跑官方 fork hooks;任一 exit 2 轉發 BLOCK;manifest 缺 → exit 0 永不 brick)+ `inject_fork_governance_preamble.sh`(SessionStart install-if-missing + 設計紀律 preamble)+ `.claude/settings.json` 註冊;**治理本體在 node_modules = npm install 覆蓋官方控管**,`build-fork-governance.mjs` 從 DS repo hooks 生 manifest,`test-fork-governance.mjs` 回歸鎖。Audit verify:dispatcher 轉發 exit 2 / manifest event 覆蓋 / obsolete hook 移除。對應 template CLAUDE.md「🚀 Onboarding(C-prime)」段 |
...
| # | Audit | What it catches |
|---|-------|-----------------|
| 84 | **Mirror-build integrity + 2-scenario E2E**(ds-repo-mode-only)| `node scripts/test-2-scenario-architecture.mjs`:20 test case + Mirror M0-M7 — published-template mirror allowlist 正確(scaffold + apps/template + scripts subset 全送達;`.claude/{skills,commands,agents}` 走 fork-manifest name-allowlist — fork-relevant 子集 committed 進 mirror、非 fork 名 = 洩漏擋下,治理本體走 npm ds-canonical 非 plugin,2026-06-18 C-prime)+ package.json transform(`workspaces` + DS dep `*`→`^X.Y.Z`)+ Scenario A/B 結構一致。錨例:2026-05-29 mirror 從 5/26 默默失敗數週稽核沒抓到 |
...
**強制 chain**:`/design-system-audit --deep` 跑時 Phase 1 前自動跑 `node scripts/audit-preflight.mjs`(對應 SSOT `.claude/memory/feedback_audit_discipline_full_sweep_deterministic_preflight.md`)
...
3. **Coverage matrix**:每原則 → audit dim 對應(N 對應 / NO COVER gap)— log JSON(`.claude/logs/audit-preflight-{date}.json`)存 counts + gap 摘要;完整每原則→dim 對應 SSOT = `.claude/references/principle-dim-map.json`(`--verbose` 才 dump coverageMap 進 log)
...
每 fix group:Edit(非 Write)→ `npx tsc -b` pass(禁 `--noEmit`,root tsconfig `files: []` silent pass)→ commit 描述性 message。Typical groups:cva drift / Spec Rule A / a11y / Anatomy / CLAUDE.md contradiction。
...
產出 short report(date / coverage / findings / deferred P2 / commits / next trigger);需跨 session 持久化的 audit 計畫 / backlog 寫 `.claude/planning/` audit doc(memory 只留短 index pointer,per 8-home Level 9)。
