...
# Design System Audit (78 main dimensions + group sub-rows, Groups A–P + Future-proof preflight)
...
| # | Audit | What it catches |
|---|-------|-----------------|
| 47 | **Tailwind utility registry compliance**(對齊 Atlassian `@atlaskit/tokens` / Carbon / Ant / Polaris token-first)| `check_opacity_token_usage.sh` 升 `check_tailwind_token_registry.sh`,讀 `packages/design-system/src/tokens/utility-registry.json` SSOT。檢 `leading-N` / `tracking-*` / `gap-px|0.5|1.5` / `w-fraction` / `text-(xs|sm|base|lg|xl|...)` / `font-(thin|light|semibold|black)` / `rounded-(xl|2xl|3xl)` 等繞 SSOT utility |
...
| 58 | **Fork-user plugin install enforcement**(2026-05-26 codify per user「我們做那麼多 plugin 不就是要避免這件事?」)| SessionStart hook `check_fork_user_plugin_install.sh` 偵測:(a) cwd 不是 DS repo(無 `packages/design-system/src`)/(b) `package.json` 含 `@qijenchen/design-system` dep /(c) `~/.claude/plugins/design-system/` OR `.claude/plugins/design-system/` 不存在 → 三題 YES 印強制提示。互補 product-workspace `scripts/check-plugin-installed.mjs` npm postinstall layer。對應 product-workspace/CLAUDE.md「第 −1 步」段 |
...
| 62 | **Fork-user Netlify onboarding canonical**(2026-05-26 codify per user verbatim「使用者不知道該註冊 Netlify,Claude 會知道如何引導使用者完成環境建置嗎?」)| Audit product-workspace + fork repos:(a) `netlify.toml` 含 access control headers(X-Robots-Tag noindex + visitor_access private 註解); (b) `.storybook/manager-head.html` 含 Netlify Identity widget script; (c) `scripts/setup-netlify-access.mjs` 含「沒 Netlify 帳號」前置 explainer + gh auth pre-check; (d) `package.json` 有 `setup:netlify` npm script; (e) **CLAUDE.md `Access control` 段含「Claude 引導使用者 Netlify onboarding」7 條話術**(解釋 Netlify / GitHub 1-click 註冊 / CLI 不熟處理 / 權限怕設錯 / setup 失敗 fallback / deploy 驗證 / GitHub CLI 未 login)+ OAuth security 本質段。錨例:2026-05-26 user 抓「Claude 會知道引導 user 完成環境建置嗎?」 |
...
| 64 | **Post-main-push SSOT propagation canonical**(2026-05-26 user correction「不是只要一 knowledge audit deep 之後就要,是等我 push main 後才要」)| Audit:(a) CLAUDE.md `# Git solo-work canonical` 有 Step 5.5 SSOT propagation 段;(b) Hook `check_post_main_ssot_propagate.sh` 存在 + 註冊在 PostToolUse Bash matcher;(c) hook 偵測 `git push origin main` + diff HEAD~..HEAD 含 SSOT-affecting paths(`packages/{design-system,storybook-config}/src` + `.claude/{rules,hooks,skills,commands,references}` + `.claude-plugin/*.json` + `hooks/hooks.json` + `CLAUDE.md`)→ inject 提議 bump beta.N+1;(d) /knowledge-prune SKILL.md「Phase Z reference」 + /deep-audit-cross-codex SKILL.md 點向 canonical(不複製到 skill,DRY);(e) 整鏈 cover 任何 SSOT-affecting 來源(/knowledge-prune / /deep-audit-cross-codex / 一般 dev / 新元件 / bug fix)。對應「一次更新所有 DS repo 增刪改,不需手動 1 個 1 個」directive |
