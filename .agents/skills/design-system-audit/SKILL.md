...
| 43 | **Reader-facing guidance 品質**(Autodocs 導讀 + Rule note)| AI judgment **NO-SAMPLE** 全讀 reader-facing `tags:['autodocs']` 的 `parameters.docs.description.component` 與所有 `.principles.stories.tsx` Rule notes。導讀必用人話回答「解決什麼 + 何時用／近親分界」，不可只是 component/API 摘要；Rule note 必說明原則為何，不只下結論；兩者都避免不必要中英夾雜(技術術語例外)。存在/stub 最低線由 `audit-content-quality.mjs --check` 機械擋，語意完整度不得以 script PASS 取代。對齊 `story-rules.md`「Autodocs component 導讀必寫」與 `../story-writing/references/example-selection.md`「Rule note 品質」。|
