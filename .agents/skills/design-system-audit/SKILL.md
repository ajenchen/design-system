...
description: Systematic audit of this design system for world-class quality. Runs the full audit dimension list(per `## The N audit dimensions` section) covering spec hygiene / code correctness / a11y / naming / tokens / patterns / canonical governance consistency / Layout Family compliance / prop value collisions / shadcn alias leakage / home-name-vs-scope fit / spec hardcoded-values, and remediates actionable findings under the shared authority classifier. Stops only for a genuine product/UI/UX SSOT tradeoff or a shared-governance human-only boundary. Invoke via /design-system-audit when asked to audit, re-audit, check quality, or verify design system health.
---

<!-- _generated: scripts/gen-codex-adapter.mjs; source: packages/design-system/ds-canonical/skills/design-system-audit/SKILL.md; provider: codex; do not edit this adapter view. -->

<!-- provider-binding: profile=repository-legacy-surfaces-v1; provider=codex; strategy=generated-binding-header; assumptionCount=46; assumptionFingerprint=sha256:da64347276b581f624fd0d05fbc283f5c522c60659eb02cbf5f01c15c7fb2661; evidence=packages/governance/canonical/providers.json#codex -->
...
- `provider-transport`: Use only the registered transport binding and retain provider/context identity evidence.
...
> **Budget note**:本檔 = N-dim **registry SSOT**(N 以 `node scripts/dispatch-audit-dims.mjs` 輸出為準,禁 hardcode;dispatch-audit-dims.mjs / deep-audit parse 此檔,`packages/design-system/ds-canonical/hooks/check_dim_count_drift.sh` enforce 下游禁 hardcode — 不 parse 本檔),per canonical governance 的「foundational SSOT 例外 ≤ 800-1200」,**不受 250 SKILL cap**(dim table 本質就是 registry,拆出會 break dispatch parser SSOT)。

Purpose: catch every bug class this project has shipped historically PLUS structural gaps relative to Polaris / Material / Atlassian / Ant / Carbon / Apple HIG. Each audit has a clear rubric tied to `packages/design-system/ds-canonical/rules/`, canonical references, or an owning spec. Reports/checkpoints are evidence receipts, not approval milestones; large engineering scope remains standing-delegated. Only a genuine product/UI/UX SSOT choice or a shared-governance human-only boundary may stop execution.
...
對齊 canonical audit taxonomy 6 維,本 skill 是 **D1 設計語言 + D2 程式語言** home;D3-D6 chain:
...
- Working dir = project root；record current worktree fingerprint and preserve unrelated dirty changes。Dirty worktree 本身不是 user checkpoint；無法隔離 overlapping edits才 fail closed
- `AGENTS.md` shared bootstrap + `packages/design-system/ds-canonical/{rules,references}/` 的 applicable canonical 必先 fully read(規則隨時間變,不憑記憶；provider instruction/view 不得取代 canonical source)

---

## The audit dimensions(Groups A–Q,full list per below;count canonical 不 hardcode per `check_dim_count_drift.sh`)
...
| 3 | **SSOT reciprocal** | A → B pointer exists but B → A reverse pointer missing (`packages/design-system/ds-canonical/rules/spec-rules.md`:reciprocal 必須存在) |
...
| # | Audit | What it catches |
|---|-------|-----------------|
| 14 | **命名一致性 + repository hygiene** | 機械核心:`node scripts/naming-structure-invariant.mjs`(component PascalCase / file kebab / 主檔對應)+ `node scripts/repository-hygiene-invariant.mjs --profile=authority --check`(全 Git worktree inventory、top-level home purpose、transient junk、declared symlink、排除 generated/fixture/baseline/archive 後的 exact duplicate)。判準 SSOT:`packages/design-system/ds-canonical/references/repository-hygiene.md` + policy JSON。語義層仍逐 home/file 判 active authority / generated / fixture-baseline / archive-evidence / concrete future-reserved / transient，並查 exports/scripts/workflows/globs/rg；禁把 zero-reference 單獨當刪除依據。Deep mode 另枚舉 managed-repos，逐一稽核當前 runtime 可用 repo；不可用者標 unobserved，不冒充 PASS。 |
| 15 | **Cross-doc 一致性** | shared governance 自身 + cross-spec full dup(Rule-of-3)+ tsx docblock-spec drift + stale upgrade markers + **(2026-06-04 加)docblock-vs-同檔-code + spec 段內描述性一致(Mode 表 / typography / padding / gap cross-section)**。詳 `audit-prompts.md` Dim 15。**Enforcement 升級**:本 dim 易被「散文 skim」淺跑漏→ 改由 cross-provider deep audit A.1b **強制 adversarial per-component**(Validator F = A.1b 證據存在閘(CRITICAL BLOCK)+ CP-A1b per-component 覆蓋 checkpoint),不再可 skim |
...
| # | Audit | What it catches |
|---|-------|-----------------|
| 23 | **Story canonical-drift + Migration coverage**(spec/tsx vs stories) | 跑 `node scripts/compile-stories.mjs --all --check` — 兩類 finding:(a) 已 migration 元件 key 不齊 = **P0 drift**(立即修);(b) 未 migration 元件(無 `componentMeta` export / 無 spec frontmatter)= **P2E migration pending**(自動 chain `/story-auto-compile-migrate` 批次處理,不 silent skip、不等 milestone sign-off)。進階模式 `--deep` 必跑本 Dim 直到全 DS 元件都 migrated + 0 drift |
...
| # | Audit | What it catches |
|---|-------|-----------------|
| 28 | **Manual story 拆分原則 alignment**(對齊 Polaris / Carbon / Storybook 官方)| Per-component grep `*.stories.tsx`(non-anatomy/principles)反 pattern:(1) `WithStartIcon`+`WithEndIcon` 拆兩 story(同 slot rule 違規,該 `WithIcon` 對照 grid)(2) `Default`+`AllVariants` 同檔(冗餘)(3) ≥2 個 variant 拆細(`Primary`+`Secondary`+`Tertiary` 各自 — 該合 `AllVariants`)。`// @story-split-rationale: <reason>` 檔首 allowlist 例外。Hook `check_story_invariants.sh` R2 slot_split(live write-time BLOCK;原 check_story_slot_split folded)同源,本 dim 對既有元件 batch verify。對應 `packages/design-system/ds-canonical/rules/story-rules.md`「拆分原則」+ canonical `/story-writing` skill Phase 0 |
| 29 | **Trait-based展示 stories compliance**(對齊 M19 ensure-canonical pipeline + Polaris/Material/Carbon/Ant/Storybook)| Per-component verify(a)spec.md frontmatter 有 `traits:` 宣告(b)展示 stories.tsx 包含每 trait 的 required core stories(c)scope-N/A 的 trait 在 spec.md 邊界案例段有 rationale。違反列 P0 修。對應 `category-templates.md` v2 + hook `check_story_invariants.sh R3`(原 lib/`check_story_category.sh` folded 進 R3)。Hook 是 write-time block,本 dim 對 inventory-discovered 的全部元件 batch verify + 找未宣告 traits 的元件(P2 migration pending) |
| 30 | **Principles canonical compliance**(對齊 Polaris / Material / Ant 共識)| Per-component verify principles.stories.tsx:(a)ONE complete `UsageGuidance`(何時用 + 何時不用 + sibling comparison)即 sufficient；採 split style 才需 ≥ 2 個 distinct decision dimensions(`WhenToUse` / `WhenNotToUse` / `Vs*Rule` / `ContentGuidelines`)(b)額外 story 必須教 distinct topic 且通過 earn-existence，禁止為湊數開第二篇(c)無 deprecated 命名(`Forbidden*` / `Donts` / `Pitfalls` / `Prohibitions` / `NonGoals` / `VisualDonts` → `WhenNotToUse`)。SSOT = `category-templates.md`「Universal core(v3 整合)」+「Canonical naming」節；機械 = hook `check_canonical_propagation.sh` E.1 + `scripts/audit-content-quality.mjs` Check 5e。本 dim 對既有元件 batch verify內容完整度與重複教學。 |

### Group M — Overlay body API discipline(2026-05-01 新增)

| # | Audit | What it catches |
|---|-------|-----------------|
| 31 | **Overlay body 無 stripped-padding boolean variant**(對齊 Material/Atlassian/Mantine/shadcn 主流;Polaris flush API 例外但 scope 極窄)| Per-overlay grep `components/(Dialog\|Sheet\|Popover)/*.tsx`(非 stories)反 pattern:`(flush\|naked\|bare\|stripped\|unpadded\|noPadding\|paddingless)\?:\s*boolean` 在 body component。違反 = list-as-region 場景該由 consumer 用 className override(`!px-0 !pt-0 !pb-0`)+ 自管 list outer wrapper 處理,不該加 body variant。Rationale:variant 不解決底層脆弱(加 1 row search/banner 就破功)+ 把 1 surface decision 拆兩 API。對應 hook `lib/_overlay_handcraft.sh` Check 6(`post_edit_dispatcher.sh` 消費;原 check_overlay_handcraft.sh 2026-05-13 fold)+ `overlay-surface.spec.md`「List-as-region in overlay body」。Hook 是 PostToolUse WARN(additionalContext,非 block),本 dim 對既有 overlay 元件 + 未來新增 overlay primitive(Drawer / FileViewer body 等)batch verify。`// overlay-body-stripped-variant-allow:` 檔頭 allowlist 例外(必含 ≥3 家世界級對照 + multi-row hold 保證)|
...
| 33 | **Component classification + abstraction discipline**(對齊 M21 + M22 + M23)| Per-component verify 6 子維:(a) **Internal vs Components 一致性**;(b) **Premature abstraction** rationale + cite;(c) **Sub-component 5-file 結構過度**;(d) **Benchmark claim 缺 source**(M22);(e) **DS internal canonical 優先**(M23)— spec / tsx 含 world-class DS keyword 但 visual decision(color / size / spacing / typography / state)未先 grep DS 既有 token / variant / pattern 命中 → flag;(f) **Component benchmark evidence debt 必為零**— `rg '@benchmark-unverified|@benchmark-citation-allow' packages/design-system/src/components -g '*.spec.md'` 任一命中即 fail,不得以 frontmatter list 或 blanket marker 代替 claim-level direct evidence。Sub-agent 對每 benchmark cite 反查:該屬性在 `tokens/` / 近親 spec 已有 codified canonical 嗎?有 → 該 cite 應為**輔證**(內部 canonical 主)而非**主導**(外部覆蓋內部)。違反 = M23 自開新 tier(2026-05-03 chevron 事件:Ant 5 家 muted 覆蓋 DS icon-only Button neutral-9 預設)|
...
| 36 | **Naked variant cell-as-input row-mode propagation**(M19)| `variant="naked"` consumer 內 wrapper 必 import + apply `nakedCellRowModeAlign` SSOT。Hook `check_field_family_invariants.sh` A.1(原 check_naked_row_mode_propagation folded;P0 BLOCKER)同源 |
| 37 | **Field state machine 「focus dominates everything」**(M19 v13.3)| 禁 per-control `(open\|isOpen) && 'border-primary'`(Field default 處理)+ naked variant 禁平行 outline ring。SSOT `field-wrapper.tsx` 三 compoundVariant。對齊 Material 3 / Polaris / Ant 共識。Hook `check_field_family_invariants.sh` A.3(原 check_field_state_token_consume folded;P0 BLOCKER)同源 |
| 38 | **Inline-action gap canonical**(2026-05-05)| `<ItemInlineAction>` sibling gap = `gap-2` (8px) per `inline-action.spec.md:80`。Hook `check_pattern_invariants.sh` C.2(原 check_inline_action_canonical_gap folded;P1 WARN)同源 |
| 39 | **Row-layout slot primitive consumption**(M1+M17)| 禁自刻 `<span h-[1lh] shrink-0 flex items-center>` slot wrapper(item-anatomy / field-wrapper 外),必消費 `<ItemPrefix>` / `<ItemSuffix>`。Hook `check_pattern_invariants.sh` C.4(原 check_row_slot_handcraft folded;P0 BLOCK)同源 |

### Group O — Storybook content quality(2026-05-15 user-mandated,gap 補)

User 2026-05-15 verbatim 抓「DS 深度稽核漏 storybook content quality」+「title 是否夠有 title 的感覺」+「應該要全盤檢查並避免下次深層稽核又漏東漏西」。對應 `governance/planning/storybook-governance-gap-2026-05-15.md` 完整 enumeration。
...
| 43 | **Reader-facing guidance + technical-probe classification**(Autodocs 導讀 / Rule note / test-only)| AI judgment **NO-SAMPLE** 全讀 reader-facing `tags:['autodocs']` 的 `parameters.docs.description.component` 與所有 `.principles.stories.tsx` Rule notes。導讀必用人話回答「解決什麼 + 何時用／近親分界」，不可只是 component/API 摘要；Rule note 必說明原則為何，不只下結論；兩者都避免不必要中英夾雜。另全量判斷 `tags:['test-only']` 是否真的只服務 hover/focus/open/perf/import 等 automation probe；reader-facing scenario 不得被藏，probe 不得用 `!autodocs` 或 `!dev`/`!test` 消失。存在/stub/visibility 最低線由 `audit-content-quality.mjs --check` + `test-storybook-test-only-semantics.mjs` 機械擋，語意完整度與分類不得以 script PASS 取代。對齊 `story-rules.md`「Autodocs component 導讀必寫」+「Technical probe visibility」。|
| 44 | **Public vs Internal classification**(2026-05-23 user 拍板;**2026-06-06 refine intent-based**,SSOT in `packages/design-system/ds-canonical/rules/ui-development.md`「Public component vs Internal primitive canonical」段)| **判準 = intent-based**(非機械空-render):問「DS 是否想讓 consumer 直接 import 用它(看 benchmark / 何時用 / export 給 consumer)?」YES(**即使需 compose/wire**)= public;只被 DS 內部元件用、consumer 用 wrapper 不碰它 = internal。**禁用舊「空 `<X/>` render 測試」**(誤判 composition primitive,如 FieldControlGroup 空=透明 div、ResizeHandle 需接線 → 實為 public,benchmark Bootstrap input-group / VS Code resize)。「現在 DS 自家沒用到」≠ internal。Public 住 `components/<Name>/` OR `patterns/<name>/`(無 `internal: true` frontmatter),storybook `Design System/Components OR Patterns/<Name>`。Internal 住 `components/Internal/<Name>/` OR `patterns/<name>/`(`internal: true` frontmatter),storybook `Design System/Internal/...`,export 加 `@internal` jsDoc。**Exception:compound-component public API**(`Dialog.Root/Trigger/Content` / `Field + FieldLabel + FieldError + FieldDescription` 等 documented composition pattern)豁免 — 對齊 Radix Dialog / MUI FormControl + Mantine Input.Wrapper compound idiom。對標世界級:Polaris Building blocks vs layout primitives / Material `@mui/material` vs `@mui/utils` / Atlassian `@atlaskit` vs internal `<unstyled>` / Carbon turnkey vs utilities / Apple HIG presented controls vs implementation primitives |
| 45 | **Mechanical output structural validation**(2026-05-15 user-mandated)| 對每元件跑 `compile-stories.mjs <Name>` 取 generated rows;grep 確認(a) `AllSizes` 含所有 cva sizes (b) `AllVariants` / `ColorMatrix` 含所有 cva variants (c) `Accessibility` story 含 ARIA hint / keyboard map (d) `See also` cross-link 反指 spec.md 既有 link section。不只 drift,**output structure 對 spec/cva 完整覆蓋**。對應 SSOT:`scripts/compile-stories.mjs` 邏輯 + `../story-writing/references/anatomy-standard.md`。Future:加 `compile-stories.mjs --validate` mode 把 grep 移進腳本 |
| 46 | **Manual vs Mechanical boundary**(2026-05-15 user-mandated)| Per-元件 grep `.stories.tsx`(非 anatomy/principles),若含 trait-derived `AllSizes` / `AllVariants` / `WithIcon` hand-written export 而非 import auto-compile 結果 = anti-pattern(該 migrate 進 auto-compile)。對應 SSOT:`category-templates.md` v2 trait-based。例外只能用 scoped `// @story-trait-rationale: <declared-trait> -> <existing-story-export> — <non-deferred reason>`；marker 只豁免該 trait，trait/target 必存在，禁止 file-wide escape 與「之後補／不在本次」等 defer 文案。|

### Group P — World-class tier(2026-05-17 加,codex 共識)

| # | Audit | What it catches |
|---|-------|-----------------|
| 47 | **Tailwind utility registry compliance**(對齊 Atlassian `@atlaskit/tokens` / Carbon / Ant / Polaris token-first)| `check_opacity_token_usage.sh`(讀 `packages/design-system/src/tokens/utility-registry.json` SSOT;原計畫 rename → `check_tailwind_token_registry.sh` 未實作,功能已在 check_opacity_token_usage.sh)。檢 `leading-N` / `tracking-*` / `text-(xs|sm|base|lg|xl|...)` / `font-(thin|light|semibold|black)` / `rounded-(xl|2xl|3xl)` 等繞 SSOT utility（硬 block）;micro_gap `gap-px|0.5|1.5` / fraction（registry 明列 14 條 `w-1/3..`，不含合法 `w-1/2`）2026-07-11 #71 已接線為 **soft WARN**（registry rationale「flag for human review」= 非 P0 block，避免誤殺 Calendar/Chart/FileViewer/Carousel 既有合法 micro-gap;行尾 `@token-registry-ok:` 可豁免）|
| 48 | **Unused / orphan token detector**(2026-05-21 升級:跑 `scripts/audit-orphan-tokens.mjs --check` SSOT,不用 raw grep `var()` — 漏 Tailwind `@theme inline` bridge + `@utility` body + class-name match + JS literal mirror,N 條合法消費路徑 false-positive)。Script 內建 structural-keep 分類器(palette tier 1-10 / mask alpha / chart reserved / state variants / neutral / SOP 5-piece / JS literal mirror)per `tokens/orphan-tokens.spec.md` SSOT。真 retire 候選 = comprehensive scan 後仍 0 consumer + 不落任何 structural-keep 類別。0 真孤兒 = PASS;有真孤兒 → 工程 evidence 能證明無 UI/UX effect 則自主 retire／補分類；若會改視覺 SSOT 且存在真取捨才 human-only |
| 49 | **a11y axe-core 自動跑 + contrast ratio**(Storybook a11y addon + Carbon AVT + Atlassian linters)| **MUST chain `npm run a11y:check`**(`scripts/audit-a11y.mjs`:headless axe 對每 story 跑 wcag2a/wcag2aa 含 color-contrast AA + baseline-diff gate);WCAG AA contrast ≥ 4.5:1(text)/ 3:1(large text / UI)。addon 已裝(`packages/storybook-config/addons-preset.ts`)為 dev-time 輔助。Carbon 每 PR 跑 AVT,Atlassian 季度 audit + linter integration |
| 50 | **Bundle size budget per component**(Material UI size-limit / Material Web public tracking / Ant tree-shake)| Per component gzip size 上限(eg. Button ≤ 5KB / DataTable ≤ 50KB);CI fail if regress > 10%。對齊 size-limit pkg + Carbon export audit。已建成:`package.json` `size-limit` 段 + budget SSOT `packages/design-system/bundle-budget.json` + `node scripts/check-bundle-size.mjs --check`，並由 `deep-audit-deterministic-plan.json` Dim 50 與非 blocking drift monitor 執行；standard release 只依 protected required CI。|
| 51 | **Theme / density visual matrix**(Material 3 dynamic color / Apple HIG Dynamic Type)| Deep mode 每 core story 跑 `scripts/visual-audit.mjs --matrix=theme-density` 4-cell(light/dark × density-md/lg)截圖(capture-only;matrix cell 無 baseline 槽,drift Δ% flag = future 需先建 baseline)。HC 尚未建 baseline；RTL 依 global compatibility contract 明確不支援，排除於 matrix 且不得宣稱覆蓋。對齊 Polaris visual regression / Carbon dark token matrix |
| 52 | **Header canonical cross-family invariants**(W1-W6,2026-05-17 ship per M31 codex 共識,對齊 GitHub Primer + Ant + Material v1)| Per chrome / overlay header tsx 跑:(W1) 含 Tabs child 必有 `withTabs` prop(border auto-suppress) (W2) tabs padding = header padding(`--layout-space-loose`) (W3) `--tab-height-lg` == `--chrome-header-height`(md/lg 對等) (W4) header + tabs flush stack 無 negative margin (W5) tabs default = sm 已 land + md 標 future tier (W6) cva default 已從 md 改 sm。Hook:`chrome_header_dispatcher.sh`(settings.json 註冊)→ `lib/_tab_lg_chrome_header_equal.sh`(W3)/ `lib/_header_with_tabs_border.sh`(W1)/ `lib/_chrome_header_handcraft.sh`(Layer 3 ChromeHeader consumption),全 P0 exit 2 傳播(2026-05-26 Track C fold;2026-05-31 + 2026-06-06 升 P0)|
...
| 54 | **M35 Nearest same-purpose canonical compliance**(2026-05-20 codify per codex Layer B D4)| 對每 `*.stories.tsx` wrap 既有 primitive(Sidebar / DataTable / ChromeHeader / Dialog / Sheet / Popover)的 file 跑:(a) 檔頭含 `@story-baseline:` cite marker?(b) `packages/design-system/ds-canonical/references/story-baseline-registry.json` 內 primitive 的 `requiredHelpers` 全 import?(c) `antiPatterns` regex 任一 match → fail?(d) `buttonCanonical` button variant + size + iconOnly + pressed 全 satisfy?Hook `check_story_invariants.sh R8` write-time enforce(block-severity antiPattern = P0 BLOCKER,2026-06-02 升級;warn-severity = stderr only),本 dim batch verify。錨例:2026-05-20 AppShell stories 連 5 round drift 後 codex Layer B 抓 root cause = SSOT 消費被當引用儀式 |
...
| 56 | **AppShell primary-header consistency**(2026-05-21 codify;2026-06-18 beta.74 擴 V3 + responsive 豁免)| 對每 consumer `.tsx`(stories / app code)grep `layout="primary-header"`(三種 JSX 引號形式 `layout="x"` / `{"x"}` / `{'x'}` 皆偵測,grep -E 字面引號),verify 3 條:(V1) 同 file 含 `globalHeader=` prop(否則邏輯矛盾)/(V2) 不含裸 `<SidebarHeader>`(WorkspaceBrand 該在 globalHeader,不重複;**例外**:同 file 用 `useSidebar`/`isMobile` = mobile-only Sheet 補品牌的 responsive fork → 豁免)/(V3) 不含 `<SidebarFooter>`(帳號入口家在 globalHeader 右、收成 Sheet 鏡像到 Sheet header 右,**不該**用 sidebar footer〔那是 primary-sidebar 慣例〕;**無** isMobile 豁免)。trigger 用 tag-boundary `([^A-Za-z]|$)` 避 prefix-extended 名誤觸。World-class cite:GitHub repo sidebar 無 header + Gmail / Figma brand 在 global top bar + Material modal nav drawer「account switcher 放 drawer header」。Hook `lib/_app_shell_primary_header_consistency.sh`(V1-V3)write-time exit 2 BLOCKER(via chrome_header_dispatcher;原 check_app_shell_primary_header_consistency.sh fold,可 escape `// @app-shell-primary-header-allow:`)。對應 `app-shell.spec.md`「WorkspaceBrand 放置 SSOT」+「帳號入口(Account entry)放置 SSOT」段(均含 Responsive 精修子句)|
| 57 | **M29 DS Anchor Preflight enforcement coverage**(2026-05-26 codify per user verbatim「該程式化的都沒程式化」)| 對每 `*.tsx`(production code,非 stories / test)grep wrap DS primitive(`<Sidebar>` / `<AppShell>` / `<DataTable>` 等)→ verify 過去 30 turns transcript 含 `Grep`/`Read` tool call hit `packages/design-system/src/**/*.spec.md` 或 `*.stories.tsx`,OR 檔頭含 `@story-baseline:` marker 或 inline 3-column owner table。Hook `check_ds_anchor_preflight.sh` write-time P0 BLOCKER(exit 2,2026-07-07 升級;escape `GOVERNANCE_BYPASS_DS_ANCHOR=1` audit-logged)。對應 meta-patterns.md M29 + self-verify.md Pre-edit phase。錨例:2026-05-26 App.tsx 漏 SidebarTrigger / collapsible / startIcon mock-drift = M29 hook 不存在使 infra 沒攔 |
| 58 | **Fork-user 治理鏈 enforcement**(主路徑 = committed-config + exact npm corpus，免 plugin；2026-06 cloud 記錄僅是歷史 evidence，見 `reference_cloud_governance_loading`)| SessionStart hook `check_plugin_fork_health.sh` 偵測 fork repo(cwd 非 DS repo + `package.json` 含 `@qijenchen/design-system` dep)治理鏈缺失：已有 committed launcher 時提示公開入口 `npm run setup:all`（舊 snapshot 才 fallback `setup:governance`）；launcher 缺失則要求 reviewed full-snapshot template bootstrap PR，禁止 mutable self-bootstrap。r2 remote marketplace diff 只作 legacy plugin compatibility notice，不代表 package freshness。**Provider-neutral chicken-egg 層**:manifest-declared shared launcher set 只 materialize 一次到 `governance/bin/`;Claude/Codex config 共同引用它。`fork-governance-dispatcher.sh` 讀 exact npm manifest 跑官方 hooks(exit 2 轉發 BLOCK;manifest 缺時 native feedback 不 brick)，相容檔名 `inject_fork_governance_preamble.sh` 現在只做 SessionStart 唯讀驗證並輸出短狀態，不載入 full-text preamble；最終 authority 仍是 installed checker + protected CI。`test-fork-governance.mjs` 必證刪除整個 `.claude` 後 Codex dispatcher 仍完整，避免 provider lifecycle ownership 交叉依賴。|
| 59 | **Target-bound authority preflight scope coverage**(2026-05-26 extend；2026-07-26 authority reconciliation)| `check_substantive_edit_approval_preflight.sh` scope 涵蓋 `packages/design-system/src/**`、`apps/**.{tsx,ts,css}`、`node_modules/@qijenchen/design-system/**`，每個 MultiEdit target 都經同一 classifier；工程 remediation AUTO，產品／UI／UX真取捨需 exact decision，unknown fail closed。Audit verify 三 scope + allowlist `*.stories.tsx`/`*.test.*`/`scripts/*`。對應 memory/feedback_ship_then_revert_anti_pattern.md SSOT |
| 60 | **M26 propose-without-benchmark enforcement**(2026-05-26 backfill)| UserPromptSubmit hook `check_propose_without_benchmark.sh` 偵測 user prompt 含 trigger keyword(propose me / give me options / 列 option / 建議…方案·做法 / 哪個比較 / 對齊哪家 / 比稿 / 要怎麼設計 / 該用哪個 / design tradeoff / recommend / world-class 對照;2026-05-26 收緊,拿掉 看法/design/behavior 等籠統詞防 false-positive)+ 過去 20 turns transcript 中 `WebFetch`/`WebSearch` tool_use count < 2 → P0 措辭 directive context inject(exit 0 by design — UserPromptSubmit 不可 exit 2)「propose 前必跑 ≥3 source benchmark」。對應 meta-patterns.md M26 + propose-options/SKILL.md。錨例:2026-05-05 user 反覆糾「為什麼每次都沒 webfetch 只憑印象」 |
...
| 62 | **Fork-user Netlify onboarding canonical**(2026-07-22 三修 → CLI supply-chain fail-closed + Dashboard/manual,SSOT `governance/memory/reference_deploy_targets.md`)| Audit product-workspace + fork repos:(a) `netlify.toml` 含 X-Robots-Tag noindex header **+ `[[edge_functions]]` path="/*" function="basic-auth" wire**; (b) `netlify/edge-functions/basic-auth.ts` 存在且讀 `STORYBOOK_BASIC_AUTH`，未設／格式錯誤回 503 fail closed，設定有效但帳密缺失／錯誤回 401 + WWW-Authenticate，只有正確帳密才放行；`.storybook/manager-head.html` 不含 Identity widget; (c) `scripts/setup-netlify-access.mjs` 必對 `netlify-cli@26.2.0` / 5 high 輸出 `NETLIFY-CLI-AUTO-INSTALL-BLOCKED-001`，禁任何全域安裝、自動下載 runner 或預裝 CLI 呼叫，只能唯讀診斷 + Dashboard import/env/branch-deploy 指引，且未驗證人工設定前 exit 2; (d) `package.json` 有 `setup:netlify`; (e) product instruction canonical 需明說 GitHub Dashboard import、免費 Edge Function Basic Auth、SEO headers、deploy 驗證、manual-required 與各 cloud 環境同路徑，並禁寫「`_headers`/Dashboard Basic Password 免費」「Identity 已 deprecated」或「CLI 已安全／已自動安裝」。Generated provider view 仍不是 authority/edit target。 |
| 63 | **Deploy URL auto-reply infra**(2026-05-26 codify per user verbatim「完成部署之後都應該自動回吐部署的連結,每次必定自動回,不論是現在這個 session 還是其他的」)| Audit product-workspace + fork repos:(a) `scripts/deploy-url.mjs` 存在 + executable + 輸出 production 或 preview URL JSON;(b) `package.json` 有 `deploy-url` script wired;(c) canonical hook `packages/design-system/ds-canonical/hooks/inject_deploy_url_after_push.sh` 已由 `registrations.json` 註冊，偵測 `git push origin <branch>` 後 auto inject deploy URL into AI context;(d) hook scope skip `push --delete`(branch cleanup 非 deploy);(e) hook silent skip if no `.netlify/state.json` 或 `scripts/deploy-url.mjs`(DS repo 自身 + 非 Netlify-wired repos 不誤觸發)。Cross-session benefit:provider adapter materialize hook view 後任何 fork runtime 自動受惠 |
| 64 | **Protected-release five-step SSOT propagation canonical** | 以 `infra/governance/release-workflow.json` 驗唯一 standard graph:`pr-checks → merge → publish → readback → consumer`。(a) one task = one branch + one PR，main ruleset 禁 direct push；(b) `pr-checks` 只以 required CI + conversation resolution 完成；(c) `release:auto` 依 Standing Authorization續跑 protected merge、npm publish、GitHub/npm exact-version readback與 template/WM exact-version PR/readback，不等 chat trigger；(d) exact version / immutable publish / protected-main consumer readback fail closed；(e) SessionStart mutation、mutable dist-tag dependency、PAT/direct-main fallback 或 skill-specific propagation 副本皆禁止。CI evidence 的 standard 唯一路徑是 `npm run audit:ci-evidence-observe`：一次 live observer 綁 `active PR tree = squash-merge tree = tag tree`、required-check producer、repository_dispatch release/mirror、immutable Release/BOM、三包 npm SLSA provenance，並原子寫 dim 64/66 共用的 observation digest；不經 staging/import/unsigned observation。Retired `release:preflight`、candidate freeze、offline attestation、preview/canary、model certification、soak、fleet promotion可作 optional assurance/history，但不得進 standard blocking graph。 |
...
| 66 | **Immutable publish + exact consumer readback** | **CI-ENFORCED / CI evidence**。Audit:(a) `infra/governance/release-workflow.json` 的 publish workflow 只從 protected-main release action發布三個 exact npm packages；(b)同一 release 的 GitHub Release + npm exact version均有 live readback；(c) template 與 WM 只透過 reviewed exact-version PR 更新，兩個 protected `main` 的 lockfile readback皆命中該版；(d) mirror source 必是 `.github/workflows/mirror-to-published-template.yml` 的 `repository_dispatch` success，不得誤寫 `workflow_run`；(e) template/WM audit SSOT 都是單一 required check `Verify consumer`，lockfile 的 version/integrity/resolved 必逐包等於 npm live readback；(f) consumer propagation 由 `consumerctl` 的 `governance-release` exact-version PR 契約驗證，禁止 PAT 與 direct-main；(g) credentials 只可暫停當下 action，完成後 `release:auto` AUTO resume。dim 66 必與 dim 64 內嵌同一 observation digest，任一混版即 fail closed。只讀 workflow 原始碼、optional preview/canary／soak、模型判斷或手寫 JSON 不構成 five-step PASS；未掛載外部 surface = `UNOBSERVED`，不得冒充完成。|
...
| 68 | **Stories-vs-spec canonical drift**(2026-05-27 codify per user「DS 自己 stories 教錯 = consumer 抄 stories 抄錯」root cause)| 對每 component `*.stories.tsx` + `*.anatomy.stories.tsx` + `*.principles.stories.tsx` grep:含 `<SidebarHeader>` 內 `<ItemAvatar>` / 同類 spec 明文禁止的 pattern → 違反(DS 教 consumer 錯 pattern,類似 2026-05-27 sidebar 3 stories 在 WorkspaceBrand 用 ItemAvatar 違反 chrome header canonical)。**主防線 = R8 registry**(`packages/design-system/ds-canonical/references/story-baseline-registry.json` requiredHelpers / antiPatterns + canonical `check_story_invariants.sh` R8/R9 機械攔)+ canonical `check_chrome_header_avatar_canonical.sh` / `check_sidebar_menu_button_implicit_wrap.sh` 具體錨例 hook;`// @canonical-pattern:` / `// @anti-pattern:` 文字 marker **降格為建議性輔助**(2026-06-12 R2 拍板 — 全庫僅 sidebar.stories 2 處採用且無機械強制 = 紙防線,不再「必加」;存在時 audit 順驗其與 spec 一致)。配 Dim 53(spec-vs-code)+ 本 dim(spec-vs-stories)雙向 drift verify;系統性 stories drift audit run via design-system-audit Dim 68 batch sweep |
...
| 71 | **Consumer DS primitive misuse anti-pattern**(2026-05-27 per user「做產品真的要能使用跟 ds repo 一模一樣的元件」)| 對 consumer `apps/**/*.{tsx,ts}` production + stories grep:`<RadioGroupItem>` 無 `<SelectionItem control={...}>` wrap / `<LinkInput placeholder=...>` 無 `value` prop / Overlay story 無 `defaultOpen` 視覺 snapshot 看不到 content / **硬寫色值-字級-shadow 繞 token(`bg-[#hex]` / `text-[14px]` / `shadow-md`,2026-06-02 Pattern 8,CF conformance-model 主防線)** / **AppShell slot 餵 raw element(`sidebar={<div>}` 等,2026-06-12 Pattern 9,app-shell.spec:296-299)** / **手刻 RAW primitive 取代 DS 元件(raw `<table>`+`<thead>`/`<th>`→`<DataTable>` / `<img rounded-full>`→`<Avatar>` / native `<select>`→`<Select>` / `<hr>`→`<Separator>`,2026-06-16 Pattern 10,user 抓 fork prototype 手刻 table 不用 DataTable;反 pattern 由 `build-ui-canonicals.md` ❌→✅ 對照表 SSOT 驅動;落實「優先消費既有元件」大原則機械閘 — 補既有 pattern 只抓「DS 元件用錯」、漏「根本沒用 DS 元件亂刻 raw HTML」之洞;`@ds-misuse-allow:` 有理由可豁免)**。`DataTable` 單欄是合法 business schema,column count 不得作 misuse 訊號。`Empty` 的 `description` 由 `EmptyProps.description: string` type-level authority 強制，不重複建立脆弱 JSX regex hook。`CircularProgress size={number}` 是公開 API 且 spec 明列 16/20/32/48 合法情境，尺寸是否適合 host 需 context judgment，禁止用 literal-number regex 誤殺。**Native table semantic classifier(2026-08-02 technical narrowing)**：只有同檔同時可證 `contentEditable` 文件 host + 封閉 `TABLE/THEAD/TBODY/TR/TH/TD` allowlist 被 sanitizer 實際使用 + attribute stripping + `editor.innerHTML` sanitize 後才交 `onChange`，才分類為 WYSIWYG document table（非 app data grid）；普通 raw JSX table、缺任一證據或 passthrough `sanitize*` 仍 BLOCK，完整判準 owner=`build-ui-canonicals.md#native-table-語意分類app-data-grid-vs-rich-text-document`。**判斷層補掃(2026-06-12 fork 四不像 + 先驗滲漏 anchor;hook 零誤判原則不上 regex)**:(a) page-level entry 手拼 raw 殼而無 `<DS.AppShell>`(對照 app-shell.spec「何時用/不用」表;愈不用 DS 愈不觸 hook = 反直覺洞)(b) 手刻 right panel(`w-[Npx] border-l` div,該走 `aside={<AppShellAside>}`)(c) ChromeHeader/頁頂 tabs 沒走 `withTabs` lockstep(header-canonical.spec「withTabs 連動」段)(d) BulkActionBar 內 primary 鈕 / 浮層 shadow(bulk-action-bar.spec 禁止事項 — regex 無法 bound 到 actions slot,判斷層接)(e) FileViewer toolbar 加鈕不走 `onCapabilitiesChange`(file-viewer.spec:183-194;R9 skip-list 豁免該家族故無寫入網)(f) chrome 面(ChromeHeader/SidebarFooter)title 下第二行(registry 簽名涵蓋 stories;fork production code 由本 dim 接)。Hook `check_consumer_app_invariants.sh(r3,2026-06-11 merge)` BLOCKER。Per-violation cite spec.md file:line。Escape `// @ds-misuse-allow:` |
| 72 | **DS API surface tightening**(2026-05-27 — 治標 vs 治本)| Hook 71 偵測 anti-pattern 是 lint 層攔截;治本要 DS API design 強到 misuse 即 fail tsc。Audit:逐 component review API surface — 真正必填的語意 prop 應在 type 層必填(例如 `EmptyProps.description`);不得在無 owner contract 時把公開 numeric size 收成 enum,也不得用 column count 代理內容品質。`DataTable` 單欄是合法 business schema。配套 codify in `tightening-roadmap.md`(若存在;不存在則以 `props-naming.md` + 各 spec API 段為準,對齊 audit-prompts.md dim 72),分 quarter ship。對應 Dim 71 是攔當前 misuse,本 dim 是消除未來 misuse 可能 |
| 73 | **Full-story visual+interaction sweep enforce**(2026-05-27 codex M31 P0 finding)| Audit report JSON `storyResults.length === manifest.totalStories`(動態讀 `ds-story-manifest.json`,禁 hardcode 數字 — 2026-07-04 dim 73 修 stale 916)。Sample < totalStories = reject(per user「不准抽樣」)。Hook `check_full_story_visual_interaction_sweep.sh` PostToolUse audit-report.json BLOCKER。Escape `"_sampling_allowed": "<rationale>"`(極罕見)|
...
| 75 | **Exact-version consumer setup + legacy Claude compatibility diagnostic**| Fork fresh/cached checkout 必顯式跑公開入口 `npm run setup:all`:先依 committed exact package/lock 安裝、驗簽並執行 role hard gate，再 provision canonical peer CLI；`setup:governance` 只是內部／舊 snapshot primitive。SessionStart 只讀驗證，不得 mutation。`check_plugin_fork_health.sh` 的 marketplace diff 僅是 Claude legacy plugin compatibility notice，**不是** package freshness authority，也不得導向任何 mutable dist-tag。治理升級只能由 reviewed exact-version PR 帶入完整 lock/adapters；merge 後再跑 setup。`sync-all` 僅能套 authenticated exact-version transition，不能把 remote marketplace metadata 當候選版本。|
| 76 | **Escape marker abuse cap**(2026-05-27 per user「不亂加 escape markers」)| Consumer file 10 escape markers 累計 ≥3 distinct types OR ≥5 total → BLOCK。修法 3 選 1:重構走 DS canonical / 拆 file / env override `GOVERNANCE_BYPASS_ESCAPE_MARKER_AUDIT`。Hook `check_escape_marker_abuse.sh` enforces escape philosophy「rare per-line documented exception,非 daily tool」|
...
| 82 | **Consumer app story title `Apps/<app-name>/...` namespace**(2026-05-28 codify per template create-app duplicate-id anchor)| `check_consumer_app_invariants.sh(r4,2026-06-11 merge)` P0 BLOCKER:consumer apps `apps/<name>/**/*.stories.{tsx,ts,mdx}` 的 `title:` field 必開頭 `Apps/<app-kebab-name>/...`(per `packages/design-system/ds-canonical/rules/story-rules.md`「Title 命名 2-namespace canonical」)。錯 prefix → Storybook glob 撈到 2 個同 id story → build duplicate warning + 只顯第一個 → 新 app 在 sidebar 不可見。Hook 從 file path 自動推 expected prefix。DS-internal stories(`packages/design-system/**`)走 `Design System/...` 另條 canonical,out of scope。Escape `// @app-story-title-skip:`。Anchor:2026-05-28 npm run create-app verify-flow-test e2e 抓 4 collisions,`scripts/create-app.mjs:patchStoryTitles()` 修生成 + hook 防手動 edit |
| 83 | **Fork-context runtime + naming SSOT**(2026-05-29 codify per user「fork 後 fork user 在自己獨立環境下仍能如預期運行」;net-new vs dim 62/63/64)| Net-new aspects 不在 dim 62/63/64:**(a)** Hook fire integrity in fork cwd — verify canonical registrations + every provider adapter's event/environment mapping(`GOVERNANCE_*` core;legacy native env 只在 adapter edge)在 fork cwd 正確 fire;(b) each registry-declared generated product instruction entry 可 cross-load installed `ds-canonical/rules/meta-patterns.md`，並明定自己只是 delivery/discovery projection、not authority/do-not-edit;(c) Naming SSOT 3 層 — DS-internal source dir(`template/ds-product-template/`)≠ published GitHub Template Repository(`ajenchen/ds-product-template`)≠ fork user new repo 名(self-chosen)清楚 documented in `template/README.md`「命名 SSOT」段;(d) **跨 repo 交付 canary** — `node scripts/verify-published-deploy.mjs`:L1 mirror workflow 最近 run 必 success + L2 published `.storybook/main.ts` === 本地 template + L3 `--live` playwright render 非空白。Why net-new:dim 66 只驗 local fixture build,dim 83(a)-(c)只驗 source — 都沒驗 source 真送達 live。Skill chain:`/deep-audit-cross-codex` Phase 0 cwd detection 切 ds-repo / fork-user-repo 2-mode 跑此 dim |
...
| # | Audit | What it catches |
|---|-------|-----------------|
| 84 | **Mirror-build integrity + 2-scenario E2E**(ds-repo-mode-only)| `node scripts/test-2-scenario-architecture.mjs`:20 test case + Mirror M0-M7 — published-template mirror allowlist 正確(scaffold + apps/template + scripts subset 全送達；fork-relevant skills/commands/agents 僅以 manifest name-allowlist 產生 **generated delivery/discovery projection，not authority/do not edit**；非 fork 名 = 洩漏擋下；治理本體走 npm ds-canonical,2026-06-18 C-prime)+ package.json transform(`workspaces` + DS dep `*`→`^X.Y.Z`)+ Scenario A/B 結構一致。錨例:2026-05-29 mirror 從 5/26 默默失敗數週稽核沒抓到 |
| 85 | **ds-canonical source → provider/package view freshness**(ds-repo-mode-only)| `node scripts/sync-ds-canonical.mjs --check`:canonical `packages/design-system/ds-canonical/` 是唯一 semantic owner；npm package 與 provider skill/hook/reference/rule surfaces 都必是 deterministic generated delivery/discovery projections，byte/manifest provenance 不得 stale，也不得反向覆寫 canonical。錨例:本 session 13 處死引用同時有 stale generated 副本 |
| 86 | **Provider plugin manifest + generated-view 一致**(ds-repo-mode-only)| `node scripts/plugin-structure-validate.mjs`:provider plugin manifests valid + version 對齊 + generated skills/hooks/rules view 與 canonical source/provenance 一致；consumer fork 僅接收明列的 delivery subset。任何 provider view 都不是 SSOT 或 edit target。其餘 mirror allowlist 走 dim 84 |
...
| 88 | **Infra-self ref integrity**(ds-repo-mode-only;2026-05-30 加,根因防線)| `node scripts/check-dangling-infra-ref.mjs --check`(governance docs 引用的 hook/script 必存在磁碟 OR 同行 annotate retired/未實作;認 `check_X.sh`→`_X.sh` lib-consolidation form)+ `node scripts/check-skill-deadref.mjs --check`(禁 provider instruction 的 brittle line ref + 已移除 section deny-list + product `*.spec.md` 把 generated provider instruction adapter 當 authority)。錨例 2026-05-30:抓 40 處死 hook ref + env-smoke set-e bug；2026-08-02:抓 33 specs / 41 refs 的 provider-view authority drift。M7/M34 broad-vs-narrow gap closed。**2026-05-31 infra-audit 修**:SCAN_DIRS 補 `governance/memory`(原漏=memory infra ref 零防線)。**已知 P3 限制**:annotation-bypass — 同行含 retired/folded/未實作 字樣即 Bucket A,未驗該 hook 真曾存在(fabricated「(retired)」ref 可繞);需 git-history verify 才全閉,暫接受(adversarial-only 風險,queued #9)|
...
| 90 | **Layout-space macro 合規 sweep(judgment 層,DS-src 盲區)**(2026-07-01,user「確保都遵循沒偏移」)| **LLM adversarial per-component**:並列 **區塊/卡片/表單欄位** 之間的 macro 垂直/水平 gap、chrome 水平內距、header→content、content→action button —— 該用 density-scaling **layout-space token** 卻硬寫固定 `gap-N`/`space-y-N`/`p-N`/`px-N`(規則 1-6)。**判準邊界**見 `tokens/layoutSpace/layoutSpace.spec.md`「該用 token vs 刻意固定」段:**只抓 macro 結構違規,禁 flag 合法 magic number**(同質 list 列間距 / micro-icon / 刻意視覺平衡 / 元件內部固定如 FieldGroup)。Consumer hook 只對 `utility-registry.json#/spacing/canonical_micro_geometry` 可用結構證據證明的 inline item/control、semantic list、toolbar、rule-5 grouped fields 放行 `gap-1/2`;一般 macro `gap-1/2` 仍 BLOCK。**每 finding 必對抗二次驗證殺假陽性**(2026-07-01 錨例:sweep 35 raw → 8 confirmed → 逐項覆核剩真違規 Sheet gap-4;over-flag ProfileCard/apps-scaffold/list-gap 被判非違規)。magic-number hook skip DS-src → 本 dim 仍是唯一 DS-src layout-space judgment 防線，也覆核 consumer 無法機械證明的邊界案。SSOT `layoutSpace.spec.md` 6 規則 + 邊界段 |
| 91 | **Failure-class 治理覆蓋常設閘(每類病根必有防線)**(2026-07-10,user「建立治理的基本原則,稽核應確認」)| `node scripts/audit-failure-class-coverage.mjs --check` — canonical registry(`packages/design-system/ds-canonical/references/failure-class-registry.json`,20 class 起)每 class 必 status ∈ {protected, remediating(必帶 plan), judgment(必帶 auditDim 兜底)};registry 缺失 = fail-closed。新 failure class(deep-audit / user / peer provider 抓)必加 row,否則紅燈。Coverage matrix 是 derived evidence，寫入 Git-owned `<git-dir>/governance-runtime/evidence/audit/failure-class-coverage-matrix.json`，不能反向成為 authority |
...
1. Read `AGENTS.md` shared bootstrap + applicable canonical `packages/design-system/ds-canonical/{rules,references}/` fully + `git status --short`；若 provider instruction/view 存在，只驗其 projection freshness，不把它當 authority
2. **Build baseline**(任一 fail → fail-closed engineering remediation；依既定 retry/evidence 仍重現才 technical blocker):
   - `npx tsc -b` — 0 errors
   - `npx vite build` — `✓ built in`
   - `npm run build-storybook` — clean
3. **Mechanical content-quality baseline**:
   - `node scripts/audit-content-quality.mjs --check` — `✅ No content drift`(category 集以 script `violations` 鍵為準,禁 hardcode 數)
   - `node scripts/extract-canonical-rules.mjs` — `✅ All extracted rule keywords covered`
   - violation → 列 P0
4. Build fail → 不跑依賴 build 的 dims；agent 先定位並在 frozen scope 內修復後重跑。只有 login/credential/額外付費等 human-only boundary 或依重試規則形成真 blocker 才停止
...
**強制 chain**:`/design-system-audit --deep` 跑時 Phase 1 前自動跑 `node scripts/audit-preflight.mjs`(對應 SSOT `governance/memory/feedback_audit_discipline_full_sweep_deterministic_preflight.md`)
...
Launch audits through the current provider's registered parallel-worker mechanism while preserving identical prompts and isolated result artifacts. Use prompts in [references/audit-prompts.md](references/audit-prompts.md). If no such mechanism exists, run serially without reducing coverage.

**Every audit prompt declares three metadata lines at top**:
- **Type**: `Absolute` or `Consistency` (per canonical audit rules「Consistency 類稽核」)
...
- 對應 SSOT:`memory/feedback_audit_discipline_full_sweep_deterministic_preflight.md`
...
若 dim 依既定 staged/batched compute 仍真不可能全掃，標 `REVIEW-BLOCKED` + exact evidence；不得寫 sample escape clause，也不得把 compute／工程策略選擇丟給 user。
...
(c) Cross-check `packages/design-system/ds-canonical/rules/` + `packages/design-system/ds-canonical/references/structural-token-retention.md` + tokens/{name}.spec.md — 屬 forward-looking / palette completeness / dark mode pair 嗎?
...
**Mechanical strength**:
- `stop_self_audit.sh` NO-SAMPLE 偵測(keyword:sample top N / sampled top / subset / pick top N / top hot / too many to scan / sampled components)= soft WARN 下 turn 注入(`inject_pending_self_audit.sh`);SKIP / heavy-agent pattern + BLOCKER 級由 `check_audit_sample_escape.sh`(下行)exit 2 承擔
...
| **P2E (AUTO)** | engineering rule / audit coverage / governance ownership / cross-cutting refactor(helper extraction 41 files) | agent 依 frozen scope、evidence、tests、rollback自行收斂；task／deliverable 明確要求時才加 maximum-assurance review |
| **P2H (HUMAN-ONLY)** | Rule B scope / new UI rule / public-vs-internal semantic classification 等會改產品／UI／UX SSOT且仍有真取捨 | batch-at-end 給 user 決策，不混入工程選項 |

### Authority gates — reports are receipts, not approval milestones

| # | When | Action |
|---|------|--------|
| 1 | Triage 完 | P0/P1/P2E 連續執行；P2H 若有，全部 audit 完後一次彙整 |
| 2 | Audit surfaces pattern 未在 canonical rules/spec | 工程 guard自動 draft+review+落地；新產品/UI/UX semantics 才 P2H |
...
每 fix group:Edit(非 Write)→ `npx tsc -b` pass(禁 `--noEmit`,root tsconfig `files: []` silent pass)→ commit 描述性 message。Typical groups:cva drift / Spec Rule A / a11y / Anatomy / canonical governance contradiction。

### Phase 3.5 — 進階 6 維稽核 D3-D6(對齊 canonical audit taxonomy)
...
| 3.5b | D3 效能 | `/performance-audit` | 修實作與工程 canonical auto；產品／UI／UX SSOT 真取捨才 STOP |
| 3.5c | D4 UX | `/ux-audit` | P0 a11y 必修 / P1 triage |
| 3.5d | D6 原則自檢 | `references/principle-audit-protocol.md` 4 子維(合理 / 一致 / 無矛盾 / 完整)| 工程 canonical substantive AUTO；產品／UI／UX SSOT 真取捨 STOP；對齊 / 補 pointer AUTO;scan 前必讀「常見 FP 記憶」節 |
...
產出 short report(date / coverage / findings / deferred P2 / commits / next trigger);需跨 session 持久化的 audit 計畫 / backlog 寫 `governance/planning/` audit doc(memory 只留短 index pointer,per rule-placement Level 9)。

**Self-improvement capture(強制)**:每 audit 寫 3 行(無發現也寫「無」,不省略):
- 新 FP pattern + 回填位置(`principle-audit-protocol.md`「常見 FP 記憶」)OR「無」
- 新 meta-pattern + authority(P2E auto / P2H product-UIUX decision)OR「無」
- 修完矛盾 / user 糾正 + 回填 home(governance memory / canonical rule / spec)OR「無」

---

## Non-goals + common failure modes

**Non-goals**:不擴張 frozen scope重寫 spec/story或加 feature；不 skip evidence/authority gate；不 collapse output(user 要 file:line)。Scope 內工程 remediation 不因「不修 env」或 milestone而停。

**Common FP**:Agent hallucinate fix → cross-check file:line;Rule B 多 scope-N/A → 套 canonical scope default 不 stub;cva FP → 只在 spec/anatomy 與 code 矛盾才報;Story aria-label/cva → 排除;a11y over-flag → Radix 已處理,wrapper 不重複;skip authority classification → mechanical change scope 不清。

## References

- [references/audit-prompts.md](references/audit-prompts.md) — Subagent prompts for judgment dimensions；實際 coverage 以 91-dim matrix 與本檔 current table 動態核對，禁止靠歷史區間推定完整
- [references/historical-bugs.md](references/historical-bugs.md) — Bug classes indexed by audit
- [references/checkpoints.md](references/checkpoints.md) — Historical operational examples；若含 blanket MUST-ASK，一律受本檔與 shared-governance authority classifier 覆蓋
