...
| 66 | **Immutable publish + exact consumer readback** | Audit:(a) `infra/governance/release-workflow.json` 的 publish workflow 只從 protected-main release action發布三個 exact npm packages；(b)同一 release 的 GitHub Release + npm exact version均有 live readback；(c) template 與 WM 只透過 reviewed exact-version PR 更新，兩個 protected `main` 的 lockfile readback皆命中該版；(d) credentials 只可暫停當下 action，完成後 `release:auto` AUTO resume。只讀 workflow 原始碼、optional preview/canary／soak、模型判斷或手寫 JSON 不構成 five-step PASS；未掛載外部 surface = `UNOBSERVED`，不得冒充完成。|
