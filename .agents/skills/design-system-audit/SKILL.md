...
| 43 | **Rule note 品質**(原則>結論 / 無中英夾雜)| AI judgement sample-based:讀 `.principles.stories.tsx` Rule notes,判 (a) 是否「告訴讀者原則為何」而非「只說結論」;(b) 是否無中英夾雜(技術術語例外)。對齊 `../story-writing/references/example-selection.md`「Rule note 品質」 |
...
| 64 | **Protected-release SSOT propagation canonical** | Audit:(a) one task = one branch + one PR，main ruleset 禁 direct push；(b) release-affecting PR 內含 exact version bump、generated views、BOM 與 `release:preflight` commit/tree attestation；(c) required checks、conversation resolution、preview/canary 與 external protection readback 全綠後依 Standing Authorization merge，不等 chat trigger；(d) Release workflow 用 provenance 發 immutable package；(e) successful Release 只透過短效 GitHub App 開完整 template mirror PR，fleet 再依 rings 對 WM/consumers 開 exact-version PR；(f) 無 SessionStart mutation、mutable dist-tag dependency、PAT/direct-main fallback 或 skill-specific propagation 副本。整鏈 cover knowledge-prune/deep-audit/一般 dev/新元件/bug fix，provider-neutral CI 為 authority。 |
...
| 66 | **Immutable cross-repo promotion + visual parity gate**(2026-07-25 provider-neutral 收斂)| Audit:(a) `release.yml` + `release-finalize.yml` 只從 protected main 建立並讀回簽章 immutable release/BOM/exact npm identities；(b) `mirror-to-published-template.yml` 只消費 exact finalizer evidence，以 least-privilege GitHub Writer App 開 template PR，禁 PAT、direct-main、mutable tag；(c) `consumerctl plan/check/apply-fanout` 依 ring/wave 產生 reviewed exact-version `governance-release`，consumer `sync-design-system.yml` 重建 exact release 並只開 Writer App PR；(d) protected-default `governance-anchor` 由獨立 Check App 發 required verdict，composition/dogfood/visual assertions 全通過；(e) 完整遠端 readback 與同一 frozen deep-audit run 的 CI evidence receipt 綁定 candidate commit、BOM、PR、checks。只讀 workflow 原始碼、模型判斷或手寫 JSON 不構成 PASS；任何鏈節未啟用一律 pending/fail-closed。|
...
| **P2E (AUTO + maximum-assurance review)** | engineering rule / audit coverage / governance ownership / cross-cutting refactor(helper extraction 41 files) | agent 依 frozen scope、evidence、tests、rollback自行收斂 |
...
產出 short report(date / coverage / findings / deferred P2 / commits / next trigger);需跨 session 持久化的 audit 計畫 / backlog 寫 `governance/planning/` audit doc(memory 只留短 index pointer,per 8-home Level 9)。
