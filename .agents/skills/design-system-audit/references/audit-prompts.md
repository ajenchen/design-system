...
Per-overlay grep `components/(Dialog|Sheet|Popover)/*.tsx`(非 stories)反 pattern:`(flush|naked|bare|stripped|unpadded|noPadding|paddingless)\?:\s*boolean` 在 body component。對齊 Material/Atlassian/Mantine/shadcn 主流。違反 = list-as-region 該 consumer override(`!px-0 !pt-0`)+ 自管 list outer wrapper。Hook `lib/_overlay_handcraft.sh` Check 6 同源(`post_edit_dispatcher.sh` 消費;原 check_overlay_handcraft.sh 2026-05-13 fold)。
...
**MUST chain `npm run a11y:check`**(`scripts/audit-a11y.mjs`:headless axe 對每 story 跑 wcag2a/wcag2aa 含 color-contrast AA + baseline-diff gate);contrast ≥ 4.5:1(text)/ 3:1(large text / UI element)。對齊 Carbon AVT 每 PR + Atlassian linter integration。addon 已裝(`packages/storybook-config/addons-preset.ts`)為 dev-time 輔助。
...
## 90. Layout-space macro 合規 sweep(PURE-JUDGMENT,2026-07-01 dim 加入;2026-07-10 Validator C 抓到 rubric 漏補)

Your job(NO-SAMPLE,DS-src 全 component/pattern):LLM adversarial per-component 掃「並列區塊/卡片/表單欄位
之間的 macro 垂直/水平 gap、chrome 水平內距、header→content、content→action button」—— 該用 density-scaling
**layout-space token** 卻硬寫固定 `gap-N`/`space-y-N`/`p-N`/`px-N`(layoutSpace.spec.md 規則 1-6)。
