...
同一天、同一條規則,還在**量具**上發作三次(它們不在發布鏈上,但同根):
...
- 我自己掛的監看:`release:status --json` 的 stdout 被一行人話診斷汙染成不可解析,python 失敗被 `|| true` 吞掉,30 分鐘零事件 —— 而零事件看起來跟「沒有變化」一模一樣。診斷改走 stderr;監看解析不出來時明講「監看壞了,不是沒有變化」。
