...
- Hover / active 背景**依 host 深淺分支**(2026-07-07 補措辭缺口,對齊 tag.spec + code 既有行為):**solid** colored host → 同色系 hover / active token(Tag solid blue dismiss → `--blue-hover`;solid neutral → `--inverse-neutral-hover/active`);**subtle** colored host(categorical step-1 淺底)→ 維持預設 `--neutral-hover/active`——淺底上中性遮罩對比即足,同色系 step-1 遮罩幾乎不可辨。逐 variant 對照表 SSOT 見 `tag.spec.md`「Dismiss 行為(Inline Action 客製)」段
