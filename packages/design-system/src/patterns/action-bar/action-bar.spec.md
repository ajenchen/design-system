...
分隔線**只有四個正當理由**。每一條線都必須對得上其中一個；對不上就是裝飾，不得存在。

| # | 理由 | 判準 | 加不加 |
|---|------|------|--------|
| 1 | **群組結束** | 群組溢出（···）的右側，無論右側是什麼 | **必加** |
| 2 | **邊界看不見** | 相鄰兩側視覺同類（都無框 / 都有框），不畫線就分不出換群 | **必加** |
| 3 | **誤觸保護** | 關閉 / 解除鈕的**左側緊鄰其他操作按鈕** | **必加** |
| 4 | **功能域切換** | 兩側分屬不同功能域，且需要讓使用者看出這個切換 | **可加**（須具名理由） |

**視覺差異不是禁令**（2026-08-06 修訂）：有框 ↔ 無框的對比只會讓**理由 2 不成立**（邊界本來就看得見），**不構成禁止**。若同一位置存在理由 1 / 3 / 4，照樣加線。舊版寫「有框結尾 → 工具層 text ＝ 不加」是把「充分條件」寫成了「排他條件」，與同表的「功能性分群可選」及「群組溢出無論右側視覺類型」自相矛盾，故移除。

**理由 3 的三個邊界情況**（與實作一致）：

- 關閉鈕**左邊沒有操作按鈕**（它是唯一操作）→ **不加**。沒有東西需要被隔開。
- 關閉鈕與其他操作之間**隔著非操作內容**（如 BulkActionBar 的「已選 N 筆」文字 + 既有分隔線）→ **不加**。隔離已經存在，再加一條是重複標記。
- 關閉鈕**不在操作列末端**（如 BulkActionBar 置於最左）→ 理由 3 不適用，其左側是容器邊緣而非其他操作。

**理由 4 的具名要求**：加線者必須在該處寫下它區隔的是**哪兩個功能域**（程式碼註解或 story label）。說不出來就是裝飾。「看起來比較整齊」「對稱」「習慣這樣」都不是理由。

**優先序與去重**：理由 1 高於其他（群組邊界即使右側是 text 工具層也必加）。多個理由同時成立時**只畫一條線**，不疊加。

**孤立檢查（結果導向的自我驗證）**：任何一顆按鈕若左右兩側都有線，代表其中一條沒有真正的理由 —— 回頭逐條檢查理由，刪掉對不上的那條。

**落點歸屬（誰放這條線）**：由 DS 元件自動放，consumer 零自刻。已落地：Dialog 標題列（`dialog.tsx:184-193`）、Notice 家族（`notice.tsx`，Alert / Toast 共用同一基座因此行為一致）、ButtonGroup（`ButtonDivider`）。Consumer 自刻是「漏放 / 幾何漂移」的根因。
...
「左搜尋 / 右操作」的資料工具列(DataTable / AppShell / AgentPanel 整頁示範皆此型)空間不足時**先讓搜尋框讓路**,操作群不縮;只要空間放得下「下限 + 間距 + 操作群」,列就不溢出:
...
| 空間夠(內容寬 ≥ 下限 + 間距 + 操作群)時列不溢出;最後一顆操作鈕右緣 = 內容右緣 | 列 `min-w-0` | 機械閘 `scripts/action-bar-toolbar-invariant.mjs`:五個寬度 × 四支示範量像素;空間連下限都放不下的格子只驗「下限守住」並印出(見下一列) |
...
[儲存(framed) | 複製(framed) | 🗑(text danger)] ┆ [↺(text) | ···global] ┆ [✕]
                                                  ↑                        ↑
                                              text→text 角色接壤      關閉保護
      有框→無框：沒有對上任何理由，所以不加（不是因為「禁止」）
```

- 儲存、複製 = 有框業務（tertiary）；🗑 = 無框業務（text danger）→ 同屬業務層、無功能域切換 → 不加
- 🗑（業務）→ ↺（工具）= 兩側都無框，邊界看不見 → 必加（理由 2）
- ↺ → ···global = 同屬工具層、視覺同類但無換群 → 不加
- ✕ 在末端且左邊緊鄰 ···global → 必加（理由 3 誤觸保護）；···global 左側那條屬理由 2，兩條各有理由，不是孤立
...
這裡要不要分隔線？（逐條對理由，對不上就不加）
├── 左邊是群組溢出（···）嗎？                    →  必加（群組結束）
├── 兩側視覺同類、不畫就分不出換群嗎？            →  必加（邊界看不見）
├── 右邊是關閉鈕，且它左邊緊鄰其他操作按鈕嗎？    →  必加（誤觸保護）
│     └── 只有關閉鈕 / 中間隔著文字 / 關閉鈕不在末端  →  不加
├── 兩側分屬不同功能域且需要讓人看出來嗎？        →  可加（須寫明是哪兩個域）
└── 以上皆非                                     →  不加（有框↔無框的視覺差異本身不是加線理由，也不是禁令）
