...
- 底部中央:水平置中、貼齊媒體底緣疊於其上(定位 class 見 `.tsx`)
- 間距:`gap-1.5`(6px)
- 點擊:`scrollTo(i)` 直接跳該張
...
**為什麼吃得到例外**:指示點的可視形狀是 **6×6 的圓**(`carousel.tsx:408` `h-1.5` + 未選中 `w-1.5`)。
6px 不是「有點難點」,是幾乎不可能瞄準;堅持相等等於宣告它不可用。這正是例外要接的那一類。
(現張的點加寬成 24×6,`w-6` —— 寬的那一軸已經夠瞄,所以水平外擴量兩種狀態相同,見下表。)

**外擴量**(`carousel.tsx:409-411`,`before:-inset-y-[9px] -inset-x-[3px]`):
...
dots 條貼在 `bottom-3`(12px)`carousel.tsx:378`,所以往下 9 仍在 carousel 根之內(實測留 3px),往上是媒體本身、沒有任何可點目標。
...
W3C APG carousel pattern 有兩條合法路:**tabbed**(dot `role="tab"` + slide `role="tabpanel"` + 完整 tabs 鍵盤契約)或 **grouped**(slide `role="group" aria-roledescription="slide"` + dots 為 group 內原生 button、現張標記)。本 DS 採 **grouped**(<https://www.w3.org/WAI/ARIA/apg/patterns/carousel/>),對齊 Embla 官方 Accessibility plugin(plain button + label,無 tab roles,<https://www.embla-carousel.com/docs/plugins/accessibility>)與 bluecadet embla-carousel-a11y(follows APG)。理由:

1. **Embla snap ≠ slide**:`basis-1/3` 多張並排時 1 dot 對應 1 個 snap(多張 slide),tab↔tabpanel 1:1 對應不成立
2. **鍵盤契約衝突**:tabs pattern 要求方向鍵在 tablist 內移 focus(roving tabindex);本元件方向鍵 = 切張(根容器),兩者互斥
3. **compound API**:`CarouselItem` 不知自己 index,`aria-controls` 接線需額外 registration 機制,複雜度不對價

現張標記採 `aria-current="true"`(非 APG 原文的 `aria-disabled`)— M23 DS 既有 canonical 優先:FileViewer filmstrip(`file-viewer.spec.md`「Filmstrip 規則」,2026-06-01 source-verified)同類控件已 codify `group` + `<button>` + `aria-current`;Bootstrap 5 carousel indicators 同 idiom。兩者皆 APG 合法變體,DS 內一致性勝

❌ **禁半套 tabs**(dot `role="tab"` + `aria-selected` 但無 `aria-controls`、slide 非 `tabpanel`)— 對 SR 宣告了不存在的 tabs 結構
