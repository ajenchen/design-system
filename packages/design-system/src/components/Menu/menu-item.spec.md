...
| **selected × 鍵盤焦點（疊加）** | **畫框**（`focus-ring-inset`,2px 內描邊） | foreground | 同上 owner。**2026-09-07 user 拍板「A5畫框」**:底色已被選中佔走,再深一階等於把兩個意義塞進同一個通道;原 `--neutral-selected-focus` 已退役。**舊文勘誤(2026-08-11)**:先前寫 DropdownMenu「僅鍵盤 cursor 深化」用 `-active`——實作上 Radix 反白滑鼠也觸發(規範與實作不符),且 `-active` 是按壓專屬 token;現已全家統一:滑鼠釘住、鍵盤 `-focus` 深化,無消費者差異 |
