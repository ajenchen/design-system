...
- **AspectRatio 不是 responsive 佈局工具** — 只鎖比例;寬度仍由父層 / className 控制(見「為何無 SizeMatrix」)
- **「直接寫 CSS `aspect-ratio` 就好?」** — DS 內一律消費本元件(既有 primitive 優先):Radix padding-bottom 方案 SSR-safe 且跨瀏覽器一致(見「定位」)
