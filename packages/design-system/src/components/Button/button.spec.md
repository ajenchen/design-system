...
**間距機制(對齊 button.tsx 實作)**:
- Has-label → 外層 `gap-1`(sm/md/lg;xs 為 `gap-0`)+ label 兩側隱性 `px-1` **並用**:label 兩側對稱呼吸由 `px-1` 單一規則產生;icon ↔ label 實際視覺間距 = gap + padding = 8px(xs 為 4px)
- Icon-only(無 label)→ `ICON_ONLY_BASE`(`aspect-square p-0 gap-0`,padding-free 派)條件移除 padding 與 gap,從 `h-field-X` 推 width=height,flex-center 讓 icon 自然置中
...
- `<Button dismiss />` 明示(唯一機械觸發;Button 無 `onClose` / `onDismiss` prop,不依 callback 自動觸發)
- 語意判準:consumer 的點擊語意是 X close(關閉 surface / 忽略訊息)→ 應傳 `dismiss`

**不該用 dismiss**(這些 callback 語意非 dismiss):
