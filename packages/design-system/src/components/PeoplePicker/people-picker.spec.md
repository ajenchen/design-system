...
| Empty-state inline-search placeholder(未選時 → trigger empty placeholder) | `select.tsx:204` `triggerEmptyPlaceholder = placeholder \|\| '搜尋…'` / `combobox.tsx:739` `items.length === 0 ? placeholder : ''`(2026-05-15 Drift A fix 對齊 PeoplePicker SSOT)|
...
| 多人 inline-search、length=0、open | ❌(empty)| placeholder = **trigger empty placeholder「請選擇人員」** — `combobox.tsx:739`(Drift A fix)|
