...
Table 分三層:
- **Header**(固定頂部,結構性地在 scroll 容器外、body 上方——不用 CSS sticky,永遠固定在頂部):含 left / center / right 三區,center 區與 body center 的水平捲動 JS 同步 scrollLeft(center-header 跟隨 center-body 捲動位置,同步機制見「捲軸」段 + `data-table.tsx`)。Header bg 用 `--muted`(code `HEADER_PANEL = 'bg-muted border-b border-divider'`(**底色 + 下分隔線 + 只疊一層是同一個不可分的不變條件**,畫在三個 header panel 上、row 不畫;舊常數名 `HEADER_BG` 已不存在),比 surface 深一階,同 anatomy ColorMatrix)
...
| P | 凍結邊界線的垂直範圍只到水平捲軸帶上緣(`::after` 絕對定位的包含塊是 padding box,透明 border 在它外面),表格底緣少 15px;v33 特地在 spacer 上補 border 讓線**貫穿**到底 | 捲軸佔版面的平台 + 有釘選欄 | 中低(1px 視覺缺口) | **未修**。**同時撤回**先前「AG Grid 同理(pinned 分隔線也只到列區底)」這句 —— v33.3.2 原始碼正好相反,該句無證據支持(M22)|
