...
**命名**:`meta.width` / `meta.minWidth` / `meta.maxWidth`(px)。**不用 TanStack `size`** — DS 內 `size` 既定為 `'sm'|'md'|'lg'` density(49+ 處),避 namespace 衝突。內部 pre-process copy 到 TanStack root,resize feature 正常。No-resize default:`width` = reserve(cell ≥ width,flex 可 grow,不可 shrink)。`enableColumnResize=true`:`width` = 初始,`minWidth` = 拖拉下限(default 80)。**不變條件(invariants,L2 test + hook 守)**:(1) cell width = column width(跟 padding/state/mode 無關)(2) view↔edit cell width 0 delta (3) view↔edit cell height 0 delta(textarea `field-sizing:content`)(4) Field 填滿 cell 高度(1px 容差於 cell.border-r)(5) No-resize column ≥ meta.width。(6) 出現垂直捲軸時,header 與 body 的**內容盒等寬** —— body 的 `overflow-y:auto` 捲軸吃掉 15px,header 沒有捲軸就會寬 15px;欄寬是彈性分配的,同一份 `minWidth` 在兩個不同寬度的容器裡分配結果不同,**每一欄都會偏,而且逐欄累積**(2026-09-03 實測第 2/3/4 欄各差 3.8 / 7.5 / 11.3px)。作法:量 `centerBody.offsetWidth - clientWidth` 動態補等寬 `padding-right` 到 center header panel(對齊 AG Grid 的 header scrollbar spacer)。**不用** `scrollbar-gutter: stable` —— 那會在沒有捲軸時也永久預留空位,content-fit 看起來像恆有捲軸。量測由「每次 render 後」與 ResizeObserver 兩個來源驅動:列數變(分頁/篩選/展開)走前者,容器尺寸變(視窗/面板拖曳)走後者。**機械閘** = `scripts/data-table-invariants.mjs` I11(自然狀態)+ I11b(強制走到補償分支)。I11b 用一條 15px 透明右邊框造出與真捲軸同值的量測(`clientWidth` 不含 border、`offsetWidth` 含),因為 CI 的 headless Chromium 是 overlay 捲軸(gutter = 0),只靠自然狀態驗不到「補多少」—— 把 padding 整段拿掉,CI 照樣綠(2026-09-03 實測;註入 `::-webkit-scrollbar` 寬度也無法讓它佔版面)。對應 `scripts/data-table-invariants.mjs`(script 內 I1-I3 label 字串仍用 `display↔edit` — 2026-07-16 FieldMode display→view 更名前的歷史命名,語意同 view↔edit)。改 `columnSizeStyle` / 切 layout 必跑 invariant test 才 commit。
