...
**捲動事件裡不做同步重畫(2026-09-10 codify;上面幾條之後量到的下一層根因)**:前面幾輪把「主執行緒做了多少事」壓到最低,
剩下的成本集中在**它做事的時機**。`@tanstack/react-virtual` 的 `useFlushSync` 預設 true,每一個 scroll 事件都在事件處理器裡
`flushSync(rerender)`,整棵表格的 render + commit 因此算進那一次捲動事件,而且每個事件各算一次、無法合併。
守法 = `useVirtualizer({ useFlushSync: false })`,交還給 React 以 user-blocking 優先度排程,多個捲動事件合併成一次 render。
**世界級對照(第一手原始碼)**:AG Grid 33.3.2 [`onVScroll`(ag-grid-community.js:25744-25777)](https://www.npmjs.com/package/ag-grid-community/v/33.3.2)
在捲動事件裡只記 `nextScrollTop`、同步假捲軸,然後 `animationFrameSvc.schedule()` 就結束,真正重畫列在 rAF 的
`executeFrame(60)`(同檔 :34057)裡;只有動畫幀服務被停用時才走同步那條(`this.scrollGridIfNeeded(true)`,:25774)。
也就是「捲動事件裡不重畫」是它的**預設**,不是降級路徑。

**關掉同步重畫必須同時補「方向預掛」,兩者是一組**:render 不再卡在捲動事件裡,代價是它結構性地**晚一拍**落地,
那一拍合成器已經把視窗往前推了「速度 × 一次 render 的間隔」,那段距離的列若沒先掛就是可見的白。
守法 = 捲動中一律往捲動方向預掛(長度 = 量到的速度 × 量到的間隔 ÷ 列高,上限 48 列;靜止時自然歸零,
正常速度只多掛 2–3 列)。原本這個前掛只在「量到機器跟不上」時才開,對快機器永遠是 0。
**隔離實證(6,000px/s,其餘條件全同、只差這一項,6 跑中位數 / 最大)**:最長單次空白 58 / 99ms → **18 / 34ms**、
空白幀 8 / 10 → 4 / 9、空白面積 13 / 23 → 6 / 17。沒有這一條就只有「空白變少但單次變長」的半套
(2026-09-10 稍早以 `useFlushSync:false` 單獨上陣被本機同窗 A/B 擋下,原因正是這個)。

同一輪一併修掉的三個「在捲動中量了沒人看的東西」:
- **共用量測排程器**(`src/lib/measure-scheduler.ts`):截斷偵測(`use-truncated`)、頭像堆疊寬度(`person-display`)這類量測,
  捲動中延後、停下後在 rAF 裡分批做,**每幀最多 8ms**(超過就留到下一幀)。切片是必要的:只延後不切片,等於把整段捲動累積的量測
  擠進放手那一刻的單一任務,4× 節流實測超過 50ms 的任務反而 24 → 32 個。8ms 而不是 AG Grid 的 60ms,是因為它的迴圈每做完一個小任務就
  重新檢查捲動位置、隨時能轉向(`executeFrame` 的 while 第一行),我們跑的是別人的量測函式、一旦開始不能中斷。
- **捲動幾何快取**:`scrollTop` / `clientHeight` 只在權威更新點讀(捲動事件、ResizeObserver),render 與 layout effect 一律讀快取。
  對照 AG Grid 的 `nextScrollTop`:畫列時讀記錄,不回頭問 DOM。
- **列拖曳把手在閂上期間延後量測**:捲動時把手一定是隱藏的(`rowDragScrollLatch`),但 hover 代理會不停把 `data-hovered` 換到指標
  底下的新列,每換一次就量一次位置。改成延後到閂鎖放開再補量 —— **延後不是跳過**,跳過會留下「指標停在同一列不動、放開後沒有任何
  事件重新量 → 把手回不來」的洞。
- **標籤溢出只量一次**(`Combobox`):`useOverflowCount` 原本每次掛載固定跑兩趟(同步一趟 + 雙 rAF 一趟),第二趟幾乎總是同結果。
  雙 rAF 當初是為了「批次渲染時標籤還是 0 寬」,那個條件同步那趟就量得出來 → 改成量到 0 寬才補跑;兩個 ResizeObserver 也各自
  吞掉 `observe()` 必送的初始觀測(它回報的就是同步那趟剛量過的同一個版面)。

**實測(本機,全功能整合範例 `RoadmapAllInOne`,同一支儀器交叉跑)**:

| 指標 | main | 本輪 |
|---|---|---|
| 一次手勢的幾何讀取次數(攔截 `scrollTop`/`clientHeight`/`getBoundingClientRect` 等) | 11,095 | 1,875 |
| 捲動事件總耗時(1×) | 583–598ms | 72–74ms |
| 捲動事件 p95(1×) | 15.1–15.6ms | 0.86–0.89ms |
| 主執行緒任務 > 16.7ms(1×) | 54–59 | 2–3 |
| 捲動事件總耗時(4× 節流) | 834–854ms | 62–66ms |
| 任務 > 50ms(4× 節流) | 24 | 10–12 |
| 6,000px/s 空白幀(6 跑中位數 / 最大) | 28.5 / 32 | 7.5 / 15 |
| 6,000px/s 最長連續空白 | 59 / 100ms | 34 / 50ms |
| 6,000px/s 空白面積×ms | 59 / 63 | 17 / 28 |
| 6,000px/s 殼幀 / 停捲補齊 / 幀距最大 | 0、0ms、252ms | 0、0ms、249ms |
