...
- **判準(自適應機器畫列能力,2026-09-10)**:只看**捲動造成的 commit**(初次載入、換頁、靜止時資料變動照舊全畫)。
  跟得上的機器(捲動 commit 平滑後 ≤ 20ms)完全走原路:一般速度直接畫真列、零骨架。量到機器跟不上(> 20ms)才啟用兩件事:
  ...
  否則畫好的列落地時視窗早已捲過去、整片白。「跟不上」的判定有遲滯(進入 > 20ms、退出 < 10ms),退出後殼還沒補完時用 4 幀預算
...
- **兩個閘同時守**:`scripts/data-table-fast-scroll.mjs`(極速 6,000px/s 白區 ≤ 400ms、補齊 ≤ 1,000ms,**不節流、CPU 節流 2× 與 4× 各一輪**,
  節流模擬慢機器;2× 是「跟不上」旗標最容易震盪的中速帶)與下方一般速度的呈現閘(零骨架)。列快取閘 `data-table-scroll-cost.mjs` R0
...
- **捲動定位**:把手在顯示與原有 opacity 淡出期間都必須跟隨所屬列,hover 結束不代表已停止繪製。**裁切與所屬列相同**(2026-09-09):把手是 fixed 浮層、不受 body 面板 overflow 裁切,所以列滑到表頭底下或視窗底下時,列被面板裁掉多少、把手就用 `clip-path: inset()` 裁掉多少;把手不得出現在 body 可視範圍之外。
