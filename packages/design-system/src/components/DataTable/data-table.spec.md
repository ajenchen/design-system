...
| E | 釘選欄 header 的 hover ⌄ 選單與排序箭頭改變面板固有寬 → center 欄寬重排 | 預設模式 + 釘選欄 header 內容寬 ≥ 該欄 size | 中高(滑過去就跳) | **已修**(2026-09-03,由缺陷 B 的修正連帶解掉,經對抗式驗證確認):釘選欄 header cell 現在是解析出來的固定寬 + `overflow-hidden`,`hidden group-hover:inline-flex` 的 ⌄ 選單再也無法把面板的 max-content 撐大 —— 這個機制在現行程式碼裡已不存在。**面板寬已算出來**(2026-09-04 `f3fe9f2e`):`width: leftWidth` 字面仍在,但 `leftWidth` 現在是 Σ 解析寬(`panelWidth`),不再是量測值,量測旁路已移除(同 v33 `PinnedColumnService.setupHeaderPinnedWidth`,見不變條件 (9));**殘留** = `hasRowActions` 右面板量測(六之二之零)。**閘缺口**(2026-09-05 稽核登記):沒有任何 check 對釘選 header hover / 排序後再量面板寬與 center 欄寬,I11c 只量靜態;「機制已不存在」是對抗式讀碼的結論,不是 runtime 斷言 |
