...
**每列成本與骨架門檻(2026-09-11;user 在自己的機器上回報「非常卡頓」後,在真實瀏覽器量出來的)**
...
**骨架只在「這台機器真的畫不動」時出**:原判準只看「視窗移動距離 ÷ 預掛緩衝 > 2」,但那把**使用者甩得快**與**機器畫不動**
混為一談 —— 真實滾輪一次十格位移約 1,000px,是 200px 緩衝的 5 倍,於是每一次正常甩動都被判成跟不上,
實測捲動時 100% 的幀都有骨架、最多同時 159 個骨架元素,畫面整片灰。
守法 = 加上必要前提 `commitCost > 33ms`(兩幀):**畫得動的機器不論甩多快都不出骨架**(行為回到 main),
緊急跳轉那條同樣要求這個前提。這是收緊,不是取代,只會讓骨架更少。

**每列成本的三處根因(CPU 剖析 5× 節流,依自身時間排序)**:
- **殼列記帳原本無條件跑**(自身時間 307.6ms,全場最貴):每列每區一次 `Map.get/set` + `Set.add`,每次 render 重配三個容器,
  commit 時再做集合差集 —— 就算一個骨架都沒出也照跑。加上休眠旗標(這一輪不會出殼且上一輪沒有殼)後,一般捲動完全不進這條路。
- **cost-model 的 layout effect 讀 `scrollTop`**:layout effect 跑在 React 改完 DOM 之後,讀捲動位置會逼出同步版面,每次 commit 一次。
  那個值的用途只有「下次 render 判 `offsetChanged`」,改用 render 開始時已經讀到的 `S.renderOffset`,語意相同、零 DOM 讀取。
- **釘選面板同步一次事件讀三次捲動位置**(自身時間 266ms):重用同一個事件裡已讀到的值,降到兩次。
- 另外兩處在元件層(見各自 spec):`Tag` 截斷量測加字型 / 文字寬度兩層快取;`PeoplePicker` 頭像串的元素查詢改快取。

**實測(user 的真實 Chrome,`data-shell-state` 直接讀)**:每列成本 **13.7 → 6.4–7.8ms**。
一次 commit 仍需 56–72ms(視窗 13 列),所以在那台機器上**骨架仍會出現** —— 這是 13 列 × 6.4ms 的物理結果,不是 bug;
要讓它消失,下一步得再往下砍每列成本(見下方待辦),或接受 main 的做法(不出骨架、改成畫面凍住半秒)。

**下一層(已量到、本輪未動)**:`Tag` 的截斷量測仍需讀 `clientWidth`(每個 Tag 一次,逼出版面);
`use-truncated` 的 `scrollWidth/clientWidth` 同理;React 的 `beginWork` 與 `removeChild` 是 500 列虛擬化的固有成本。
