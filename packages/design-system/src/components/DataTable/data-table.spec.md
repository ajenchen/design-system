...
- **共用量測排程器**(`src/lib/measure-scheduler.ts`):截斷偵測(`use-truncated`,768 次)、頭像堆疊寬度(`person-display`)這類量測,
  捲動中延後、停下後在 rAF 裡分批做,**每幀最多 8ms**(超過就留到下一幀)。切片是必要的:只延後不切片,等於把整段捲動累積的量測
  擠進放手那一刻的單一任務,4× 節流實測超過 50ms 的任務反而 24 → 32。8ms 而不是 AG Grid 的 60ms,是因為它的迴圈每做完一個小任務就
  重新檢查捲動位置、隨時能轉向(`ag-grid-community.js:34057` `executeFrame` 的 while 第一行),我們跑的是別人的量測函式、一旦開始不能中斷。
...
| 指標 | main | 本輪 |
|---|---|---|
| 一次手勢的幾何讀取次數 | 11,095 | 1,925 |
| 6,000px/s 空白幀(4 跑中位數 / 最大) | 26.5 / 32 | 0.5 / 3 |
| 6,000px/s 最長連續空白 | 60 / 200ms | 9 / 34ms |
| 6,000px/s 空白面積×ms | 61 / 123 | 1 / 7 |
| 6,000px/s 殼幀 / 停捲補齊 | 0 / 4 幀、0 / 65ms | 0 / 0、0 / 0 |
| dpr2 + 2× 節流 bursts 缺列幀 | 11 / 10 | 6 / 5 |
| dpr2 + 2× 節流 bursts 內容延遲 p95 | 32 / 19ms | 15 / 18ms |
