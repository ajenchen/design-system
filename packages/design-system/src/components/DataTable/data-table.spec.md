...
| B | 釘選區:欄寬沒進「算一次」→ 整區退回 `flex: 1 1 base`,由瀏覽器在 header / body 兩個容器各跑一次(= 使用者最初回報「欄位對不齊、愈右邊差愈多」的同一個機制,只是換個區出現);另加「量 header → state → 灌給 body」的首幀 0 + `offsetWidth` 取整 | 一律;非整數縮放 / DPR 1.25 放大誤差 | 中高 | **已修**(2026-09-03):`resolvedWidths` 現在涵蓋 left / center / right 三區(v33 的 `HeaderCellCtrl` / `CellPositionFeature` 同樣不分區,釘選欄只是不參與 flex 分配);首幀由 `useLayoutEffect` 消掉;機械閘 I11c 對三區各自斷言。**未修**:面板寬度仍是量出來的而不是算出來的(見缺陷 E) |
...
| E | 釘選欄 header 的 hover ⌄ 選單與排序箭頭改變面板固有寬 → center 欄寬重排 | 預設模式 + 釘選欄 header 內容寬 ≥ 該欄 size | 中高(滑過去就跳) | **已修**(2026-09-03,由缺陷 B 的修正連帶解掉,經對抗式驗證確認):釘選欄 header cell 現在是解析出來的固定寬 + `overflow-hidden`,`hidden group-hover:inline-flex` 的 ⌄ 選單再也無法把面板的 max-content 撐大 —— 這個機制在現行程式碼裡已不存在。**殘留**:量測旁路(`width: leftWidth`)仍在,已退化成冗餘;欄位集合變動(隱藏 / 釘選 / 列數變)那一幀,body 面板仍可能被一個過期的 `leftWidth` 覆蓋自己已經正確的固有寬 —— 面板寬改成「算出來」(Σ 解析寬,同 v33 `PinnedColumnService.setupHeaderPinnedWidth`)即可徹底移除該旁路 |
...
| G | 右對齊欄被排序時,排序箭頭把標題推離右緣 20px(body 值仍貼右) | 該欄被單欄排序 / hover | 中 | **已失效**(2026-09-04):前提消失,不是修好 —— 2026-09-04 user 拍板「header 的規格就是要一致,只有內容會置右」後,表頭**一律靠左、不跟欄位 `align` 走**(`data-table.tsx:2494` / `:2527`),系統裡不再存在「右對齊的表頭」,排序箭頭也就不可能把標題推離右緣。原敘述寫於表頭還會跟著 align 走的年代 |
| H | `data-table.css` 的 `::-webkit-scrollbar` 客製(10px、track、thumb、corner)實測未生效,瀏覽器畫的是原生 15px | 一律 | 中(死碼 + 規格與 css 數字矛盾) | **已修**(2026-09-04):**原本記載的根因是錯的**。逐條隔離實測:`::-webkit-scrollbar:horizontal{height:10px}` → 實際 15px(無效);不帶偽類的 `::-webkit-scrollbar{width/height:10px}` → 實際 10px(有效)。**Chrome 的捲軸尺寸只讀不帶偽類的那條規則**,`:horizontal`/`:vertical` 只能改外觀。2026-04-30 拆成兩條偽類是為了躲「`display:block` 讓沒溢出的軸也冒捲軸」——但不帶偽類且不寫 `display` 時實測**不會**誤顯(只有水平溢出時 v=0、完全不溢出 0/0),當初要躲的是 `display:block`。原記載的「`@supports` 重新宣告標準屬性導致忽略偽元素」把該組合隔離重現出來是 10px,**不成立**。**修法選了「只留 W3C 標準屬性、webkit 那一整套刪除」**:讓 webkit 生效雖然控制最完整(含 corner 與 thumb hover),但 `::-webkit-scrollbar` 會把 overlay 捲軸的機器**強制變成佔版面的捲軸**,為兩項 webkit-only 加值付這個代價不划算;`scrollbar-width: thin` + `scrollbar-color` 是標準屬性、Firefox 也吃、不改變任何人現有的捲軸行為模式(實測 Chrome/macOS classic 由 15px → 11px)。實際寬度不寫死在任何地方,`measureScrollbarGutters` 每次量。**與 ScrollArea 的 SSOT**:機制不可能共用(ScrollArea 是 Radix 自繪 DOM 拇指、不吃版面寬),能共用的顏色早就是 SSOT —— 兩邊都吃 `--scrollbar-thumb`/`--scrollbar-track`(semantic.css:383-385,2026-05-09 抽出)|

| I | 補償用實體方向屬性,RTL 下捲軸在另一邊 | `enableRtl` 類情境 | 中 | **已修,但機制已換**:兩處 padding 補償都移除後,橫軸補償是內層 wrapper 的 `minWidth`(方向無關)、縱軸是 `border-bottom`(block 方向,RTL 不鏡射)→ 原始擔憂在今天的實作下不成立,與 logical property 無關。**面板邊界線 `.dtPanelBoundaryRight/Left::after` 仍用實體 `right`/`left`,未修** |
...
| M | 欄間分隔線的歸屬 header 與 body 不對稱(右釘選 + rowActions 時同一位置 header 無線 / body 有線;拖曳中最後一格突然畫線)| 該組合 | 中低(1px 視覺)| **已修**(2026-09-04):根因是**表頭與列用了兩個不同語意的判準**。列那側是 CSS `.dtCellGrid:last-child::after { content: none }`,判「DOM 上的最後一個子元素」;右釘選區後面還接一格 rowActions,所以最後一**欄**的 cell 不是 `:last-child`,照畫線。表頭那側只看欄陣列索引(`i < headers.length - 1`),不知道後面還有 rowActions → 同一位置變成列有線、表頭沒線。表頭判準補上 rowActions 這一項後兩邊同義。同時移除兩處死碼:表頭的 `&& !(isRight && i === headers.length - 1)`(前半成立時後半恆真)與 `cellEl` 那個從宣告日起就沒被用過的 `_isLastInRow` 參數(呼叫端還為它算了一段條件式,讀起來像列這側也有判斷)。**AG Grid v33 對這題是兩邊都無條件畫**(`.ag-cell{border-right}` / `.ag-header-cell::before`),靠外框蓋最外緣;我們的區邊界另有 `dtLeftBoundary/dtRightBoundary` 專責,所以取「本區最後一格不畫」,但判準只有一份語意。**「拖曳中最後一格突然畫線」查無此事**:`ResizeHandle` 的線是 `{showLine && <span/>}`,`showLine=false` 時拖曳中也不會渲染,原敘述應是舊實作的殘留 |
| N | `centerColsWidth` 被算兩次(`:1475` 與 `renderBodyRows` 內),註解宣稱是同一個 SSOT | 一律 | 低(今天數值相同)| **已修**(2026-09-04):`renderBodyRows` 不再自己重算一次中段總寬,改由呼叫端把 `centerColsWidth` 傳進去(`regionWidth ?? …` 只留作 fallback)。**AG Grid v33 的對應解是 `CenterWidthFeature`**:表頭容器與列容器共用**同一個 feature 實例**算寬(`visibleCols.bodyWidth` 一個欄位 + 同一段 spacer 邏輯),結構上不可能出現第二份計算 |
