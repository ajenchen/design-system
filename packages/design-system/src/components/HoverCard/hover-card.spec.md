...
**ARIA / Pattern**:Radix `hover-card` 刻意**不**設 popup `role`、`aria-*` 或 trigger 關聯，定位為 sighted-mouse-user 增強；它不提供可由鍵盤操作的 popup 語意，並把 content 內可 Tab 的節點移出循序 Tab order，但不代表已把渲染內容從 accessibility tree 隱藏。Radix 只提供:hover / focus 開、Esc 關 + `data-state`。Trigger 的可聚焦性需 consumer 自補(如 Avatar `hoverCard` 在 `avatar.tsx` 手動加 `tabIndex=0` + `aria-label`;**不掛 role/aria-haspopup**——2026-07-06 拆除,無 activation 行為的 role 宣告 = 對 AT 空承諾,對齊 Radix 官方零 role/aria 定位)。詳 [Radix Accessibility docs](https://www.radix-ui.com/primitives/docs/components/hover-card#accessibility)。
