...
- **Loading(panel content loading)**:Sheet 為純容器,不獨立 own loading state。Async body 場景由 consumer 在 body slot 渲 `<Skeleton>` line-stack,或有文字解釋時用 `<Empty icon={<CircularProgress size={48}/>} description="正在載入…"/>` compose(spinner-only 不得用 Empty,對齊 `empty.spec.md`「禁止事項」與 `circular-progress.spec.md`「何時用」2026-09-08 口徑)取代 content,不開 / 不關 Sheet 即可。
