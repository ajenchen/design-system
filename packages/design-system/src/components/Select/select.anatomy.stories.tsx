          ...
          {([
            ...
            <div key={display} className="flex items-center gap-4">
              <div className="w-52 shrink-0">
                <Select display={display} options={statusOptions} value="active" size="md" />
              </div>
          ...
          <table className="text-caption border-collapse">
            ...
            <tbody>
              {[
                ...
                ['loading', 'boolean', 'false', '這個值在讀取 / 驗證 / 儲存(Field 家族 SSOT,與 Input loading 同義):觸發點右側、箭頭左邊放列圖示尺寸的轉圈 + aria-busy;選單照常可開,與選項有沒有載入無關'],
                ['optionsLoading', 'boolean', 'false', '選項清單載入中(2026-09-09 改名自 loading):指示只在選單內 —— 沒有任何可顯示選項時渲一列「載入選項中」訊息列(與一筆結果等高),觸發點不轉圈;本機過濾舊選項保留,遠端搜尋抓資料中舊選項不顯示'],
                ['filterOption', 'boolean', 'true', '遠端搜尋傳 false:不在本機用搜尋字過濾、伺服器回什麼列什麼;關鍵字空時列建議群組、抓資料中清舊清單'],
                ['onSearchChange', '(value: string) => void', '—', '搜尋字改變時回呼(含清空);遠端搜尋據此抓資料並切 optionsLoading'],
                ['suggestions', 'SelectOption[]', '—', '遠端搜尋、關鍵字空時顯示的建議清單(部分選項);DS 自動包成有標題的群組,讓使用者知道選項不只這幾筆'],
                ['suggestionsLabel', 'string', "'建議'", '建議群組標題'],
                ['searchHintText', 'string', "'輸入關鍵字搜尋'", '遠端搜尋、關鍵字空、沒有建議也沒在載入時的提示列文案'],
                ['emptyText', 'string', "'沒有選項'", '真的沒有任何可選時的訊息列文案(本機過濾無結果 / 打開就沒選項 / 遠端回傳空;一句到底、可覆寫)'],
            ...
            <tbody>
              <tr>
                ...
                {(['default', 'hover', 'focus'] as const).map((st) => (
                  <td key={st} className="p-3 border-b border-divider align-top min-w-[180px]">
                    <Select options={statusOptions} value="active" size="sm" error />
            ...
            <tbody>
              {MODES.map((m) => (
                <tr key={m}>
                  ...
                  {SIZES.map((sz) => (
                    <Td key={sz}>
                      <Select mode={m} display="tag" size={sz} options={statusOptions} value="active" disabled={m === 'disabled'} />
                    </Td>
                  ))}
                </tr>
              ))}
