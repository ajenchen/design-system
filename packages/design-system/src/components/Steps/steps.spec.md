...
|---|---|---|
| upcoming(linear)| `bg-muted` + `fg-disabled` 數字 | `--primary-hover` 外環 |
| upcoming(non-linear)| `bg-secondary` + `foreground` 數字 | `--primary-hover` 外環 |
...
**不會**變成 current 的 filled 藍——這對齊使用者 mental model「我只是在看,這步還沒做」。non-linear current 與 upcoming 同為 `bg-secondary` 底,focus 純靠外環色區分(current 用 `--border-hover`、upcoming 用 `--primary-hover`)。sm 尺寸的 dot 在 non-linear upcoming 仍用 `fg-disabled` 灰點(8px dot 最小化呈現)。
