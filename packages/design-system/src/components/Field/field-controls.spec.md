...
**InlineEdit ≠ mode ≠ variant**:它是 **view↔edit 二態切換**——靜止 = 純值/格式化(內容 + 隱形 Pressable),點擊/Enter 才生一個 **edit-mode Field 控件**(預設 `<Input mode="edit">`,可 `renderEdit` 換 Textarea/Select)。站在 mode 軸**之上**。**Model A:InlineEdit 不自帶 geometry cva**——(a) 值-格式化路徑委派 `<Control mode="view">` 取全幾何 + 內部間距(多 tag gap / avatar↔label 天生一致);(b) 純值/標題 `<Tag as="h1">` 套 `fieldViewGeometry` helper。本體只給:orientation-aware `-mx`(vertical 貼 label / horizontal 對齊 sibling)+ hover bg + focus 藍框 + 隱形 Pressable。
...
**InlineEdit 多行 / padding / 零位移契約(Model A)**:read↔edit 零跳 = view 與 edit **同一顆控件、只差 chrome**。**多行(`multiline`)**:edit 自動用 `<Textarea mode="edit">`,view 用 `fieldViewGeometry(size, true)` = `items-start py-2`(**= Textarea edit `py-2`**)。**多 tag / avatar / prefix 換行的 wrap py 由委派控件 view mode 提供**(= 它 edit 的 py)—— 故 view×default **不可** collapse 成 `!py-0`。水平:InlineEdit `-mx-[--field-px]` 拉整塊到欄左緣、委派控件 view 的 `px-[--field-px]` 被 -mx 抵消 → 值落欄左緣,edit 同框 -mx → 零水平跳。
