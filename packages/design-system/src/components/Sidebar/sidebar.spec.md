...
- **Active item**:`SidebarMenuButton` 帶 `id` + 命中 `activeId` 時自動加 `data-active="true"`,呈現選取底色（`bg-neutral-selected`)。**選中 × 互動疊加(2026-08-11,owner = `item-anatomy.spec.md` 同名格)**:滑鼠 hover 當前項**釘住不變**(先前無規則、靠編譯順序,現為顯式 class);鍵盤焦點停在當前項**畫框**(`focus-ring-inset`,2px 內描邊;2026-09-07 user 拍板「A5畫框」,原本的深一階底色已退役 —— 底色被選中佔走就不能再用底色表達游標)。未選中項維持與滑鼠 hover 同色的底、不畫框。**目前僅視覺 + `data-active` attribute,不設 `aria-current="page"`**——SR 讀不到「current page」語意。`variant="meta"` 永不參與 selection（`data-active` 永遠 false）。
