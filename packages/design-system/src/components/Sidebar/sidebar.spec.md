...
**SidebarFooter 屬「列式 footer」(2026-09-17 codify)**:配方(`border-t` + `py-2` + **左右內距 0**)與判準的 owner 在
`../../patterns/overlay-surface/overlay-surface.spec.md`「底部區域:按鈕列 vs 列式」—— 底部內容需要**鋪滿容器兩側**時用這個配方,
左右 gutter 由列自己帶;底部放**按鈕**則用 `SurfaceFooter`。同配方的另一個住所是 `components/Menu/menu-item.tsx` 的 `MenuFooter`。
**本元件實作正確、不需更動**,此段只是把它掛進共同的判準 owner,避免兩處各自演化。
...
1. `SidebarGroupLabel` 本身仍是純裝飾 header(`div` `role="presentation"`),**只有尾端的 chevron** 是 trigger（`Collapsible.Trigger asChild` 包住 `ItemInlineActionButton`，帶 hover bg / `cursor-pointer`）——刻意不把整個 label 升格為 button，理由見下方 rationale
2. Label 尾端自動渲染 `ChevronDown` icon(accordion 慣例),open 時旋轉 180°
