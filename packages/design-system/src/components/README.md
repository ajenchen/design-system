...
| 跨元件共用的 checklist | `packages/design-system/ds-canonical/skills/component-quality-gate/` | 是 invoke-time workflow |
...
進 `components/` 前:
1. 走 `packages/design-system/ds-canonical/skills/component-quality-gate/`(完整 checklist)
...
判斷 test 見 `packages/design-system/ds-canonical/rules/story-rules.md` → 「Internal vs Components 三 test」。

## 建立前必 Read

本 README + 該元件所在 pattern spec(若屬 Family) + `packages/design-system/ds-canonical/rules/ui-development.md`「元件 Props 命名」 + `packages/design-system/ds-canonical/skills/component-quality-gate/`。
