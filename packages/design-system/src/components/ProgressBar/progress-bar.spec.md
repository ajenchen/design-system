...
**實作基礎**：基於 Radix `@radix-ui/react-progress` primitive（原生提供 `role="progressbar"` + `aria-valuenow` / `aria-valuemin` / `aria-valuemax` / `aria-valuetext`），外包本 DS 的 status / affix 語意與 token。單一高度、不提供 size 選項(值與理由見下「單一高度」節)。
...
|--------|------|-----------|---------|
| `inProgress`(預設) | 進行中 / 未完成 ratio | `--info` | 處理中、靜態比例顯示、CSV 匯入 |
...
❌ **不得硬寫色值**——所有 status 色必走 `--info` / `--success` / `--error` token,consumer 不可 override fill 色(若業務需要其他色,提到系統層討論新 status,不是每個消費者自己改)
...
- 「轉圈跟長條都是進度,隨便選」— 不可量化(fetch / 不知時長)**只能** CircularProgress;ProgressBar 必須能回答「百分之幾」(見分界 SSOT flowchart)。
- 「先轉圈、拿到進度再切長條」— 同一操作禁止混搭,維持 indeterminate 到底(見「禁止混搭」)。
- 「上傳列表自己拼 ProgressBar + 檔名」— 檔案情境一律 `FileItem`(見「與 FileItem 的分界」)。
...
- **超界 value**:`value < 0` / `> 100` 自動 clamp 到 0–100(tsx `clampedValue`);`affix="value"` 顯示時四捨五入為整數
- **Dark mode**:fill 色透過 semantic token(`--info` / `--success` / `--error`)自動反轉,詳見 `color.spec.md`
