...
| **Thumb** active | border `primary-hover`(同 hover)+ 陰影 `--elevation-200` | `--primary-hover` |
...
Slider 不是 button——它是「當前位置指示器」,底色不該動(動了會暗示是另一個狀態);「不用色變」指 thumb 底色與 range 填色,thumb border 仍會加深至 `primary-hover`(同色相 emphasis 變化,見視覺規格表)。陰影(elevation)是世界級 slider 的標準 hover 語言:Material 3 的 state layer + elevation、iOS slider 的 scale + shadow、Linear 的 drop shadow。對齊 `elevation.spec.md` 的 `--elevation-100` / `--elevation-200`。 <!-- @benchmark-unverified: see frontmatter benchmark list for canonical DS source URL -->
...
| Hover(thumb) | thumb border 加深成 `primary-hover` + 加 `--elevation-100` 陰影 | 滑鼠 hover 在 thumb 上 |
| Active(拖曳中) | border 維持 `primary-hover` + 加 `--elevation-200` 陰影 | 按住拖曳 |
