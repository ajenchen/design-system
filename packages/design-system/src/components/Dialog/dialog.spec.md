...
**第二個消費者**(2026-09-16):AgentPanel 蓋板態的左內距 —— 窄容器下面板不再蓋滿宿主,左邊留同一顆 token 的距離、底下鋪遮罩
(`agent-panel.spec.md`「與 app 的推擠與斷點」)。本 token 的 owner 仍是本段;改值時兩個消費者一起變。
...
- **層級**(2026-09-08 定):遮罩 `z-30` < 並存 modal 內容 `z-40` < 代理蓋板 `z-[45]` < 一般 modal `z-50`。沒有 URL 的確認框(不傳 persistentElements)維持一般 modal,蓋在常駐區域之上。代理蓋板態自己的遮罩(同一支 `CoexistenceMask`,純提示:接住指標但沒有行為;2026-09-16)也在 `z-30`,所以並存 modal 仍在它之上、面板之下;本遮罩算洞時把任何 `[data-coexistence-mask]` 當「遮罩不是洞」跳過,並存 modal 的外部點擊守衛把常駐殼子樹(含那張遮罩)視為不關。
