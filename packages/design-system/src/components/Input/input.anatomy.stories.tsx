    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <div className="flex items-center gap-2">
          ...
          <SegmentedControl size="sm" aria-label="startIcon" value={hasStartIcon ? 'on' : 'off'} onValueChange={(v) => setHasStartIcon(v === 'on')}>
            <SegmentedControlItem value="off">off</SegmentedControlItem>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* 同上:停用項不得是當前值 → value 取實際生效值(預覽也是 hasEndAction && isEdit) */}
          <SegmentedControl size="sm" aria-label="endAction" value={hasEndAction && isEdit ? 'on' : 'off'} onValueChange={(v) => setHasEndAction(v === 'on')}>
            <SegmentedControlItem value="off">off</SegmentedControlItem>
            <SegmentedControlItem value="on" disabled={!isEdit}>on</SegmentedControlItem>
          </SegmentedControl>
          {!isEdit && <span className="text-[11px] text-fg-muted">僅 edit 模式</span>}
