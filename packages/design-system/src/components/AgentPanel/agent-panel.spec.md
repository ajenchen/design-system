...
- **遮擋與貼邊**(`AgentFabDock`;演進:2026-09-02 拖到邊 → hover 小鈕 → 拖曳自由座標 → 2026-09-03 user 拍板
  ...
  - **命中區 = 可視形狀的外接矩形**(2026-09-04 user 拍板:「其實只要觸控範圍跟視覺範圍是對齊的話,此問題就解決了」)。
    講精確:**不小於視覺**(每個看得到的點都點得到)、**也不外推**(不超出可視形狀的外接矩形)。
    圓角外的角落落在外接矩形內,**刻意包含** —— 那正是最早的死區(貼邊 D 形兩角合計約 84px²、
    在家圓形四角約 343px²);把它們排除就等於把圓角的命中切回去,問題會原樣回來。
    這與「外推」的差別在**方向**:外推是把盒子撐得比視覺大(隱形帶會搶走底下內容的點擊、
    並把 Radix 錨點推遠),包含角落只是不去切掉視覺外框以內的部分。
    ...
    於是命中盒 = 可視形狀的**外接矩形**:看得到的每一點都點得到,圓角外的角落也在內(見上)。
