...
**不在本規則(不是 hover 觸發,保留過渡)**:選中切換類動畫 —— 分頁底線移動(`after:transition-colors`)、手風琴展開(`animate-accordion-*`)、展開箭頭旋轉(手風琴 / 樹 / 表格,`transition-transform`)、輪播指示點寬度(`transition-[width]`)、側欄收合寬度、進度條數值、浮層進出場(本檔「進出場動畫 token」段)。
