...
**判準只有一條,從設計面問:四周有沒有 4px 可以讓框長出去?**

> **量元素四邊到「最近的障礙」的距離,取最小值。**
> **≥ 4px → 外描邊。< 4px → 內描邊(`outline-offset: -2px`)。**
...
### 可捲動容器不自動判內描邊(2026-09-07 二次訂正)

user 追問:「可捲動的容器,為何不是也是根據該容器的範圍去看周遭到底有無空間來決定?」——**對,我先前那條是偷懶。**

先前寫「可捲動容器該軸淨空一律視為 0」。錯在兩點:
1. **元素被捲到邊緣時,它本體也一起被切** —— 框跟著被切不算視覺不一致,那是「這一項正在捲出視野」的正常結果。
2. **鍵盤 Tab 進捲動容器時,瀏覽器本來就會把元素捲進視野** —— 只要給 `scroll-margin`,它就會多捲一點,**框自然有位置**。DS 現況 `scroll-margin` **全 repo 用量 0**,等於這個現成解法從沒被用過。

**正確做法:可捲動容器 → 給焦點元素 `scroll-margin: 4px`,然後照一般方式量四周,不特別判內描邊。**

真正必須內描邊的,是**捲不出空間**的情形:元素在該軸上填滿容器、或該軸根本不捲。

### 訂正後四個判定,有兩個改變

| 元件 | 先前 | 訂正後 | 理由 |
|---|---|---|---|
| **SidebarMenuAction** | 內 | **外 + `scroll-margin-block`** | 縱向可捲,捲得出空間 |
| **FileViewer 縮圖** | 內 | **外 + `scroll-margin-inline`** | 橫向可捲(`file-viewer.tsx:605`),捲得出空間 |
| Tabs trigger | 內 | **內(不變)** | `overflow-y-hidden` 且 trigger 高 = 殼高,y 軸**永遠**捲不出空間(`tabs.tsx:311` 註解自述加 `overflow-y-hidden` 是為了阻 y 軸 auto-promote)|
| Avatar 內的 × | 內 | **內(不變)** | 元素填滿 `w-4 h-4` 槽、無捲動,永遠沒有空間 |

**所以 FileViewer 的正解也跟著改**:選中維持 `ring-2 ring-primary`;鍵盤焦點用**外描邊** `outline-2 outline-ring outline-offset-2` + 縮圖列加 `scroll-margin-inline`。
(K1 那版寫成內描邊,依本次訂正作廢。)
...
**訂正後不再有「兩條判準」** —— 裁切邊與鄰居都是障礙,用同一把尺量,取四周最小值。
可捲動容器在捲動軸上淨空視為 0,所以仍然靜態可判,不需要在執行期算捲動位置。
