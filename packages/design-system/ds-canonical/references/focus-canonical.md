...
**(2026-09-07 訂正,user 指出原判準是從程式面定義的)**
先前寫成「祖先的 `overflow` 不是 `visible` 就一律內描邊」。**那是看寫法,不是看設計。**
一個永遠不會捲、四周又很空的容器,框根本不會被切到,視覺上當然該往外畫 —— 硬判內描邊會造成
「明明有空間卻縮在裡面」的不一致。正確的問法是**「框會不會被切掉」**,而那是距離問題:

- **可捲動的容器**(內容真的會超出、有捲動機制)→ 元素可以被捲到貼邊,**該軸淨空一律視為 0** → 內描邊
- **不可捲動但有 overflow 的容器** → 量元素到裁切邊的**實際距離**,跟量鄰居一樣處理

這樣兩種障礙用同一把尺,規則從兩條併成一條,而且判準回到設計語言(有沒有空間),不是 CSS 屬性。
