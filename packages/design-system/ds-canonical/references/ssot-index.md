...
| **Protected solo workflow**(1 task = 1 branch + 1 PR / required CI / standing engineering delegation)| `infra/governance/release-workflow.json` + `feedback_solo_dev_workflow.md` + AGENTS.md「Git solo-work canonical」 | protected main 禁 direct push；PR 建立正常；required CI/conversations resolved 後由 `release:auto` 自主走 `pr-checks → merge → publish → readback → consumer`；preview/canary/attestation 非 blocking | ✅ aligned(M28 + hooks + GitHub ruleset)| 2026-07-20 migration / 2026-08-02 five-step reconciliation |
