...
`providerCertification=not-checked`, and `externalActivationRequired=true`. Those fields mean the
checkout passed its deterministic bootstrap boundary; they do not claim that a cloud/provider
runtime is certified or that external branch protection, apps, secrets, and required checks are
activated.

The locked execution contract requires Node.js 22.12.0 or newer, Bash, Git, `jq`, and Python 3.
...
or cross the v2 runtime boundary: it first needs one reviewed full-snapshot bootstrap PR plus
independent readback. Product-owned root README and other root documents are never taken over.

Changes to `.github/**`, `.npmrc`, `governance/bin/**`, `scripts/**`, or package scripts require a
separately reviewed full-snapshot path. A still-legacy consumer first completes the one-time
bootstrap; an established v2 consumer uses the recurring control-plane update protocol with an
exact sequence/predecessor readback chain. Ordinary `sync-all` cannot replace the control plane
that authorizes its own next execution. Both reviewed paths run from the DS authority, materialize
only a fresh snapshot, execute no candidate code, and emit proposals rather than applying to the
live consumer or inventory.
