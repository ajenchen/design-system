...
**世界級 idiom**:**hover bg / ring / focus outline 不超出 slot box**,保護 gap token 如實呈現。這對齊 `packages/design-system/ds-canonical/references/ui-dev-rules.md#同-flex-列的互動-slot-幾何鐵律`。
...
對應 DS:`components/ScrollArea/` + `packages/design-system/ds-canonical/references/build-ui-canonicals.md`「overflow 使用三規則」。
