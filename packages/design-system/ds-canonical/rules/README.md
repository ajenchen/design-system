...
# Canonical rules charter

## 這裡只收:path-scoped 規則(編特定 file 才 load,降 every-session context cost)

每條 rule 一個 `.md` file,內含:
- frontmatter `paths:` glob(`**/*.spec.md` / `**/*.tsx` 等)— provider materializer 可投影成原生 path-scoped discovery view
- body — 該 file family 的設計 / 撰寫 / 命名 / 紀律規則

**Authority 邊界**:本目錄是唯一可編輯 owner；`.claude/rules/**` 等 provider discovery home 是可重建 adapter view。支援 path-scoped rules 的 provider 只在編對應 file 時載入；其他 provider 由 canonical bootstrap/skill 主動讀取。

## 當前居民

| Rule | Paths | Scope |
|------|-------|-------|
| [meta-patterns.md](meta-patterns.md) | always(fundamental)| 33 active M-rules(M1-M32 + M36 + M37,M27/M33/M34/M35 retired)|
...
| [self-verify.md](self-verify.md) | `src/**` + canonical governance paths + `*.spec.md` + `*.tsx` + `*.css` | 4 階段(Pre/Mid/Post/Pre-commit)自主驗證 routine(2026-05-17 codify M10/M11/M20/M32 散在 meta-patterns 之 SSOT)|

## 跟 provider bootstrap 分工
...
1. **過 3 題**(per AGENTS.md `# 治理 canonical`)— 既有 rule cover?Rule-of-3?7 天後還 fire 嗎?
2. **檔名**:`<topic>.md`(kebab-case),內含 frontmatter
3. **paths frontmatter**:glob 該 rule 該 load 的 path
4. **加進本 README.md 表格**
5. **MEMORY.md / AGENTS.md 補 pointer**(若 cross-reference)
