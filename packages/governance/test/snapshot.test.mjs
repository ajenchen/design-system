...
test('manifest source exclusions are the single digest boundary for generated subtrees', async (t) => {
  ...
  // 2026-09-21:`scripts/.gate-bundles/` 一併列為排除。它放的是兩支閘在跑之前 esbuild 打出來的
  // 暫存檔(已 gitignore)。先前沒有排除,結果這份**產物的內容取決於本機有沒有跑過那兩支閘** ——
  // 本機 generate 出來的快照與 CI 重新產生的不一致,Authority governance anchor 直接紅。
  // 產物只能由進版控的來源決定,不能由本機殘留決定。
  assert.deepEqual(second.excludes, ['scripts/.gate-bundles/', 'scripts/generated/'])

  // 對照:排除路徑底下的位元組不得影響摘要(與上面同一條,換成新增的那個排除)
  await write(repoRoot, 'scripts/.gate-bundles/local-bundle.mjs', 'local-a\n')
  const bundleFirst = (await sourceRecords(inspected.contract)).find(record => record.id === 'repository-automation-corpus')
  assert.equal(bundleFirst.hash, second.hash, 'gate-bundles 是本機暫存,不得進入 canonical 摘要')
  await write(repoRoot, 'scripts/.gate-bundles/local-bundle.mjs', 'local-b\n')
  const bundleSecond = (await sourceRecords(inspected.contract)).find(record => record.id === 'repository-automation-corpus')
  assert.equal(bundleSecond.hash, second.hash, '改動 gate-bundles 內容一樣不得改變摘要')
