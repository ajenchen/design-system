...
function validateClosedPrivateRuntimeBase(runtimePlatform) {
  const configured = CLOSED_PRIVATE_RUNTIME_BASES[runtimePlatform]
...
export function assertClosedGitLocalConfiguration(repoRoot, options = {}) {
  ...
  if (hookPathNames.length > 0 && !allowHookPathMigration) {
    ...
    invariant(
      hookPaths[0] === hook.hookRoot,
      'Repository-local Git hook path must be the exact absolute canonical .husky directory',
    )
  }
...
export function materializeClosedHookToolProfile({
  ...
  const temporaryBase = validateClosedPrivateRuntimeBase(runtimePlatform)
  const root = realpathSync(mkdtempSync(join(temporaryBase, 'qijenchen-closed-hook-tools-')))
  chmodSync(root, 0o700)
  if (canonicalRepo) {
    invariant(
