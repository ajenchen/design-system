...
export async function reconstructExpectedUpgrade({
  ...
  const expectedNpmArtifact = resolveExactNpmArtifact(repo)
  ...
  try {
    ...
    invariant(
      npmRuntime?.cli
        && npmRuntime?.artifact?.version === expectedNpmArtifact.version
        && npmRuntime?.artifact?.resolved === expectedNpmArtifact.resolved
        && npmRuntime?.artifact?.integrity === expectedNpmArtifact.integrity
        && npmRuntime?.toolchain?.npm === expectedNpmArtifact.version,
    ...
    run(process.execPath, [
      cli, 'audit', '--audit-level=high', `--registry=${NPM_REGISTRY}`,
    ], { cwd: sandbox, env: npmEnv, label: 'protected-base npm vulnerability audit' })
