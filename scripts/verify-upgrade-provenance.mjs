...
//   -> immutable GitHub Release closed eight-asset set -> receipt/trust/BOM/scaffold/package/SBOM bytes
//   -> exact npm SRI.
...
function bomReleaseAssets(bom) {
  ...
  if (entries.length !== IMMUTABLE_RELEASE_ASSET_COUNT || new Set(entries.map(item => item.name)).size !== entries.length) {
    throw new Error('release BOM does not resolve to the writer\'s closed eight-asset set')
  }
  return entries
}
...
export function validateImmutableReleaseSnapshot({
  ...
  if (tagIdentity.verification.verified !== true || tagIdentity.verification.reason !== 'valid'
    || !SHA256_RE.test(tagIdentity.verification.signatureSha256 || '')
    || !SHA256_RE.test(tagIdentity.verification.payloadSha256 || '')) {
    throw new Error('release tag identity is not a direct GitHub-verified signed annotated tag object')
  }
  canonicalTimestamp(tagIdentity.verification.verifiedAt, 'GitHub release tag verifiedAt')
  ...
  const expectedAssets = bomReleaseAssets(bom)
  ...
  if (releaseAssets.size !== IMMUTABLE_RELEASE_ASSET_COUNT
    || JSON.stringify([...releaseAssets.keys()].sort()) !== JSON.stringify(expectedNames)) {
    throw new Error(`GitHub immutable release ${tag} must contain exactly the writer's closed eight-asset set [${expectedNames.join(', ')}]`)
  }
  ...
  const trust = validateReleaseTrustEvidence(
  ...
  return {
    ...
    releaseSetSha256: exactReleaseSetSha256,
    finalizationReceiptSha256: assetDigests.get(NPM_FINALIZATION_RECEIPT_ASSET),
    releaseTrustEvidenceSha256: assetDigests.get(RELEASE_TRUST_PREFLIGHT_ASSET),
    externalActivationRequirementsSha256: trust.externalActivationRequirements.sha256,
...
export async function resolveImmutableReleaseSnapshot({
  ...
  if (releaseAssets.size !== IMMUTABLE_RELEASE_ASSET_COUNT
    || JSON.stringify([...releaseAssets.keys()].sort()) !== JSON.stringify(expectedNames)) {
    throw new Error(`GitHub Release ${tag} does not contain the writer's exact eight-asset set`)
  }
