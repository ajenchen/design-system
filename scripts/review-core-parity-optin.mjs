...
function assertSelectedFullCoreEvidenceEqual(left, right) {
  assert.deepEqual(Object.keys(left), Object.keys(right))
  for (const name of Object.keys(left)) {
    assert.equal(
      left[name].byteLength,
      right[name].byteLength,
      `${name} canonical byte length must be reproducible`,
    )
    assert.equal(
      left[name].bytesSha256,
      right[name].bytesSha256,
      `${name} canonical content digest must be reproducible`,
    )
...
const blockedFullParityOnly =
  process.argv.includes('--blocked-full-parity-only')
const selectedFullParityOnly =
  process.argv.includes('--selected-full-parity-only')
const isolatedFullParityChild =
  blockedFullParityOnly || selectedFullParityOnly
const isolatedEvidenceRootArgument = process.argv.find((argument) => (
  argument.startsWith('--isolated-evidence-root=')
))
const isolatedManifestSha256Argument = process.argv.find((argument) => (
  argument.startsWith('--isolated-manifest-sha256=')
))
const isolatedEvidenceRoot = isolatedEvidenceRootArgument
  ?.slice('--isolated-evidence-root='.length) ?? null
const isolatedManifestSha256 = isolatedManifestSha256Argument
  ?.slice('--isolated-manifest-sha256='.length) ?? null
assert(
  [
    transportBudgetGuardOnly,
    singleCellPartitionGuardOnly,
    blockedFullParityOnly,
    selectedFullParityOnly,
  ].filter(Boolean).length <= 1,
  'provider-neutral review core focused modes are mutually exclusive',
)
assert(
  isolatedFullParityChild
    || (!isolatedEvidenceRootArgument && !isolatedManifestSha256Argument),
  'isolated frozen-target arguments are valid only for one full parity child phase',
)
assert.equal(
  Boolean(isolatedEvidenceRootArgument),
  Boolean(isolatedManifestSha256Argument),
  'isolated full parity target requires both evidence root and manifest digest',
)

function runIsolatedFullParityPhase({
  phase,
  flag,
  explicitRoot,
  manifestSha256,
  ...
  const result = spawnSync(process.execPath, [
    ...
    flag,
    `--isolated-evidence-root=${explicitRoot}`,
    `--isolated-manifest-sha256=${manifestSha256}`,
  ], {
    cwd: ROOT,
    stdio: 'inherit',
  })
  if (result.error) {
    throw new Error(`${phase} isolated child failed to start:${result.error.message}`)
  }
  if (result.signal !== null) {
    throw new Error(`${phase} isolated child terminated by signal:${result.signal}`)
  }
  if (result.status !== 0) {
    throw new Error(`${phase} isolated child exited nonzero:${String(result.status)}`)
  }
  const readback = loadActiveDeepAuditRun({
    repoRoot: ROOT,
    explicitRoot,
    requireCurrent: true,
  })
  assert.equal(
    readback.manifestSha256,
    manifestSha256,
    `${phase} isolated child must leave the shared frozen target unchanged`,
  )
  console.log(`✓ ${phase} full parity completed in a fresh fail-closed child process against manifest ${manifestSha256}`)
}
...
try {
  ...
  assert.equal(
    blockedFullParityOnly && selectedFullParityOnly,
    false,
    'full parity child must execute exactly one isolated phase',
  )
  let frozen
  if (isolatedFullParityChild && isolatedEvidenceRoot) {
    assert.match(
      isolatedManifestSha256 ?? '',
      /^[a-f0-9]{64}$/,
      'isolated full parity child requires an exact manifest digest',
    )
    explicitEvidenceRoot = isolatedEvidenceRoot
    frozen = loadActiveDeepAuditRun({
      repoRoot: ROOT,
      explicitRoot: explicitEvidenceRoot,
      requireCurrent: true,
    })
    assert.equal(
      ...
      isolatedManifestSha256,
      'isolated full parity child must bind the parent-frozen manifest',
    )
  } else {
    explicitEvidenceRoot = `provider-neutral-review-core-${randomUUID()}`
    frozen = prepareDeepAuditRun({
      repoRoot: ROOT,
      explicitRoot: explicitEvidenceRoot,
      selfProvider: 'codex',
      authorProvider: 'codex',
    ...
    explicitEvidencePath = frozen.evidenceRoot
    if (!isolatedFullParityChild) {
      runIsolatedFullParityPhase({
        phase: 'blocked-full',
        flag: '--blocked-full-parity-only',
        explicitRoot: explicitEvidenceRoot,
        manifestSha256: frozen.manifestSha256,
      })
      runIsolatedFullParityPhase({
        phase: 'selected-full',
        flag: '--selected-full-parity-only',
        explicitRoot: explicitEvidenceRoot,
        manifestSha256: frozen.manifestSha256,
      })
      console.log(`✓ combined full parity parent aggregated blocked-full then selected-full as isolated serial phases against one frozen manifest ${frozen.manifestSha256}`)
    }
  }

  if (blockedFullParityOnly) {
  ...
  let firstArtifactEvidence = preparedArtifactEvidence(first)
  ...
  second = null
  firstArtifactEvidence = null
  }

  if (selectedFullParityOnly) {
  ...
  assertSelectedFullCoreEvidenceEqual(
