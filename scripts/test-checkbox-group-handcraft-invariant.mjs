...
const run = () => spawnSync(process.execPath, ['--', 'scripts/checkbox-group-handcraft-invariant.mjs'], { stdio: 'pipe' }).status ?? 1
let ok = true

// 1) 現況必 PASS
if (run() !== 0) { console.error('✗ baseline run 應 PASS 卻 FAIL(repo 已有真實違規)'); process.exit(1) }
...
// 3) 還原後必 PASS
if (run() !== 0) { console.error('✗ 還原後應 PASS(還原不完整)'); process.exit(1) }
