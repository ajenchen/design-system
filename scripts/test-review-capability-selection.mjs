...
const archiveResultCarriers = executionEvidence.map((row) => {
  const projection = {
    schemaVersion: 1,
  ...
  return { ...projection, carrierDigest: digest(projection) }
})
const archiveExecutionEvidence = executionEvidence.map((row, index) => ({
  ...row,
  carrierDigest: archiveResultCarriers[index].carrierDigest,
}))
const archiveResponseRows = archiveExecutionEvidence.map((row) => ({
  requestDigest: row.requestDigest,
  carrierDigest: row.carrierDigest,
  providerRunId: row.providerRunId,
  responseSha256: row.responseSha256,
}))
const archiveStructuralProjection = {
  ...structuralProjection,
  responseDigestSetDigest: digest(archiveResponseRows),
}
const archiveStructuralCompletion = {
  ...archiveStructuralProjection,
  completionDigest: digest(archiveStructuralProjection),
}
const archiveReceipt = buildGenuineCrossProviderReviewReceipt({
  ...genuineReceiptArgs,
  structuralCompletion: archiveStructuralCompletion,
  executionEvidence: archiveExecutionEvidence,
})
const archiveSnapshot = captureDeepAuditSnapshot(process.cwd())
const archivePrepared = {
  run: {
    manifestSha256: digest('archive-manifest'),
    manifest: {
      runId: randomUUID(),
      head: archiveSnapshot.head,
      tree: archiveSnapshot.tree,
      indexDigest: archiveSnapshot.indexDigest,
      worktreeFingerprint: archiveSnapshot.worktreeFingerprint,
      inventoryDigest: archiveSnapshot.inventoryDigest,
      rubricDigest: digest('archive-rubric'),
      providerIdentityDigest: digest('archive-provider-identity'),
    },
  },
  exchangeSchedule,
}
const archiveHandoff = buildGenuineProviderNeutralReviewArchiveHandoff({
  prepared: archivePrepared,
  resultCarriers: archiveResultCarriers,
  structuralCompletion: archiveStructuralCompletion,
  executionEvidence: archiveExecutionEvidence,
  receipt: archiveReceipt,
})
assert.equal(archiveHandoff.completeArchiveHandoff, true)
assert.equal(archiveHandoff.requestCount, archiveExecutionEvidence.length)
assert.equal(archiveHandoff.artifactCount, archiveExecutionEvidence.length + 3)
assert.equal(archiveHandoff.artifacts.filter(
  artifact => artifact.role === 'provider-result-carrier',
).length, archiveExecutionEvidence.length)
assert.equal(
  buildGenuineProviderNeutralReviewArchiveHandoff({
    prepared: archivePrepared,
    resultCarriers: archiveResultCarriers,
  ...
  archiveHandoff.evidenceSetDigest,
)
assert.throws(() => buildGenuineProviderNeutralReviewArchiveHandoff({
  prepared: archivePrepared,
  resultCarriers: [...archiveResultCarriers].reverse(),
  structuralCompletion: archiveStructuralCompletion,
  executionEvidence: archiveExecutionEvidence,
  receipt: archiveReceipt,
}), /carrier is missing, reordered, or substituted/)
assert.throws(() => buildGenuineProviderNeutralReviewArchiveHandoff({
  prepared: archivePrepared,
  resultCarriers: archiveResultCarriers.map((carrier, index) => (
    index === 0 ? { ...carrier, response: { ...carrier.response, bytes: 3 } } : carrier
  )),
  structuralCompletion: archiveStructuralCompletion,
  executionEvidence: archiveExecutionEvidence,
  receipt: archiveReceipt,
...
const archiveExplicitRoot = `review-capability-archive-${randomUUID()}`
const archiveExplicitAbsolute = ensureRuntimeEvidenceRoot({
  repoRoot: process.cwd(),
  explicitRoot: archiveExplicitRoot,
})
try {
  ...
  assert.equal(archived.reused, false)
  assert.deepEqual(archived.frozenIdentityBefore, archived.frozenIdentityAfter)
  assert.equal(archived.receipt.receiptDigest, archiveReceipt.receiptDigest)
  const readback = readGenuineCrossProviderReviewEvidenceArchive({
    repoRoot: process.cwd(),
    explicitRoot: archiveExplicitRoot,
    runId: archivePrepared.run.manifest.runId,
    archiveDigest: archived.archiveDigest,
  })
  assert.equal(readback.handoff.evidenceSetDigest, archiveHandoff.evidenceSetDigest)
  assert.equal(readback.receipt.receiptDigest, archiveReceipt.receiptDigest)
  const replay = archiveGenuineCrossProviderReviewEvidence({
    repoRoot: process.cwd(),
    explicitRoot: archiveExplicitRoot,
    handoff: archiveHandoff,
  })
  assert.equal(replay.reused, true)
  assert.equal(replay.archiveDigest, archived.archiveDigest)

  const resultArtifact = archiveHandoff.artifacts.find(
    artifact => artifact.role === 'provider-result-carrier',
  )
  const resultPath = resolvePath(archived.root, 'artifacts', resultArtifact.path)
  const resultBytes = readFileSync(resultPath)
  writeFileSync(resultPath, '{}')
  assert.throws(() => readGenuineCrossProviderReviewEvidenceArchive({
    repoRoot: process.cwd(),
    explicitRoot: archiveExplicitRoot,
    runId: archivePrepared.run.manifest.runId,
    archiveDigest: archived.archiveDigest,
  }), /missing or substituted/)
  assert.throws(() => archiveGenuineCrossProviderReviewEvidence({
    repoRoot: process.cwd(),
    explicitRoot: archiveExplicitRoot,
    handoff: archiveHandoff,
  }), /missing or substituted/)
  ...
  writeFileSync(resultPath, resultBytes)
  assert.equal(readGenuineCrossProviderReviewEvidenceArchive({
    repoRoot: process.cwd(),
    explicitRoot: archiveExplicitRoot,
    runId: archivePrepared.run.manifest.runId,
    archiveDigest: archived.archiveDigest,
  ...
  rmSync(archiveExplicitAbsolute, { recursive: true, force: true })
}
