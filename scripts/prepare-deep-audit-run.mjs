...
function usage() {
  return 'usage: prepare-deep-audit-run.mjs [--repo-root <path>] [--evidence-root <path>] --self-provider <id> --self-surface <surface> [--peer-surface <surface>] [--peer-provider <expected-id>] [--reviewer-entitlement <required-id>] [--author-provider <id>] [--self-runtime <kind>] [--peer-runtime <kind>] [--self-target <id>] [--peer-target <id>] [--replace-active] [--json]'
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!['--repo-root', '--evidence-root', '--self-provider', '--peer-provider', '--reviewer-entitlement', '--author-provider', '--self-runtime', '--peer-runtime', '--self-surface', '--peer-surface', '--self-target', '--peer-target'].includes(token)) {
...
export function main(argv = process.argv.slice(2)) {
  ...
  const result = prepareDeepAuditRun({
    ...
    peerSurface: args['peer-surface'] ?? process.env.GOVERNANCE_PEER_SURFACE ?? null,
    selfTarget: args['self-target'] ?? null,
    peerTarget: args['peer-target'] ?? null,
    requiredReviewerEntitlement: args['reviewer-entitlement'] ?? process.env.GOVERNANCE_REVIEWER_ENTITLEMENT ?? null,
