...
const COVERAGE = {
  ...
  52: { tier: 'HOOK-ENFORCED', mechanism: 'chrome_header_dispatcher.sh:tab_lg_equal + header_with_tabs_border BLOCKER(exit 2 傳播,2026-05-31 folded-hook-audit);chrome_header_handcraft WARN(Phase 3 未升)' },
  ...
  56: { tier: 'HOOK-ENFORCED', mechanism: 'chrome_header_dispatcher.sh:_app_shell_primary_header_consistency BLOCKER(exit 2 傳播,2026-05-31 修 stdin parsing + dispatcher propagate;escape @app-shell-primary-header-allow)' },
  ...
  58: { tier: 'HOOK-ENFORCED', mechanism: 'check_plugin_fork_health.sh r1_plugin_install SessionStart(原 check_fork_user_plugin_install.sh,2026-06-11 prune merge)' },
  ...
  91: { tier: 'DETERMINISTIC', mechanism: 'scripts/audit-failure-class-coverage.mjs --check(failure-class registry 每 class 必 protected/remediating+plan/judgment+auditDim;fail-closed;2026-07-10 user「每類病根必有防線」基本原則機械化)' },
}
