...
export function validateStageTopology(stages) {
  ...
  // The sole in-place transform is release-version, which owns only exact version fields in
  // explicitly listed files. Directory overlap and every other source/output self-cycle are
  // forbidden: generated bytes must never hide inside a canonical source declaration.
  for (const stage of stages) {
    const allowed = new Set(stage.sourceOutputOverlaps || [])
    if (allowed.size && stage.id !== 'release-version') throw new Error(`${stage.id} may not declare source/output overlap exceptions`)
