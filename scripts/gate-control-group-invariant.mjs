 ...
 * ── 為什麼有這支 ──
 *
 * 2026-09-20 盤點:進 required CI 的閘裡,**只有個位數有雙向對照組**,其餘不是只有「壞了會紅」
 * 就是兩邊都沒有。**「沒壞要綠」那一半在整個閘體系裡幾乎不存在。**
 * (確切數字不在註解硬寫 —— 本閘每次跑都會印出當下的「已宣告 / 尚未宣告」,baseline 檔才是 SSOT。
 *  這一條就是本輪 prune 抓到的:我原本寫「44 支」,修好列舉漏掉 `node scripts/…` 那條之後
 *  實際是 83 支,註解當場就過期了。)
 *
 ...
 * ── 為什麼是棘輪而不是一次全要求 ──
 *
 * 絕大多數不合規,一次要求等於一面紅牆:沒人評估得了落點,最後只會被整支關掉。
