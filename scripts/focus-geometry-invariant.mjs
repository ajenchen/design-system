#!/usr/bin/env node
// ═══════════════════════════════════════════════════════════════════════════
// 焦點框「只有三種幾何」不變條件(靜態掃描)
// ═══════════════════════════════════════════════════════════════════════════
//
// SSOT:`packages/design-system/ds-canonical/references/focus-canonical.md`「問題二」
//
// 2026-09-07 之前全 DS 同時存在**五種**焦點框幾何(全域 outline / ring-offset-1 /
// ring-offset-2 / ring-2 無 offset / ring-inset),那是 user 要求「焦點框粗細顏色圓角
// 至少要一致」做不到的根因 —— 不是哪個元件寫錯,是兩套機制並存且互不相同。
//
// 收斂後只剩兩種,而且**都不需要元件自己寫值**:
//   外描邊 = 全域 `styles/base.css` 的 `:focus-visible`(元件什麼都不用寫)
//   內描邊 = `focus-ring-inset` utility(同一份幾何 SSOT)
// 2026-09-23 user 拍板加第三種,只給**底色就是主色**的元素(DateGrid 選中日 / 區間端點):
//   填色元素上的內描邊 = `focus-ring-inset-emphasis` utility(1px `--on-emphasis` 白線退 3px;藍線畫在藍底上看不見,
//   白線貼最外圈又只是把藍圓削小 —— 對照 Carbon 選中日)。一樣住在 base.css,元件只掛 class,不寫值。
//
// 六條:
//   (R1) 禁 `ring-offset-*` —— 它的間隙寫死白色(`--tw-ring-offset-color` 預設 #fff,
//        `inherits:false` 導致寫進 :root 完全不生效且靜默無錯),深色主題會露一圈白。
//   (R2) 禁 `focus-visible:ring-*` 當焦點框 —— 那是第二套機制。
//        2026-09-24 補洞:R2 認的是「**focus-visible 之後的 ring utility**」,不是裸寫的
//        `focus-visible:ring-2` 而已 —— 變體前綴 `has-[…:focus-visible]:` / `group-focus-visible:` /
//        `peer-focus-visible:` / `[&>button]:focus-visible:` 都要涵蓋,寬度(`ring-2`)與只有顏色的
//        (`ring-ring` / `ring-primary`)也一樣是第二套機制。FileItem 那兩行
//        `has-[[data-row-focus-target]:focus-visible]:ring-2 …:ring-ring` 就是從舊的裸寫 regex 逃掉的。
//        判準是「冒號後面緊接 ring」:`focus-ring-inset` / `focus-ring-inset-emphasis` 的 ring 前面是
//        `-` 不是 `:`,`--ring` token 名同理,兩者結構上都碰不到本規則(掃描範圍是整行程式碼,不只 className)。
//   (R3) 禁 `outline-none` 與 `focus-ring-inset` 出現在同一個 class 字串裡 ——
//        兩者特異性同階,誰贏取決於 Tailwind 內部排序 = 靜默失效的溫床。
//   (R4) 禁重寫全域外描邊 —— 同一份值抄第二遍(H1c 那類贅碼),改全域時會漏掉。
//   (R5) 禁手寫內描邊三件組 —— 改用 `focus-ring-inset`,否則寬度/顏色要改就得改二十處(M17)。
//   (R6) 禁手寫其他退距的內描邊(`outline-offset-[-3px]` 之類)—— 填色元素改用 `focus-ring-inset-emphasis`,
//        其餘沒有第四種幾何。
//
// 掃描範圍:`packages/design-system/src` 底下全部 `.tsx`,**含 `.stories.tsx`**。
//   2026-09-24 之前 `load()` 明文跳過 stories,於是 Steps 那則示範照著手寫三件組寫了 class 卻永遠掃不到
//   ——「閘存在」不等於「閘看得到」。納入前先用同一份 RULES 對 213 支 stories 全掃,只紅 1 筆(就是那則),
//   修掉後零既有債,因此直接納入而非記成已知缺口。
//   連帶:去註解必須認 JSX 的 `{/* … */}`(stories 裡有 695 處),否則註解裡提到舊寫法會被誤判成違規。
//   殘留邊界(刻意):去註解是**逐行**的,跨行 JSX 註解的中段不認得自己在註解裡,若正好寫到舊 class 會假紅。
//   不做跨行狀態機是因為字串裡的 glob(`'src/**/*.tsx'` 含 `/*`)會誤開區塊而吃掉整個檔案 —— 假紅吵但看得見,
//   假綠靜默且會鎖死整份檔案,兩害取其輕。
//
// Run: `node scripts/focus-geometry-invariant.mjs`(`--selftest` 跑正反例)

import { readFileSync, readdirSync, statSync } from 'node:fs'
import { join } from 'node:path'
import { fileURLToPath } from 'node:url'

const ROOT = fileURLToPath(new URL('../packages/design-system/src', import.meta.url))

const RULES = [
  { id: 'R1', re: /ring-offset-\d/, why: '間隙寫死白色,深色主題露白;改用全域外描邊或 focus-ring-inset' },
  // focus-visible 之後的任何 ring utility:中間允許變體前綴收尾的 `]` `:` `/` 等(不含空白與引號,
  // 所以跨不出同一個 class 字面量);`:!?ring(?!\w)` 要求 ring 緊接在冒號後面,因此 `focus-ring-inset`
  // / `focus-ring-inset-emphasis` / `--ring`(ring 前面是 `-`)全部不命中。
  { id: 'R2', re: /focus-visible[^\s'"`,]*?:!?ring(?!\w)/, why: '第二套焦點機制;外描邊什麼都不用寫,內描邊用 focus-ring-inset' },
  { id: 'R4', re: /focus-visible:outline-2(?!.*offset-\[-)/, why: '把全域外描邊抄了一遍;刪掉即可,行為不變' },
  { id: 'R5', re: /outline-offset-\[-2px\]/, why: '手寫內描邊三件組;改用 focus-ring-inset(同一份幾何 SSOT,改寬度顏色只需一處)' },
  // hover 的內描邊(DataTable 儲存格 `hover:outline-offset-[-1px]`)是 hover 回饋不是焦點框,不在本閘範疇
  { id: 'R6', re: /(?<!hover:)outline-offset-\[-(?!2px\])\d+(?:\.\d+)?px\]/, why: '手寫填色內描邊;填色元素改用 focus-ring-inset-emphasis(1px 白線退 3px,同一份幾何 SSOT),其餘沒有第四種幾何' },
]

export function scan(files) {
  const hits = []
  for (const { path, src } of files) {
    src.split('\n').forEach((line, i) => {
      // 先吃掉同行開閉的區塊註解(含 JSX 的 `{/* … */}`),再吃行尾 `//`
      const stripped = line.replace(/\/\*.*?\*\//g, '')
      const code = stripped.replace(/\/\/.*$/, '')
      // 整行註解不算;`{\s*/*` 收的是跨行 JSX 註解的開頭(同行就閉合的已被上面吃掉)
      if (/^\s*(\/\/|\*|\/\*|\{\s*\/\*)/.test(stripped)) return
      for (const r of RULES) if (r.re.test(code)) hits.push({ path, line: i + 1, id: r.id, why: r.why, text: code.trim().slice(0, 90) })
      // R3:同一個 class 字串內同時出現兩者
      for (const m of code.matchAll(/['"`]([^'"`]*)['"`]/g)) {
        const s = m[1]
        if (/\boutline-none\b/.test(s) && /\bfocus-ring-inset\b/.test(s)) {
          hits.push({ path, line: i + 1, id: 'R3', why: 'outline-none 與 focus-ring-inset 同階打架,勝負看排序', text: s.slice(0, 90) })
        }
      }
    })
  }
  return hits
}

function load(dir, out = []) {
  for (const n of readdirSync(dir)) {
    const p = join(dir, n)
    if (statSync(p).isDirectory()) load(p, out)
    else if (n.endsWith('.tsx')) out.push({ path: p.replace(ROOT, 'src'), src: readFileSync(p, 'utf8') })
  }
  return out
}

if (process.argv.includes('--selftest')) {
  const cases = [
    { n: 'ring-offset', src: "cn('focus-visible:ring-2 focus-visible:ring-offset-1')", bad: true },
    { n: 'focus ring', src: "cn('focus-visible:ring-2 focus-visible:ring-ring')", bad: true },
    // 2026-09-24 補洞:變體前綴形式(舊 regex 只認裸寫,這三種全逃掉)
    { n: 'has-[…:focus-visible] 前綴(FileItem 真實寫法)', src: "cn('has-[[data-row-focus-target]:focus-visible]:ring-2 has-[[data-row-focus-target]:focus-visible]:ring-ring')", bad: true },
    { n: 'group- 前綴 + 只有顏色', src: "cn('group-focus-visible:ring-ring')", bad: true },
    { n: 'peer- 前綴 + 任意寬度', src: "cn('peer-focus-visible:ring-[1.5px] peer-focus-visible:ring-primary')", bad: true },
    { n: '任意選擇器前綴', src: "cn('[&>button]:focus-visible:ring-2')", bad: true },
    { n: '抄全域外描邊', src: "cn('focus-visible:outline-2 focus-visible:outline-ring')", bad: true },
    { n: '手寫內描邊三件組', src: "cn('focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-ring')", bad: true },
    { n: 'outline-none 打架', src: "cn('outline-none focus-ring-inset')", bad: true },
    { n: '手寫填色內描邊', src: "cn('focus-visible:outline-1 focus-visible:outline-offset-[-3px] focus-visible:outline-on-emphasis')", bad: true },
    { n: '正確內描邊', src: "cn('focus-visible:focus-ring-inset rounded-md')", bad: false },
    { n: '正確填色內描邊', src: "cn('[&>button]:bg-primary [&>button]:focus-visible:focus-ring-inset-emphasis')", bad: false },
    { n: '填色 modifier 下的中段壓回一般內描邊', src: "cn('[&>button]:focus-visible:!focus-ring-inset')", bad: false },
    { n: 'hover 的內描邊不是焦點框', src: "'hover:outline hover:outline-1 hover:outline-offset-[-1px]'", bad: false },
    { n: '正確外描邊(什麼都不寫)', src: "cn('rounded-md bg-surface')", bad: false },
    { n: '註解裡提到不算', src: "// 舊寫法是 focus-visible:ring-2 focus-visible:ring-offset-1", bad: false },
    { n: '虛擬游標', src: "showRing && 'focus-ring-inset'", bad: false },
    // 2026-09-24 納入 stories 後的連帶對照組:JSX 註解不紅、同行真 class 仍紅(去註解不得過頭)
    { n: 'JSX 註解裡提到不算', src: '{/* 舊寫法是 focus-visible:ring-2 focus-visible:ring-offset-1 */}', bad: false },
    { n: '跨行 JSX 註解的開頭不算', src: '{/* 說明:舊寫法是 focus-visible:ring-offset-1', bad: false },
    { n: 'JSX 註解後面接真 class 仍要紅', src: `{/* 說明 */}<div className="focus-visible:ring-2" />`, bad: true },
    // 2026-09-24 補洞的另一面:`focus-ring-inset*` 與 `--ring` 的 ring 前面是 `-` 不是 `:`,不得誤傷
    { n: '前綴 + 正確填色內描邊(防誤傷)', src: "cn('[&>button]:focus-visible:focus-ring-inset-emphasis')", bad: false },
    { n: 'group- 前綴 + 正確內描邊(time-columns 真實寫法)', src: "'group-focus-visible/listbox:aria-selected:focus-ring-inset'", bad: false },
    { n: 'tokens 陣列寫 --ring 名(掃描範圍是整行,不只 className)', src: "ring: ['--ring'], focusClass: 'focus-visible:focus-ring-inset'", bad: false },
  ]
  let ok = true
  for (const c of cases) {
    const got = scan([{ path: 't.tsx', src: c.src }]).length > 0
    if (got !== c.bad) { console.error(`✗ selftest「${c.n}」預期 ${c.bad} 實得 ${got}`); ok = false }
  }
  console.log(ok ? `✓ selftest ${cases.length}/${cases.length} 通過` : '✗ selftest 失敗')
  process.exit(ok ? 0 : 1)
}

const hits = scan(load(ROOT))
if (hits.length) {
  console.error('✗ 焦點框幾何回流(全 DS 只准三種:全域外描邊 / focus-ring-inset / 填色元素專用的 focus-ring-inset-emphasis):')
  console.error('  SSOT:packages/design-system/ds-canonical/references/focus-canonical.md「問題二」')
  for (const h of hits) console.error(`    [${h.id}] ${h.path}:${h.line}\n         ${h.text}\n         → ${h.why}`)
  process.exit(1)
}
console.log('✓ 焦點框只有三種幾何(外描邊 / 內描邊 / 填色元素內描邊),無回流')
