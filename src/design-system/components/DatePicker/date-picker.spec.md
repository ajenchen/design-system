...
跟世界級對照(Ant Design / Material X DateRangePicker / Atlassian):**input-click 切換 activeEnd**,而**非** footer toggle / radio 按鈕。
...
- showTime=true:popover 右側出現 TimeColumns(H/M [/S]),flex-row 跟 DateGrid 並排
- 底部 Separator + footer:左「此刻」(set 當前 datetime)、右「確定」(needConfirm=true)或「關閉」
- value 格式:`'YYYY-MM-DDTHH:MM:SS'`(local-time 語意,不帶 timezone)
- needConfirm=true 時 user 編輯先暫存 draft,按確定才 onChange;false 時邊編邊 commit
- showSeconds=false(default)→ TimeColumns 只顯示 H/M(對齊 Ant 預設)

### 為什麼用 prop 而非分離 `<DateTimePicker>` 元件

世界級對照:**Ant Design / Material X / Atlassian / Carbon 全採 prop 模式**(`showTime` / `withTime` / `granularity`),非分離元件。理由:
