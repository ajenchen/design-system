...
Toolbar 右側 `X` + InfoPanel header `X` **都走 `<Button iconOnly dismiss size="sm" />`**(dismiss canonical,見 `components/Button/button.spec.md`「Dismiss 視覺類」+ `patterns/element-anatomy/item-anatomy.spec.md`「Dismiss canonical」)。理由:

- **Canonical scope**:FileViewer 是 overlay chrome(Dialog 類),chrome corner close X 歸 Button dismiss 分支(button.spec.md line 284:「Dialog / Sheet / Popover / Alert **chrome corner close X** → `<Button iconOnly dismiss />`」);Inline Action dismiss 分支保留給 host chrome padding 內的 Tag / Input clear / Notice 等場景
...
- **onLoad 自動 fit-to-page**(世界級 canonical:Figma / Preview.app / Acrobat 開圖預設 fit-page,非 100%)——shell 計算 `min(cw/iw, ch/ih)` 對應的 zoom %,透過 `onZoomChange` 更新 UI 顯示真實倍率(例:40%)
- **zoom = 100% 語義 = natural pixel size**(跟 Figma / Preview 一致),不是 CSS-contained 的 fit scale
- Scroll wheel 縮放;`zoom > 100%` 時可拖曳 pan
- **Wheel step canonical 為 `0.03`(每 tick ~3% scale delta)+ smoothStep `0.005`**——對齊 Figma / Preview.app / Photoshop 細緻度。原 0.1(10%)太粗,接近 Google Slides 離散慣例非世界級連續 zoom。library 內部 multiplicative 乘算 → log 視覺等距(符合「等距」需求)
