...
**世界級對照**:Linear「Show N more」、Notion「Show N more」、Slack「Show more」、Gmail Labels「More」——全部是「section 底部的 muted 命令 row」,行為與視覺一致。
...
**為什麼結構性強制**:過去曾寫出「忘記接 isActive / onClick 的啞 item」(stories 展示 inline action 時發生),視覺上看起來可點擊卻永遠不變成 active。把 selection state 內建到 Provider、讓 button 接 `id` 就自動 wire,consumer **無法**寫出啞 item——沒傳 id 就失去 selection 能力,一眼看得出來。
...
| Token | 預設值 | 用途 |
|-------|--------|------|
| `--sidebar-width` | `16rem` | 展開寬度 |
| `--sidebar-width-icon` | `3rem` | icon 模式寬度 |
