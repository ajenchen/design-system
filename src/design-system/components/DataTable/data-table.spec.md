...
- 水平 padding 引用 `--ui-space-*` token，隨 density 響應——觸控模式自動獲得更寬鬆的操作間距
- 垂直 padding 由目標行高推導：`(行高 - 文字行高) / 2`，製造置中效果同時保持頂部對齊
- Header 與 body 共用同一組 padding，視覺節奏一致
...
**邊框標記「這裡有使用者看不到的內容」或「這是一個可互動的容器」。** 使用者看不到全部內容時，邊框告訴他這是一個有邊界的區域，裡面還有東西。沒有邊框時，被裁切的文字看起來像 render bug。

加框的條件（滿足任一即加）：
- **內容水平溢出**（需要水平捲動）——最常見的情境，欄位總寬超過容器寬度
- **有 frozen column**——固定欄與捲動區域的陰影分界需要外框歸屬
- **全表 inline edit**——可編輯容器需要邊界提示，跟 input 加邊框是同一個原則
...
- ❌ 內容不溢出、無 frozen column、非 inline edit 的表格不加外框
