...
- **Badge**（select）：Badge 可縮，文字在 Badge 內部 truncate，背景跟著縮——不是整個 Badge 被裁掉
- **多 Badge**（multiSelect）：用 `maxVisible` + `+N` 控制數量，不靠視覺裁切
...
- ❌ Badge 不可被外層 overflow-hidden 裁掉邊框——Badge 自身 shrink + 內部文字 truncate
- ❌ 數字欄位不靠左對齊——靠右才能縱向比較
