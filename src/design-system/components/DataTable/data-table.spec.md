...
| 層級 | 能力 | 狀態 |
|------|------|------|
| **L1 基礎結構** | 骨架、尺寸、border、色彩、行高模式 | 本文件 |
...
水平 padding 隨 density 響應（觸控模式自動加大），垂直方向由行高模式決定（見第三節）。
...
**Header 獨立於垂直 scroll 容器。** Header 不用 sticky——sticky 元素的半透明背景在 dark mode 會透出底下的 body rows，而 table 的底色（surface）是半透明的，無法預先算出不透明值。把 header 放在 scroll 外面，水平捲動用 JS sync，是 AG Grid 等世界級 table 的做法。
...
- 垂直 padding 由目標行高推導 `(行高 - 文字行高) / 2`，單行時製造置中效果，多行時保持頂部對齊
- `wrap: true` 的欄位可換行撐高 row

### 四、外框規則

**邊框標記「這裡有使用者看不到的內容」或「這是一個可互動的容器」。** 沒有邊框時，被裁切的文字看起來像 render bug。

加框的條件（滿足任一即加）：
- **垂直捲動**（固定高度 / 虛擬模式）——底下有看不到的 row
- **水平溢出**——右邊有看不到的欄位
...
**可推導的值用 CSS `calc()` 表達推導過程，不硬寫結果。** 這讓依賴關係留在 code 裡——上游值變動時，下游自動跟著算。

例：header divider 的上下內縮 = `(行高 - 文字行高) / 2`。改行高或字體大小時，divider 位置自動調整，不需要有人「記得」去改第三個數字。
