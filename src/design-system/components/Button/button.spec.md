...
將低頻操作收進溢出選單時，使用 `MoreVertical` icon-only 按鈕，variant 隨群組一致。MoreVertical 收納工具列**所有區塊**的低頻操作，永遠落在工具列末端。

左側通常不加分隔線。例外：工具列中**已有多條分隔線將按鈕分成多個功能區塊，且 MoreVertical 代表全域溢出**（非特定區塊的溢出）時，才可在左側加一條以明確它是全域邊界。
...
- 溢出按鈕（MoreVertical）收納所有區塊的低頻操作，永遠是工具列絕對末端；左側不加分隔線，除非工具列已有多條分隔線且 MoreVertical 為全域溢出
