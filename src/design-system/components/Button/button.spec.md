...
將低頻操作收進溢出選單時，使用 `MoreVertical` icon-only 按鈕，永遠是 `text` variant。

位置規則：
- 工具區**無關閉按鈕**：放在工具區絕對最右側
- 工具區**最右側是關閉按鈕**：放在關閉按鈕左側分隔線之前，此時 MoreVertical 自身左側仍有其他工具按鈕
...
- 溢出按鈕（MoreVertical）永遠是工具區最後一個按鈕，見上方「溢出選單」位置規則
