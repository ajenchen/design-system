...
將低頻操作收進溢出選單時，使用 `MoreVertical` icon-only 按鈕，variant 隨所在區塊一致。有兩種使用模式：

**區塊溢出（section overflow）**
MoreVertical 是某個區塊的最後一個按鈕，代表該區塊的低頻操作。左側不加分隔線——區塊右側的分隔線已自然標示邊界。

**全域溢出（global overflow）**
當 ButtonGroup 有多個區塊，但不希望每個區塊各自出現溢出按鈕時，使用一個 MoreVertical 統一收納所有區塊的低頻操作，放在 ButtonGroup 絕對末端。此時**可以**在左側加分隔線，明確標示「以下為全域溢出」。
...
| 規則 | 說明 |
|------|------|
| 功能分群 | **同視覺類型**（同為有框或同為無框）的按鈕需要功能性區隔時使用；跨視覺類型（有框→無框）本身的視覺差異已完成分群目的，不需疊加分隔線 |
...
| 避免孤立 | 單一按鈕不應兩側都有分隔線 |
| 切換按鈕依預設狀態 | text ↔ checked 的切換按鈕，依**預設（text）狀態**安排位置與分隔線；切換為 checked 時外觀改變，但位置與分隔線不變，避免 layout shift |
...
- 必須設定 `aria-label`（同時作為內建 tooltip 的文字來源）
