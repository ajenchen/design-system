...
**正方形保證三重鎖**(2026-04-25 加第 3 層):
1. **Padding 公式**:`px-[calc((field-height - icon-size - 2px)/2)]` —— 水平 padding 等於 `(高度 - icon - border 2px)/2`,**必扣 border**(`border border-transparent` base 各邊 1px),讓 icon 落在正中心(密度切換時自動重算)
2. **`aspect-square` CSS rule**:強制 `width: height`,即使消費者誤傳 `children` / `badge` / `loading` 等會撐寬的內容,CSS 仍鎖死方形
3. **`[&_svg]:shrink-0`**:Defense in depth,SVG 不被 flex `min-w-0` 擠扁(防 inner-area 計算誤差導致 icon 變 14×16 asymmetric — 2026-04-25 真實 bug origin)

**Bug origin(2026-04-25 修)**:用戶用 DS Devmode 在 mobile Safari 看 iconOnly Button 的 Lucide icon,發現 width=14px(對齊 `--font-body-size`)/ height=16px(對齊 `--font-body-lg-size`)— **真實 asymmetric 渲染**。Root cause:padding 公式 `(field-height - icon)/2` 沒扣 border 2px,inner area = 14×26 但 SVG declared 16×16 + flex `min-w-0` 容許 shrink → 水平擠成 14,垂直 16 維持。Fix:padding 公式扣 2px border + `[&_svg]:shrink-0` 雙保險。
