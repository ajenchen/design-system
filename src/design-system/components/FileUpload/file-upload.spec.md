...
## 三狀態視覺(world-class dropzone 慣例)

| data-state | 情境 | 視覺 |
|------------|------|------|
| `idle`(預設) | 使用者未互動 | `border-dashed border-divider`,背景 `bg-surface`,hover `bg-neutral-hover` |
| `drag-over` | 使用者拖檔進入區塊內 | `border-dashed border-primary`,背景 `bg-primary-subtle`,icon 變 `text-primary` |
...
**為什麼用 Empty 而非自己畫**:兩者共用「居中 icon + title + description 說明」的視覺語法,屬同一個 DS 設計語言 primitive。Empty 已 own 此結構(icon Avatar 48px、text-body-lg font-medium 標題、text-body desc、mb-4 / mt-0.5 gap),FileUpload 改重畫 = 雙邊漂移。跨元件 DRY = 世界級 DS 治理。
...
- ❌ **不在 FileUpload 裡自己畫「已選檔案清單」**:那是 `FileItem` 的職責;FileUpload 完事就發 `onUpload`,清單渲染走 consumer `{files.map(f => <FileItem ... />)}`
