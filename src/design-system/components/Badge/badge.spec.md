...
### 選 level 的流程（四步驟，從 low 起跳）

每一步都要有**明確理由**才升級；沒理由就留在 low。
...
- **為什麼合法**: Badge count 是輔助 indicator（次要訊號）,12px 會讓正圓 16×16 的 Badge 看起來擠。對照世界級:Material Badge 10sp、Polaris Badge 字級小於 body、GitHub Counter 10-11px 都小於 body footnote
...
Badge 有三種常見放置，各有不同視覺語意：

### 1. Overlay（疊加在元件角落）

最常見——Button iconOnly / Avatar / Icon 的右上角疊 badge。

```tsx
<div className="relative inline-flex">
  <Button variant="tertiary" size="sm" iconOnly startIcon={Bell} aria-label="通知 (3 則)" />
  <Badge count={3} variant="high" className="absolute -top-1 -right-1" />
</div>
```

- **語意**：元件主內容（icon / avatar）+ 補充通知
- **實作**：badge 用 `position: absolute`，不影響主元件 layout box
...
- ❌ 一個元件上疊加多個 badge——signal crowding，資訊合併成一個 badge
