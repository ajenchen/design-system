...
| **range 端點 cell bg** | 灰底半圓 track,**高度 = button** | `before:` pseudo `inset-y-0.5` + `rounded-l/r-full` + `left/right-0.5`(M8 對齊 Ant / Material X / Apple / Google)| 圓弧半徑 = button 半徑無錯位;舊版 cell-level bg 圓弧半徑 16px 比 button 14px 大 = 視覺 misalign |
...
## Nav button canonical(2026-05-03 v8)
...
**Icon 顏色 `text-fg-muted` rationale**:對齊 world-class secondary affordance 共識 — 月曆 nav chevron 是 secondary affordance 不是內容焦點:
- Ant Design `.ant-picker-header-prev-btn { color: rgba(0,0,0,0.25) }`(`https://github.com/ant-design/ant-design/blob/master/components/date-picker/style/panel.ts`)
- Material X `<IconButton color="default">` (muted grey[500])
- Apple HIG SF Symbol secondaryLabel color
- 我們套 `text-fg-muted`(semantic neutral,同一抽象層,oklch(0 0 0 / 0.45)= 45% 黑)

⚠️ RDP `PreviousMonthButton` override 必丟 `children`(RDP 把 `<Chevron>` 當 children 傳)否則會 double svg(canonical 2026-05-03 v8 抓到的 bug)。
