...
**suffix 可以塞什麼(2026-04-20 canonical — 2026-04-20 修正:支援 Switch 同時遵守 24px 閾值)**:
...
- 語意:宿主 bg 有色（Tag subtle blue 底 + blue-text 文字）,inline action icon 用 `fg-muted` 會視覺不連貫（灰 icon 夾在藍底藍字中異物感）;繼承 host 文字色維持「一個整體視覺單元」
...
### Predicate:Inline Action vs Button iconOnly(position-based,2026-04-22 精化)

**根據 action 的「位置」而非純 host-coupling 或 size**。3 個類別 +「獨立」總共 4 種:

| 位置類別 | Canonical | Size 規則 |
|----------|-----------|---------|
| **在 host chrome padding 內**(Field endAction / Tag/Chip body dismiss / overlay corner close / Alert/Notice/Toast dismiss / SidebarGroup header chevron)| **Inline Action** | 從 host 自動推(16/18 或 20/22) |
| **Row 內 inline suffix slot**(Menu / TreeView / SidebarMenuButton / SelectionItem 的 suffix,跟 row 內其他 icon / text **並列流 flow**)| **Inline Action** | 從 RowSizeContext 自動推 |
| **Row 內 dedicated action region**(FileItem right actions / DataTable frozen action column 等,action 跟 row content **視覺獨立並列**) | **Button iconOnly, variant="text", size = row tier** | `size="xs/sm/md/lg"` 對稱 row tier |
| **獨立 context**(toolbar / page CTA / form 獨立按鈕) | **Button**(任意 variant + size) | consumer 決定 |

### 為什麼 row dedicated region 用 Button(size-pair)

Row 內所有元件(cell content / InputDisplay / Badge / action button)**共用 row size tier**(sm row → 所有元件 sm)。如果 action 用 Inline Action(16/18)會比其他 row 元件(sm = 28 / md = 32)小一截,視覺不一致。Button iconOnly size 跟 row 對稱,所有 row 元件尺寸連貫。

**判斷 row inline suffix vs dedicated action region**:
- Suffix 跟 row content 共用 padding / flex flow(Menu 的 ⋯ 跟 icon/text 在同一 flex row)→ **inline suffix** → Inline Action
- Action 有自己獨立區塊 / 分隔線 / frozen column(DataTable action column 有 `border-divider` 分隔 / FileItem 右側 action 跟 body 視覺分開)→ **dedicated region** → Button

### 真實例子(position 分類對照)

**Inline Action(host chrome 內)**:
- Select / Input / NumberInput 的 clear X / ChevronDown(Field chrome padding 內)
- Tag 的 dismiss X(Tag body 內)
- Dialog / Sheet / Popover / Alert / Notice / Toast close X(chrome corner)
- SidebarGroup collapsible chevron(header 內,**可點擊 icon 就是**,不管叫 chevron)

**Inline Action(row inline suffix)**:
- MenuItem 的 suffix ⋯
- TreeView 的 hover-reveal 「⋯」/「＋」
- SidebarMenuButton 的 suffix icon
- SelectionItem 的 inline suffix

**Button iconOnly, size = row tier(row dedicated region)**:
- FileItem rich(sm row)→ Button sm iconOnly
- FileItem compact(xs row)→ Button xs iconOnly
- DataTable sm row actions → Button sm iconOnly(**不是固定 xs**)
- DataTable md row actions → Button md iconOnly
- DataTable lg row actions → Button lg iconOnly

**Button 獨立**:
- FileViewer toolbar zoom +/-
- Rich text editor toolbar bold / italic
- Page CTA / form button
...
### Dismiss 按鈕 canonical(2026-04-22 重訂:position-based + dimming 規則)

Dismiss 是「關閉 / 移除 anchor」的 icon-only action。**不再硬規定用 Inline Action,改照上方 position predicate 決定實作**,但**視覺弱化是 cross-implementation canonical**(不管實作是哪種,icon 色弱化)。

#### Dismiss 的實作 = position 決定(按 predicate)

| Dismiss 位置 | 實作 |
|-------------|------|
| 在 host chrome 內(Alert / Dialog / Sheet / Popover / Toast / Coachmark / Tag dismiss) | **Inline Action**(`ItemInlineActionButton`) |
| 在 row dedicated action region(FileItem right delete / DataTable row delete action)| **Button iconOnly, variant="text", size = row tier** |
| 在 row inline suffix(少見,但可能 future Menu dismiss action)| **Inline Action** |
| 獨立 context 的 dismiss(罕見,e.g. standalone close panel button)| **Button iconOnly, variant="text"** |

#### Dismiss 視覺弱化 — cross-implementation canonical

**不管實作是 Inline Action 或 Button,dismiss icon 色一律弱化**:

| 實作 | Icon 色(default)| Hover icon 色 |
|------|----|----|
| **Inline Action dismiss** | `fg-muted`(neutral host 規則) | `foreground` |
| **Button dismiss**(iconOnly + variant=text + 是 dismiss callback)| **icon 色 override 為 `fg-muted`**(不是 Button text variant 的 default `foreground`)| `foreground` |

**Dismiss callback 判定**:`onClose` / `onDismiss` / `onRemove` / `onClear` 之一 → 視為 dismiss 視覺類 → 套 dimming。

**為什麼 dimming 統一**:dismiss 本質是「可忽略」的副動作,不該跟 primary content 搶視覺焦點。不管實作,視覺弱化保持一致的 "opt-out hint" 語意。

#### Colored host 例外(Tag dismiss)

**`anchor 本身有 solid/branded 色背景`**(Tag 的 `red / blue / orange` variant)→ dismiss icon **繼承 host 文字色**(非 fg-muted),hover bg 配 host 色相。詳見本 spec 上方「Icon 色彩按 host 分兩支」段。

#### ❌ 禁止

- **用帶文字 label 的 Button 作 dismiss**(如「關閉」/「Close」文字按鈕)—— 雙重 affordance,違反 dismiss「可忽略」本質
- **自刻 `<button><X /></button>`** — 繞過 `ItemInlineAction` / Button 的尺寸 / hover bg / a11y 自動化
- **Button dismiss 用 variant="primary" / "secondary" / "tertiary"** — 這些 variant 視覺 chrome 太強,搶 dismiss 的弱化語意。**dismiss Button 必 variant="text"**
- **Button dismiss icon 色不 override** — 預設 Button text variant 是 foreground 色,對 dismiss 太強;必 override 為 fg-muted

#### 第三方 managed 不算例外

sonner toast 的 auto-dismiss、Radix Dialog 的 `DialogClose` wiring —— 第三方庫只負責 state logic(何時關),**我們改寫後的視覺按鈕必須套對應實作**(chrome 內 → ItemInlineActionButton / row region → Button iconOnly dismiss styled)。
