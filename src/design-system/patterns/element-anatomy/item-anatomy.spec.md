...
```
item 永久視覺層是 standalone card/pill(bg + radius + 不貼父邊 inset)
  → 連續排列必有 gap(防 card/pill 融合失去各自 identity)

其餘情況(permanent flush / permanent transparent)
  → 0 gap 合法;分隔 affordance 靠 border-b / 底部 progress bar / connector line
```
...
| **FileItem compact + bg-neutral-3**(Type B form attachment 靜態) | `bg-neutral-3 + rounded-md + inset` → **standalone pill** | **必 gap ≥ 4px** | bg 塊融合消失 item 邊界 |
...
- `file-upload.stories.tsx` compact list(Type B `bg-neutral-3` standalone pill)→ 改 `gap-1`
