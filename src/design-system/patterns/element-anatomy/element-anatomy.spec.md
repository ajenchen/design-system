...
**與既有規則的關係**:
- `overlay-surface.spec.md` 「overlay list 三 invariant」第 3 條(content 在 hover bg 內 breathing)= 本規則的**應用層 instance**(hover bg 也算「permanent」在該 state 內),非獨立規則
- `CLAUDE.md # UI 開發規則`「Padding source 分層規則」定 **padding 來源**(token / Tailwind / calc);本規則定 **padding 是否存在**(是 invariant)
