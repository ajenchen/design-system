...
| **Size** | `sm`（Toolbar 標準）；form / dialog 用 `md` |

**可配置資料操作**（篩選、排序、分組）的 variant 建議：

| 狀態 | Variant | Label |
|------|---------|-------|
| 未配置 | `text`（預設）；若需要更強的可操作性提示，可用 `tertiary` | 通常 icon-only；脈絡不明確時補 label |
| 已配置 / 啟用 | `checked` | icon-only |

篩選、排序、分組三個圖示（Filter、ArrowUpDown、Layers）在資料表格脈絡下意義唯一，Toolbar 場景通常全程 icon-only。`checked` 的底色 + 藍色 icon 已足夠傳達「目前有生效的條件」。
...
溢出是機制，不是角色。**溢出按鈕一律 `text size="sm" iconOnly`**，不隨業務層有框/無框而改變。溢出排除在全局排序之外。
...
```tsx
<Button variant="primary" startIcon={Settings}>設定</Button>
// 工具層不應搶業務操作的視覺重量
```
...
- `button.spec.md`：Button 元件層規則（variant、size、icon、狀態、ButtonGroup、ButtonDivider）
- `button.spec.md` 的 Toolbar 段落應精簡為引用本文件
