---
name: Retired Codex exec transport provenance(地端 path + visual audit bypass + large-brief 歷史)
description: Historical non-authority record of a retired Codex-specific transport; commands and mandatory wording below must not be executed as current policy.
type: feedback
originSessionId: 41fa83c2-f951-431e-911e-ed3ceb185903
status: retired
authority: non-authority-historical-provenance
supersededBy: packages/governance/canonical/providers.json
---
# Retired Codex exec transport provenance

> **Historical non-authority — do not execute the commands or operational requirements below.**
> This record preserves incident provenance only. The executable authority is the provider registry,
> provider-review binding, model-invocation profiles, and their certification/readback contracts.
> `scripts/codex-run-guarded.mjs --brief/--stdin` is retired and is not a supported entrypoint.

## Rule 1 — 地端 transport = `node_modules/.bin/codex`(3-test discovery)

**啟 codex collab 前必跑 3-test discovery,順序固定**(per `packages/design-system/ds-canonical/skills/codex-collab/SKILL.md` Step 0.4):
...
**Anchor 2026-05-27**:User 要求「ensure codex 把所有元件都驗證過並截圖」+ explicit authorize「codex 並不會動到你的檔案，對吧？那就照你建議做」。Final 62/62 PASS；當時 artifact 位於 legacy `.claude/snapshots/codex-visual-audit-2026-05-27/audit-report.json`，僅作歷史 provenance。新產出必走 Git-owned neutral runtime evidence，不可再寫 provider home。
...
(**無 model pin** + **effort = 最近一次經授權 probe 證實的最高可用檔**,2026-07-10 實測 ultra > xhigh)= Codex local adapter policy — model:解 pin 隨 reviewed CLI default;effort:檔位支援是 server 執行期資料(enum 已有 max/ultra),靜態不可知。`scripts/check-codex-freshness.mjs` 只在 Codex adapter 明確選擇時作為唯讀 diagnostic，不宣稱是 provider-neutral workflow 的每次自動前置；共通 authority 改由 `packages/governance/canonical/providers.json`、model-release registry 與 provider-review binding 負責。只有取得模型呼叫與本機設定變更的明確授權後，才可用 `--probe --allow-model` 由高往低實測並以 atomic update 更新 config 與 Git-owned runtime evidence。CLI 換版或 evidence 漂移時一律 fail closed，不可自行升級套件、呼叫模型或改 config。codex exec 不帶任何 effort/model flag 即繼承已驗證設定。
