---
name: Retired Codex-specific dual-track provenance
type: feedback
status: retired
authority: non-authority-historical-provenance
supersededBy: packages/design-system/ds-canonical/skills/independent-review/SKILL.md
---

# Retired Codex-specific dual-track provenance

> **Historical non-authority.** The fixed Claude/Codex layer mapping and Codex-specific trigger
> language below preserve the original user-feedback trail only. Current execution must resolve
> author, independent peer, isolation, transport, and certification from the provider registry and
> `resolveProviderReviewBinding`; unavailable evidence is `REVIEW-BLOCKED`.

**Date**: 2026-05-07 起;2026-06-11 合併 feedback_codex_collab_2026_05_23_directives.md(prune D8,142→98 行零保護損失)
**5-step 操作細則 SSOT** = `packages/design-system/ds-canonical/skills/codex-collab/SKILL.md` + canonical meta-patterns M31 row；`.claude/skills` 與 `.agents/skills` 都只是 generated discovery views。本檔 = provenance(user verbatim)+ 紀律 invariants。
