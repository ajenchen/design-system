...
- **Enforcement**:唯一五步=`pr-checks→merge→publish→readback→consumer`;**合併前有一道發版同意閘**(user 說「發版」才落 receipt;綁 **user 看過的預覽內容指紋**,不綁 commit 也不綁分支;一份同意只授權一次 final release。缺則停在 `AWAITING_USER_RELEASE_CONSENT`,SSOT = AGENTS.md「# Git / release canonical」1.5),其餘全 AUTO;只有未決產品/UI/UX SSOT 真取捨 ASK。Login/MFA/OAuth/credential reference 完成後 AUTO resume。hard gates 清單以 `infra/governance/release-workflow.json` 為準(prose 對照 = `feedback_solo_dev_workflow.md`「Release hard gate」,不在此重述)。
