...
- **當前誠實狀態(2026-07-29 收官更新)**:**已具外部證據** — npm Trusted Publisher(3 套件,OIDC+provenance,beta.95/96 registry readback)、GitHub Release ×2(BOM/SBOM assets)、WM consumer canary(beta.96 後 13/13 對齊 + Audit 回基線)。發版鏈 13 棒收斂並硬化:publish 冪等(integrity skip/異位元組 fail-closed)+ readback 有界重試 + tamper meta-test deterministic-isolated + 測試版本讀 package.json。**仍 blocked/not-certified**:GitHub App verdict 與特權簽章授權(2026-07-29 user 拍板 1B/3B 拆除,見對應 PR)、真實雙 provider model certification、fleet rollout 與 72h soak — 無外部簽署證據前照舊 fail closed,不可用本機 JSON 代替。
