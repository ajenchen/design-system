...
- **當前誠實狀態(2026-07-31 更新)**:**已具外部證據** — npm Trusted Publisher(3 套件,OIDC+provenance,beta.95/96 registry readback)、GitHub Release ×2(BOM/SBOM assets)、WM consumer canary(beta.96 後 13/13 對齊 + Audit 回基線)。**仍 blocked/not-certified**:external activation ledger 21 項全 `not-activated`、`controlPlaneGenesisTransition.state=open` 且 `releaseAllowed=false`(發版硬擋)、真實雙 provider model certification(`review-capability-certifications.json.certifications` 為空)、fleet rollout 與 72h soak。無外部簽署證據前照舊 fail closed,不可用本機 JSON 代替。
