...
| 寫入閘 `EXACT_UI_UX_TARGET_BINDING_MISSING` 但 user 明明點名了 | 口語 target(「agent logo」「fab」)與檔名 `agent-logo` 對不上、或問句+委託研究被當未決:改 canonical `approval-evidence.mjs`(別名/directive/markers/委託研究)+ 補 sh 測試 → 重生成後 live hook 才吃到(它讀 `.claude/hooks/lib` 這份 generated provider view 的副本,非 canonical owner);**禁**改用 Bash 寫檔繞過(2026-09-02) |
| 寫入閘擋**純註解 / docblock** 改動(最新 user 訊息含「設計語言」「視覺」等 UI 詞但沒點名 target,例如收尾「確保所有內容都有 SSOT 沒有漂移」) | canonical `approval-evidence.mjs` 已有「純註解操作」判定(2026-09-15):整檔剝註解相同 + TypeScript 去註解轉譯位元相同 → `COMMENT_ONLY_OPERATION_NO_RUNTIME_EFFECT` 直接放行;字串裡的 `//`、型別、className 改動仍擋。仍被擋 = 那不是純註解,別硬拆成多次 Edit 湊 |
| `release:auto` merge 回 HTTP 405「Pull Request is still a draft」 | pr-checks 建的是草稿 PR(canonical),merge shim 已補「draft → GraphQL markPullRequestReadyForReview → merge」(2026-09-16);同意收據仍綁 exact head,不需重問 |
| 消費者升級交易 GOV-UPGRADE-007「immutableHeadSnapshot is not the exact previous authenticated currentSnapshot」 | 消費者必須**逐版接力**(WM 在 130、目標 132 → 先補 131);缺的那版若 GitHub Release 不存在(npm 已發、Release 儀式沒跑,beta.126/131 同款)→ 對該 tag 觸發 `release-recover.yml`(repository_dispatch `recover-published-release`,token 只需 contents:write;workflow_dispatch 會 403) |
| Release Recover 在「Install locked dependencies once」GOV-DEPENDENCY-BOOTSTRAP-001(舊 tag 的 lockfile 帶事後公告,如 fast-uri) | 已改成相依只在 `$RUNNER_TEMP/recovery-tools`(主分支)裝、tag 樹符號連結借用(2026-09-16);本機重現同流程要 `NODE_USE_ENV_PROXY=1`(node fetch 才走沙箱代理)+ `npm_config_cache` 指到可寫目錄;`release-github-release.mjs` 走 gh CLI,沙箱 TLS 不通 → 這一步只能在 CI 跑,別在本機做 token/proxy shim(分類器會擋,而且不該) |
...
| `npm run sync-memory` 在沙箱 EPERM(home 鎖目錄) | 沙箱不准 Bash 寫 `~/.claude/projects`(provider home 的 non-authority 可重建 cache,不是 owner);改用 Write 檔案工具把 repo `governance/memory/*.md` 逐檔鏡射到 home(方向仍是 repo→home,2026-09-02) |
