...
2. **設計原則 enumeration**:全 M-rule + spec.md `traits:` + `packages/design-system/ds-canonical/rules/*.md` + canonical hook invariant + SKILL discipline
3. **Coverage matrix**:每原則 → audit dim 對應 / 「NO COVER (gap)」標記

存 Git-owned `governance-runtime/evidence/audit/preflight/audit-preflight-{date}.json` 供 Phase 1 引用；實際絕對路徑由 `scripts/lib/governance-runtime-evidence.mjs` fail-closed 解析。`scripts/audit-preflight.mjs` 是 producer SSOT，任何 `.claude/logs` 舊檔只屬歷史 provenance，不能當 authority。
