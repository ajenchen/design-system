...
| # | 項目 | 卡在哪 |
|---|---|---|
| A1 | 全 DS 焦點指示 SSOT 文件 | 卡在「常駐清單該不該像選單一樣讓滑鼠移動焦點」的研究。已知:`@radix-ui/react-menu` 的 `dist/index.mjs` 於 `onPointerMove` 呼叫 `item.focus()`;TreeView / DataTable 則用 CSS `hover:` 偽類(鍵盤觸發不了)+ 獨立的 state,是兩種模型 |
| A2 | 內描邊 vs 外描邊的決定規則 | user 已拍板「不統一但寫下規則」:預設外描邊;**祖先會裁切時**(捲動容器 / `overflow-hidden` 面板 / 表格列)用內描邊。待寫入 A1 文件 |
| A3 | `--neutral-selected-focus` 退役 | user 已拍板。**順序**:先補焦點框,深一階底色才變成多餘。依賴 A1 |
