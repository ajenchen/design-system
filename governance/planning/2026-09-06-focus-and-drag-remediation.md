...
### AD78 照拍板的建議執行:骨架門檻收緊 + 每列成本砍半(2026-09-11)

**user 2026-09-11 逐字**:「照你建議,確保完美完整達到我的要求和標準」。我的建議是 **(A) 保留骨架但把出現門檻拉高**、
**(B) 真正的功夫花在把每列成本降下來**。兩件都做了,以下是逐項證據。

**(A) 門檻**:加上必要前提 `commitCost > 33ms`(兩幀)。原判準只看「視窗移動距離 ÷ 預掛緩衝 > 2」,
把「使用者甩得快」和「機器畫不動」混為一談 —— 真實滾輪一次十格位移約 1,000px,是 200px 緩衝的 5 倍,
於是每次正常甩動都被判成跟不上(實測 100% 的幀有骨架、最多同時 159 個骨架元素)。
收緊後**畫得動的機器不論甩多快都不出骨架**;緊急跳轉那條同樣加上這個前提。只會讓骨架更少,不會更多。

**(B) 每列成本**:CPU 剖析(5× 節流)逐項砍:

| 自身時間(修前) | 是誰 | 修法 | 修後 |
|---|---|---|---|
| 307.6ms | 殼列記帳(每列每區 Map/Set + commit 集合差集,**無條件跑**) | 加休眠旗標:這一輪不會出殼且上一輪沒有殼 → 整套記帳不做 | 併入下一列一起量 |
| (同上) | cost-model 的 layout effect 讀 `scrollTop`(DOM 剛改完 → 逼出同步版面) | 改用 render 開始時已讀到的 `S.renderOffset`,零 DOM 讀取 | — |
| 266.8ms | 釘選面板同步一次事件讀三次捲動位置 | 重用同一事件已讀到的值 | 讀兩次 |
| 169–232ms | `Tag` 截斷量測(`getComputedStyle` + `measureText` + `clientWidth`) | 字型 / 文字寬度兩層快取,字體載入完成即清 | 仍是第二大項,但重複量測消失 |
| 139.7ms | `PeoplePicker` 頭像串兩個 `[class*=…]` 選擇器 | 元素查一次快取(斷線才重查) | **48.7ms(−65%)** |

**實測(user 的真實 Chrome,`data-shell-state` 直接讀)**:每列成本 **13.7 → 6.4–7.8ms**。

**誠實的界線**:一次 commit 在那台機器上仍需 56–72ms(視窗 13 列 × 6.4ms + 固定成本),所以**骨架仍會出現**。
那是 13 列 × 6.4ms 的物理結果,不是 bug。要讓它完全消失,只有兩條路:再往下砍每列成本(下一輪),
或接受 main 的做法(不出骨架、改成畫面凍住半秒)—— 後者 user 已經表態不接受。

**真實瀏覽器的讀數本身雜訊很大**(同一版連跑三次,每列成本量到 6.4 / 7.8 / 12.2ms;長工合計 968–1590ms),
因為自動化分頁是被遮蔽的、rAF 只在真實輸入時跑。所以**判定用可重現的無頭閘**,真實瀏覽器用來抓「無頭量不到的東西」
(骨架比例、`data-shell-state`、每列成本的量級)。兩者分工要寫清楚,免得又拿單次讀數下結論(AD77 的教訓)。

**驗證**:332 條不變式 / 捲動成本 / 溢出指示器 / 快速捲動閘(含長工與幀距斷言)/ 感知閘 dpr1 三速 + bursts + dpr2 bursts /
hover 閘 / 把手裁切閘(含對照組)/ 感知控制組,全過。**視覺稽查**:DataTable、Tag、PeoplePicker、Combobox 每個場景
與 HEAD 的基準像素差**完全相同** = 零視覺變化。
