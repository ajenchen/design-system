...
| # | 題目 | 判定 |
|---|---|---|
| 1 | **蓋板態面板要多寬** | **推導得出來,不用問。** B 條原文是「窄螢幕以抽屜**蓋滿**宿主」→ **全寬**。不是選擇題(**2026-09-16 已改字**:左留 `--layout-space-viewport-inset`、底下鋪遮罩,見 v14「來源總帳(2026-09-16)」)|
...
### AD92 空白改判「同 job 交錯比 main」——AD91 的忙等對照是壞分母(2026-09-11)

AD91 用「每個 scroll 事件忙等 120ms」的固定工作量對照當正規化分母。**它不行,因為會飽和**:
它的空白被注入的 120ms 主導,對機器慢度相對不敏感。同一段期間:

| | 忙等對照 | 被判定那一跑 |
|---|---|---|
| 變化幅度 | 737 → 1198ms(**1.6×**) | 162 → 687ms(**4.2×**) |

而且同一個 job 內它自己就量到 **825 與 1198**(1.45× 差)。拿它當分母只會把真實差距壓掉 ——
`64749b89` 的判定就是這樣:對照說機器只慢 6%,實測空白卻高 4 倍,兩者對不起來。

**正確的參考點必須有同樣的工作量與同樣的敏感度 —— 也就是同一支 story 的 main 建置。**
CI 的 DataTable job 現在會另外 build 一份 `origin/main` 的 storybook,兩個建置**在同一跑之內交錯量測**,
判定改成「分支空白中位 ÷ main 空白中位 ≤ 1.25」。這正是 user 從頭要的「把 main 當低標」。

**本機實證**:分支 52ms vs main 885ms(比值 **0.06**)。
**判定表對照組**(已進 CI):機器兩邊一起慢 → 過;main 沒慢而分支慢 → 紅;剛好 1.25 → 過;超過 1ms → 紅;
`119e279f` 的骨架回歸 438 對同期 main 801 是 0.55 → **不該誤紅**,有 case;沒有參考資料 → `skip` 並印出來,**不靜默放行**。
...
### AD94 兩個我自己寫的閘出錯,其中一個**訊息在說謊**(2026-09-11)

**(一)Dialog 高度閘的對照組在 CI 沒生效,腳本卻印「如預期紅」**
`--selftest` 只路由 `dialog-*.js` 來注入破壞,而 **CI 的 chunk 切法跟本機不同** → 一個 chunk 都沒改到 →
`fail` 是 0 → 走進 `SELFTEST ? (fail ? 0 : 1)` 分支 exit 1,但同一行的三元運算先印了
「✓ 對照組:拿掉上限後如預期紅(儀器有效)」。**訊息與事實相反,而且是綠色勾勾。**

修法三件:(a) 路由**所有** `.js`、以內容(`calc(100svh - `)判斷而不是檔名;(b) 記錄真的改寫了幾個 chunk,
**0 個就直接紅並說「這次什麼都沒驗到」**;(c) 只有在「真的改寫到 **且** 真的紅了」時才可以宣稱儀器有效。
本機重跑:「改寫 2 個 chunk 拿掉上限後,3 條斷言如預期紅」。

**教訓**:對照組要能宣稱「儀器有效」,必須同時證明 **(a) 破壞真的施加了** 與 **(b) 施加之後真的紅了**。
少了 (a) 就只是「什麼都沒做所以沒紅」—— 那不是證據,而且比沒有對照組更危險,因為它會印綠色勾勾。

**(二)長工與幀距還停在絕對門檻**
同一輪 CI 上 **main 自己**就量到長工 698ms、幀距 845ms(而分支是 647 / 731,**每一項都比 main 好**)——
...
## AD109 — 自適應緩衝的判準改成「預測」;兩個被推翻的版本與一個量測紀律

CI 的 `--ref=main` 比值閘在 `7f342b23` 紅:空白大幅改善(main 1258ms → 分支 691ms)、
但**長工 236 → 366ms、幀距 326 → 411ms 都超過 1.25 倍**。空白換長工不是淨勝 —— user 感受到的卡頓正是長工。

**版本 1(固定 10,照抄 AG Grid)**:4× 節流下最長空白 451 → 566ms、面積 516 → 972。慢機器每次 commit 要掛的列變多。

**版本 2(事後守衛 `commitCost > 50ms 就不擴`)**:**反應式所以無效** —— commitCost 要先被量到變長才觸發,
而那幾次變長的 commit 正是它該避免的。本機 3× 重現 CI:長工 374 > main 289×1.25;
把自適應**整個關掉**則 345 ≤ 379 全過。這個 A/B 證明退步確實來自緩衝擴張,不是別處。

**版本 3(預測式,採用)**:一次全量 commit 要畫 `視窗列數 + 2 × overscan` 列,
要求 `列數 × costPerRow + fixedCost ≤ 50`(Long Tasks API 的長工定義),解出 overscan 上限。
實測:1× 預測餘裕 +21.8ms → 緩衝 10(空白 30 幀/67ms → 5–9 幀/17–33ms);
**2× 以上餘裕即為負 → 緩衝回到下限 5,行為與改前完全相同**。
也就是說慢機器(含 CI runner)根本不會擴,CI 的長工不該再有可歸因於緩衝的差異。

**量測紀律(這次學到的)**:同一份程式碼在本機 3× 節流下,`--ref=main` 的長工中位數跑出
**220ms(過)與 404ms(紅)** —— 雜訊主導。所以這條**不可用本機單跑判定**,要以 CI 的同窗交錯比值為準。
我一度以為版本 3 在 3× 仍紅,重跑才發現是雜訊。

**閘改成機器無關**:原本的 A1「快機器必須拿到 10」是絕對門檻 —— 那只是在量跑閘的那台機器,
CI runner 不節流就已經是慢機器,A1 在那裡必紅。改成 A1 不低於下限 / A2 不超過 AG Grid 值 /
A3 能力越強緩衝不得更小 / A4 有餘裕時機制必須真的動(沒餘裕明白標不適用,不假裝驗過)。
對照組把回報值改成 0 與 99 → A1/A2 如預期紅。

**AD109 續 — 版本 4:緩衝只在捲動中重算(pre-commit 的 I16 抓到)**

版本 3 送出前被 pre-commit hook 擋下:`I16` 紅 ——「內容盒縮 20px 後 header 內層 minWidth 應 +20,
且**列數不得變**(沒有 React 事件)」,實際列數 15 → 13。

根因不是測試假象:`costPerRow` / `fixedCost` 是**會隨時間收斂的量測值**,每次 render 都重算緩衝
會讓它在**完全沒人操作**時自己飄動。I16 注入邊框後等重繪收斂的那幾秒裡,緩衝就變了。
閒置時掛載/卸載列是白費的工,而且會讓任何「靜止態」的量測不可重現。
...
## AD112 — 出殼斷言的能力守衛讀錯時間點(CI 同一份程式碼一綠一紅)

`1a16e2ab` 只改帳本文件、**零程式碼改動**,但 `DataTable pixel gates` 紅了;
同一份 DataTable 程式碼在 `4f8759b3` 是綠的。先查是不是自適應緩衝的回授迴圈
(緩衝變大 → commit 變長 → costPerRow 上升 → 出殼判準以為畫不動 → 出殼):
**假說不成立** —— 400px 視窗下 overscan 在 1×/2×/3× 全維持 5、出殼 0,我的改動在那個視窗下不改變行為。

**先講好消息**:那一跑的 `--ref=main` 比值閘過得很漂亮 ——
最長連續空白 main 1246ms → 分支 **167ms**;長工 330 → **120ms**;幀距 322 → 210ms;script 1121 → 836。

紅的是另一條:`--assert-max-shell-frames=0`,「2 趟 2 / 0」中位數 1。
根因在**守衛讀錯時間點**:`shellCost`(= 視窗列數 × costPerRow + fixed)是在手勢**結束後**
用一次 1px 微捲讓元件吐出 `data-shell-state` 再讀一次,而 `costPerRow` 是平滑值(0.6 舊 + 0.4 新)——
讀到的是最後那個微小 commit,**不是中途最糟的那一刻**。於是守衛判定「畫得動」而套了「不准出殼」,
但那一趟中途真的畫不動、正確地出了殼。

**修法**:元件記 `costPeak` 高水位(每次 commit 一個 `Math.max`,零成本,只在 debug 旗標開啟時輸出),
閘的能力守衛改讀峰值(舊 build 沒有該欄位時退回 `costPerRow`,行為同以前)。
本機 400px 在 1× 與 4× 都仍是 0 幀(該視窗下捲動容器實際可視列數少,兩者都落在區間內),
所以這條的真正裁決在 CI。

## AD113 — 2× 那個 1.1 秒空白解了:出殼判準問錯了問題

**CI 先確認上一輪修法**:`2abfe5b0` 全綠(required check `Verify` = success,六個 job 全綠)。

接著追第 5 項。兩個假說先後被自己的量測推翻:

1. ~~每列成本 ∝ 每列元件數~~:roadmap 每列 157 個元件、costPerRow 1.3ms;virtual-scroll 每列 **197** 個、
   costPerRow **0.9ms** —— 元件更多反而更便宜,不成立。
2. ~~儲存格內容(P13)~~:AD111 已用消融推翻。

**真正的根因是判準問錯了問題**。2× 節流下模型算出「一個視窗 65ms 畫得完」(< 120 門檻)所以不出殼,
但實際空白 1.1 秒 —— 因為那 1.1 秒裡視窗移動了 6600px = **165 列**。
判準問的是「畫得完**一個視窗**嗎」,真正的問題是「**跟得上速率**嗎」。

速率訊號一直都在(`behind` = 位移 ÷ 預掛緩衝),但被 `cannotDrawViewport &&` 擋著。
**為什麼以前必須擋**:緩衝固定 5 列時 `behind` 只反映捲動速度、與機器能力無關,單獨用它會讓快機器也出殼。
**為什麼現在可以放**:實測快機器 commit 密集 → 每次位移小 → `behind` 自然低;
1× 四趟全 0 殼(而且是在緩衝只有 5 的忙碌機器上測到的,所以這個能力感知**不依賴緩衝擴張**)。

**必須用平滑值 `S.behind` 而非瞬時 `S.pendingBehind`**:瞬時值在 1× 實測三趟出現 0/0/**1** 幀殼(單次抖動)。
平滑值有遲滯,代價是 2× 的改善從 218ms 退到 417–501ms —— 但 1× 零回歸比那 200ms 重要得多。

| CPU | 改前 | 改後 |
|---|---|---|
| 1×(四趟) | 2–7 幀 / 17–50ms / **0 殼** | 4–11 幀 / 17–96ms / **0 殼** |
| **2×** | 54–59 幀 / **1086–1193ms** / 0 殼 | 42–43 幀 / **417–501ms** / 7–8 殼 / 補齊 98–112ms |
| 4× | 35–45 幀 / 349–433ms | 44 幀 / 417ms |

長工在 2× 從 174–225ms 降到 **94–104ms**。

**回歸**:400px 不准出殼 0/0、指標底下的列全程有 hover、hover 中位 10–24ms、自適應緩衝閘 A1–A3 過。

**順修自己的閘**:A4 的前提條件寫錯 —— 原本只要求「餘裕 ≥ 一列的雙側成本」就期待緩衝超過下限,
但公式要的是「算得起**超過下限**的列數」。機器忙碌時(餘裕 8.5ms、算得起 2 列 < 下限 5)會誤紅,
而元件回下限才是正確行為。前提已對齊公式。

## AD114 — 獨立覆核推翻我三個說法(其中兩個是我自己埋的閘缺陷)

user 問「這次跑完會解決我最早提的卡頓與 hover 延遲嗎」。我開了 4 個獨立驗證 + 1 個對抗式覆核實測。
**它比我自己的評估嚴格,而且抓到三件我說錯的事**:

**① 我一直說「同機交錯 A/B」—— 錯的。** `data-table-fast-scroll.mjs:526` 是
`for (const build of BUILDS) { …跑完該 build 所有 runs… }` —— **build-major,不是逐趟交錯**。
正確說法:同一次執行、同一台機器,但兩個 build 先後跑完。覆核組另外用**反序重跑**控制順序效應
(1× 反序 branch 157 vs main 965、4× 反序 526 vs 1534),結論不變,所以結論站得住 —— 但我的措辭要更正。

**② 我寫的 overscan 閘,對照組是假的。** `data-table-overscan-adaptive-invariant.mjs:101` 原本
`const observed = SELFTEST ? [0, 99] : [...]` —— **把 observed 直接換成寫死值**去打 A1/A2,
完全沒走 `data-shell-state` 的讀取路徑;A3/A4 又在 `if (!SELFTEST)` 裡、對照組模式下根本不執行。
那句「✓ 對照組:如預期紅」只證明了大於小於號會算。**這正是 M32 要防的假對照,而我自己造了一個。**
**修**:對照組改成在頁面裡持續竄改 `data-shell-state` 的 overscan 欄位(快的強制 0、慢的強制 99),
閘走完整讀取路徑拿到壞值;A3 移出 `if (!SELFTEST)`。對照組現在抓到 5 條。

**③ 沒餘裕的機器上整支閘是零鑑別力。** A1/A2/A3 在「緩衝停在下限」時全是恆真式(5≥5 / 5≤10 / 5≥5),
A4 又標不適用 —— CI runner 正是這種機器。**補 A5(公式一致性)**:用實測到的
`costPerRow` / `fixedCost` / 視窗列數重算公式,元件輸出必須等於它。公式寫錯、常數被改、clamp 方向反了都會紅,
且**不依賴機器有沒有餘裕**。對照組實測:實際 0 / 公式 6、實際 99 / 公式 5,兩條都紅。

**④ hover 閘把最糟的失敗態濾掉了。** `data-table-hover-latency.mjs:168` 的
`s.filter(Number.isFinite)` 讓「1.5 秒內完全沒變色」的樣本**從中位數與最大值裡一起被剔除**,
只在括號印個註記 —— 而那正是 user 抱怨的「游標到了卻要等好一陣子」的極端形態。
**修**:`report()` 一併回傳 `lost`,`lost > 0` 直接判紅。

**覆核組確認站得住的**:hover 中位快約 10 倍(main 110/117ms vs 分支 9ms,兩次 A/B 可重現,
且 main 跑同一支閘是紅的 3/147);快速捲動最長連續空白 1× 158 vs 992ms、4× 526 vs 1560、6× 534 vs 2072;
DataTable 範圍內 332 條不變式全過。

**覆核組指出仍未滿足的**(誠實登記,不掩蓋):
- 「卡頓已解決」無證據 —— 分支自己在 4×/6× 仍有 526/534ms 最長空白、停手後 0.4–0.8 秒補骨架
...
- 視覺稽查主線對那三個缺陷貢獻是 0(124 scenario / 12 個 hover / **0 個幾何斷言** / **0 個 dark mode 場景**);
  攔下它們的是三支針對個案手寫的像素閘
- Switch thumb 的 16px 直徑(spec.md:90)沒有任何腳本在斷言 —— 日後尺寸走鐘不會有閘紅
- SSOT 無漂移 / 符合世界級設計這兩項,四份報告**都沒有查**,目前無證據

## AD115 — 補上 user 真實捲動路徑的閘(滾輪),並修掉 4 處「缺值偽造成 0 然後通過」

獨立覆核指出 `grep -c "mode=wheel\|mode=pinned" ci.yml` = **0** ——
user 的真實情境是「Chrome 開預覽、用滾輪捲」,而 CI 只跑 CDP 合成手勢(gesture),**滾輪這條路零覆蓋**。

**成本**:wheel 兩趟 **8.5 秒**。零覆蓋沒有任何理由。

**同機對照(n=3,分支每一項都優於 main)**:

| 指標(中位/最大) | main | 分支 |
|---|---|---|
| paint 連續空白 ms | 277 / 364 | **225 / 239** |
| **long task 數** | **7 / 8** | **1 / 3** |
| **long task 合計** | **585 / 627ms** | **83 / 211ms** |
| script | 590 / 609 | 508 / 509 |

**為什麼 wheel 原本判不了**:`groups` 只收有 `r.g`(截圖幾何)的結果,而 screencast 是 gesture 專屬
(`data-table-fast-scroll.mjs` 的 `mode === 'gesture'` 分支)。改成收全模式,
取不到值的指標由 gate / relGate 各自印「這個模式沒有這項量測,不適用」。

**順手抓到 4 處假綠**:`(r) => r.g?.presentedGapMax ?? 0` —— `?? 0` 把「沒有這項量測」**偽造成 0**
然後拿去跟門檻比,必然通過。改成 `?? NaN` 讓守衛明說跳過。這正是我一路在指控的那種病,一共 4 處。

**回歸**:gesture 的完整 ref 判定照常(空白 18ms ≤ main 969×1.25、長工 0 ≤ 71×1.25、幀距 236 ≤ 378×1.25)、
`--selftest` 三個偵測器都紅、400px 不准出殼 0/0。

## AD116 — CI 抓到速率出殼的真代價;改用「降門檻」達成同一目的

`0d2472d1` 的 CI ref 判定紅在**幀距**:branch 454ms > main 263×1.25。完整對照:

| 指標(中位/最大) | main | branch(速率 OR) |
|---|---|---|
| 最長連續空白 | 1185 / 1249 | **258 / 342** ✓ |
| 殼幀 | 0 / 0 | 10.5 / 11 |
| **幀距 max** | **263 / 321** | **454 / 706** ✗ |

也就是說速率出殼在慢機器上**把 1.2 秒空白換成 0.45 秒畫面凍結** —— 是改善,但不是淨勝,
而且那個凍結是 main 沒有的新症狀。**不調門檻換綠燈。**

**改用另一條路達成同一目的**:保留 AND(能力判準),把 `SHELL_ENGAGE_VIEWPORT_MS` 從 **120 → 60**。
120 原本取 AG Grid 每幀 60ms 預算的**兩倍**(理由:內容晚 120ms 出現比先看骨架好);
但那個取捨被實測推翻 —— 2× 下模型算 65ms「畫得完」所以不出殼,實際整片空白 1.1 秒。降回一倍即可進入。

| CPU | 速率 OR | **AND + 門檻 60** |
|---|---|---|
| 1×(三趟) | 4–11 幀 / 17–96ms / 0 殼 | 2–11 幀 / **17–19ms** / **0 殼** |
| **2×** | 42–43 / **417–501ms** | 35 / **183ms** |
| 4× | 44 / 417ms | 43 / 467ms |

**模擬 CI(3× 節流,同機 ref 判定)四條全過**,而且出事的那個指標反而優於 main:
空白 334ms(main 1446)、**幀距 210ms(main 461)**、長工 108ms(main 281)、補齊 246ms。

## AD117 — 降門檻的 CI 結果:ref 判定全過;剩下的紅是我自己製造的 SSOT 漂移

`95c08c22` 的 CI:**ref 比值判定四條全過**,連上一輪出事的幀距也反轉:

| 指標(中位/最大) | main | branch |
|---|---|---|
| 最長連續空白 | 1237 / 1263 | **164 / 172** |
| **幀距 max** | **321 / 342** | **261 / 266** ✓ |
| long task max | 358 / 366 | **113 / 120** ✓ |
| script | 1120 / 1121 | 863 / 870 |

剩下紅的是 `--assert-max-shell-frames=0`(5/4 幀),**根因是我製造的 SSOT 漂移**:
出殼門檻有**兩份**——元件 `data-table.tsx:1803` 改成 60,但閘 `data-table-fast-scroll.mjs:119` 還硬寫 120。
於是閘用 120 判「畫得動 → 不准出殼」、元件用 60 判「畫不動 → 該出殼」,兩邊打架,CI 必紅且紅得沒道理。

**修法不是手動同步**(下次還會漂),是**讓閘從元件讀那個值**:
元件把 `engageMs` 一起寫進 `data-shell-state`,閘讀它;舊 build 沒有該欄位時退回 120(原行為)。
這一類漂移從結構上消失。

順帶修:`shellCost` 從數字改成 `{ cost, engageMs }` 之後,長工門檻的後備路徑也跟上新形狀
(沒跟上的話 `Number.isFinite(物件)` 恆 false → 那條後備會靜默失效)。
...
## 2026-09-12 — 快速捲動空白:根因修正 + 目的地閘(CI 全綠)

**User 的定位(原話)**:「你知道 main 只是低標嗎?理想上 data table 整體互動和體驗越順暢越好」、
「我認為任何情境『理想』上都不應該看到空白,但也不應該為了達成此目的而讓體驗和互動卡頓」。

### 我原本錯在哪(兩層)

1. **閘的框架錯了**:本檔既有的 `--assert-*` 全是「本 build ÷ main ≤ 1.25」的**回歸閘**,
   語意是「別比 main 差」= 把 main 寫成天花板。加 `--assert-max-blank-frames=0` 絕對目的地閘修正。
2. **出殼判準問錯問題**:`cannotDrawViewport` 問「這台機器**畫得完一個視窗**嗎」= **能力**判準,
   而症狀是**跟不上捲動速率**。快機器畫得完 → 條件恆假 → 骨架永遠不出。
   實測 CPU×1(不節流)38 幀裡 **27 幀整片空白、17/17 帶全空、最長連續 410ms**,列殼只出 1 幀。
   上一輪把門檻 120→60ms 只是把同一個錯問題問得嚴格些,所以無效。

### 解法:幾何保證,不賭時序

捲動跑在合成執行緒、與主執行緒刻意隔離(Chromium RenderingNG),主執行緒被長工佔住時
React 不可能把列放到新位置 —— **任何需要主執行緒的機制都輸掉這場競速**(react-window 官方對
overscan 只敢說「can reduce」;AG Grid 是「列元素永遠在,內容是佔位」)。
已掛載列必為連續一段 `[first.start, last.end]`,此外的整個虛擬高度在同一次 render 鋪成兩塊
骨架底(`[data-row-shell-band]`,CSS 漸層)。覆蓋 = 上帶 ∪ 已掛載 ∪ 下帶 = 整個捲動區,**恆真**。

### CI 最終數字(b491d119,同窗交錯量測)

| 指標 | main | branch |
|---|---|---|
| **目的地:呈現幀空白** | — | **0 幀(2 趟 0/0,上限 0)** |
| 最長連續空白 | 1246ms | **0ms** |
| 長工最長(手勢) | 348ms | **119ms** |
| 長工最長(滾輪) | 224ms | **115ms** |
| 幀距最大 | 316ms | 284ms |

CI 8 個 job 全綠。`data-table-invariants` 332 條全過。hover 三輪 42 樣本全 0ms(節流對照 0→1→3ms 證明探針會反應)。
視覺稽查全 DS 102 scenario:對比 0 / 幾何 0 / 渲染錯誤 0;同環境(macOS)對照下 `datatable-pinned` **0.000%**。

### 這一輪修了三次才對的儀器(都是「沒有對照組的綠燈 = 零證據」的再犯)

1. 正對照原本只忙等主執行緒 —— 前提「主執行緒卡住 ⇒ 一定空白」**已被本次修正消滅**,對照組失效。
2. 改成「關掉骨架底」後,本機紅、CI 仍綠 —— CI 慢,**真列殼**大量出動把畫面填滿,
   等於在量機器快慢。兩層一起關才對。
3. 兩層都關之後幀數只剩 9,被 `presented >= 10` 判成證據不足 ——
   **干擾越有效幀越少**,那個守衛在最該綠時判紅。正對照下限降 5,負對照維持 10。

### 未關閉(登記)

- **殘留空白幀 —— 根因已定位(2026-09-12,不再是「未確認」)**:
  **捲動前緣新露出的圖磚偶爾在該幀送出時尚未完成光柵化,合成器畫成圖層底色(白)**,即 Chromium checkerboarding。
  **決定性實驗**(執行期注入 CSS,不動原始碼):把 `[data-row-shell-band]` 改塗單一實色後,
  絕大多數「空白帶」量到 **100% 該色** → 幾何正確、繪製也發生;少數幀同一區域 **100% 白**;
  另有 **75% / 81% 部分覆蓋**的中間態 —— 圖磚逐塊補上的簽名。
  它同時解釋全部觀測:單/雙幀(一個光柵化週期)、白而非骨架色(圖層底色)、永遠在前緣(新露出)、
  **不隨 CPU 節流惡化**(光柵化不在主執行緒)、**只有中央面板**(它是真捲動容器;像素量到釘選面板落後 10px,
  所以沒露出新區域)、rAF 探針失明、以及**實心填色消融無效**(繪製成本不是槓桿)。
  發生率:本機 8 趟 296 幀中 8 個空白幀(2.7%),4/8 趟出現;CI 三趟皆 0。
  **能不能消除**:繪製成本已排除(連純色實填也發生),剩下的槓桿是圖層／圖磚提示
  (`will-change: transform`、限制 band 高度)。實測 6 趟全 0 看似有效,**但同樣計數方式的對照組也是 6 趟全 0**
...
### 2026-09-12 再補:真實輸入路徑 + 真人捲動速度(閘原本兩者都沒覆蓋)

**兩個先前沒發現的量測缺口**(多角度分析指出,逐條讀碼 + 實測驗證):

1. **像素證據只接 gesture**(`data-table-fast-scroll.mjs` 的 `mode === 'gesture'` 分支)。
   wheel / mouse 完全沒有 screencast → 目的地閘只守 CDP 合成手勢,而 user 回報用的是真滾輪。
   **驗收路徑 ≠ 回報路徑。** 已把 screencast 提到 mode 分支之前,所有模式都收像素。
2. **`--mode=wheel` 結構上不可能有空白**:`RUN_TICKS` 是 `cb.scrollTop += dy`,JS 在主執行緒改捲動位置 →
   捲動與繪製同回合完成。對照組實測:關掉兩層機制 + 卡死主執行緒仍 **0 幀**;同樣干擾在 `mouse`
   (CDP `Input.dispatchMouseEvent`,走原生輸入管線)量到 **18 幀 / 348ms**。
   → selftest 改印「↷ 不適用」並附理由;gate 端加守衛,`--assert-max-blank-frames` 配 wheel 直接失敗,
   避免有人掛上**永遠不會紅的空閘**。

**第三個缺口:只有壓力速度可量。** `DELTAS` 寫死 `[300,400,500,600]` × 16ms ≈ **28,000 px/s**,
遠超過真人(Chrome 一格約 100px;連續快滾約 500-1,000 px/s、觸控板猛甩約 3,000-5,000)。
加 `--deltas` 之後才答得出「user 實際會不會看到」。

**本機三段速度(真實輸入路徑 `--mode=mouse`)**

| 速度 | branch 空白幀 | 最長連續 | 最嚴重 |
|---|---|---|---|
| 真人連續快滾 ~3,100 px/s | 8 趟 **0/8/1/5/0/0/0/0** | ≤34ms | ≤4/17 帶 |
| 觸控板猛甩 ~9,400 px/s | 6 / 7 / 10 | 22-23ms | 7-14/17 帶 |
| 壓力值 ~28,000 px/s | 8 / 12 / 12 | 33-66ms | 17/17 帶 |

**猛甩速度同窗 A/B(同一台機器,唯一變數是程式碼)**

| | main | branch |
|---|---|---|
| 空白幀 | 55 / 67 / 68 | **6 / 7 / 10** |
| 最長連續空白 | 1309 / 694 / 966ms | **22 / 22 / 23ms**(−98%) |
| 空白面積×時間 | 1146 / 1184 / 1103 | **19 / 28 / 54**(−97%) |

CI(2 vCPU)三趟 mouse 路徑皆 **0 幀**;gesture 路徑三趟 **0/0/0**。

**沒有立絕對零空白閘於真實路徑**:真人速度 8 趟裡有 3 趟仍有(最多 8 幀、≤34ms),
掛上去會 flaky —— 假綠與假紅都比沒有更糟。改用相對閘鎖住 −98% 不回頭。

### 要關掉最後那一段,需要的是結構性改動(登記,不自決)

剩下的殘留是**合成器前緣光柵遲滯**(根因已證,見上一節)。已排除的槓桿:繪製成本(純色實填照樣發生)、
`will-change`(6 對 6 無檢定力)。**唯一還沒試、且有機制根據的**是:把骨架圖樣從「每次 commit 都改
top/height 的兩塊 div」搬到**幾何穩定的 spacer 背景**上 —— 幾何不變則圖磚不必重畫。

**擋路的是已量到的事實**:列是透明的(實測 `rgba(0,0,0,0)`,格 / spacer / 捲動容器皆同,
白色來自 root 的 `bg-surface`),所以圖樣放 spacer 會從列底下透出來。要走這條路必須**讓列不透明**,
而那會動到 zebra / 選取 / hover 三層底色的疊法 —— 屬於產品／視覺 SSOT 的結構改動,**不自決**。

### 2026-09-12 拍板:不動列的底色模型(user 原話「照你建議不動列的底色模型」)

**決定**:不把表格列改成不透明底色。猛甩速度下殘留的「22ms、一小條閃白」維持現狀。

**理由(提案時給 user 的權衡)**:一般捲動速度已量到零空白(真人連續快滾 8 趟 0/8/1/5/0/0/0/0、
最長 ≤34ms);22ms 約一幀多一點,而讓列變不透明會動到**隔行變色 / 選取 / 滑鼠移過**三層底色的疊法,
外部消費者若自訂列色也可能跑版 —— 風險遠大於收益。

**連帶結論**:「合成器前緣光柵遲滯」這個根因**已定位、已記錄、不再追**。
未來若要重啟,唯一還沒試的槓桿就是這條(把骨架圖樣搬到幾何穩定的 spacer 背景),前提仍是列須不透明。

### 2026-09-12 修:捲動停下後那次重畫沒同步指標底下那一列(user 最早回報的 hover 問題)

CI 紅在 `data-table-hover-latency.mjs`:延遲很好(中位 16ms)但**「沒變色」3 次 / 16 次取樣**
—— 整整 1.5 秒完全沒反應。根因:`syncHoverUnderPointer` 只在 `scrollCommit || prevShells.size > 0` 跑,
而捲動停下後還會再重畫一次(虛擬視窗收斂 / 量測回填)並換掉指標底下那一列,兩個條件都不成立。
快機器上那次重畫幾乎不換列 → 本機 3 輪 48 次全正常;慢機器才浮出來。
修:加 `renderedAtNewOffset`,只在「這次重畫換了捲動位置」時補跑(`elementFromPoint` 會逼出版面計算,
不能每次重畫都付)。驗證:CPU×4 / ×6 共 64 個取樣,「沒變色」**0 次**。

### 2026-09-12 修:核准分類器把註解當程式碼

修上面那個 bug 時,改動的程式碼沒有任何視覺 token(`\bhover\b` 不命中 `syncHoverUnderPointer`,
識別字內沒有詞界),但**註解**裡寫了「hover」→ 判成 product-ui-ux 擋下。
兩個後果都不對:為了把 rationale 寫清楚而被罰;以及逼 agent 去索取一個本來就該 AUTO 的工程核准
(閘自己的訊息就寫「工程 remediation…不向 user 索取工程核准」)。
修:`stripCodeComments()` 在比對 `UI_OPERATION_MARKERS` 前剝掉註解,JSX 文字與字串字面值不動。
對照組 14a/14b 證明不是放寬:真的改 `className="hover:bg-blue-500"` 仍判 product-ui-ux。

### 2026-09-13 收斂驗證:三種機器速度 × 真實輸入路徑 × 與 main 同機對照

回答 user「各種速度機器之下都沒有如下問題,對嗎?」。同一台機器、同一輪交錯量測,唯一變數是程式碼。

**捲動(真人滾輪速度 ~3,100 px/s,`--mode=mouse`,n=3)**

| 機器 | main 空白幀 | main 最長連續 | branch 空白幀 | branch 最長連續 |
|---|---|---|---|---|
| ×1 快 | 20 / 18 / 21 | 50 / 51 / 88ms | **0 / 0 / 0** | **0ms** |
| ×4 中 | 45 / 53 / 39 | 672 / 1266 / 890ms | **1 / 1 / 2** | **17ms** |
| ×6 慢 | 19 / 27 / 31 | 850 / 788 / **1433ms** | **1 / 0 / 1** | **16ms** |

**hover(n=16;「沒變色」= 1.5 秒內畫面完全沒反應)**

| 機器 | main 捲動後 | branch 捲動後 | main 靜止 | branch 靜止 |
|---|---|---|---|---|
| ×1 | 中位 105ms、**6 次沒變色** | 中位 10ms、**0 次** | 中位 107ms、2 次沒變色 | 中位 10ms、0 次 |
| ×4 | 中位 115ms、**6 次沒變色** | 中位 17ms、**0 次** | 中位 128ms、2 次沒變色 | 中位 16ms、0 次 |
| ×6 | 中位 133ms、**6 次沒變色** | 中位 24ms、**0 次** | 中位 119ms、2 次沒變色 | 中位 22ms、0 次 |

**main 每 16 次捲動後 hover 有 6 次在 1.5 秒內完全沒有任何反應** —— 正是 user 回報的
「游標明明到了,table row 的反應卻要等好一陣子」。分支在三種機器速度皆為 0 次。
(hover 中位數只能當上界,真值低於儀器解析度,理由見 `data-table-hover-latency.mjs` 檔頭;
但「沒變色」的次數與解析度無關,是真訊號。)

附帶佐證:main 的串流靜置期送幀間隔最大到 **1433-1485ms**,分支是 114-523ms ——
主執行緒被塞住的程度差一個量級。

### 2026-09-13 追完:hover 探針的離群值根因(先前記成「未知」就停下,是不對的)

`延遲` 與 `首幀延遲` **16/16 逐位元相等** → 每一次命中的都是 `sentAt` 之後的第一張幀,
變色在那張幀之前就完成 → 這支探針量的是**截圖串流送下一張幀要多久**,不是列的反應。
離群值 = **串流那一次停了 175ms 沒送幀**;佐證已印進輸出(靜置期串流自己最大停 272ms,比離群值還大)。
逐條排除:元件本身 / 特定面板 / 記憶體累積 / 載入後固定時間 / 週期性。
