...
- **P2H(產品 / UI / UX SSOT 真取捨,由 user 拍板)**:列底色 150ms 過渡(A 維持 / B 表格瞬間、選單樹維持 / C 全 DS 瞬間;root 建議 B)。**Human-only 平台動作**:Codex 供應端額度(購買,或等 9/17 08:38 重置);其餘工程續跑依 Standing Authorization AUTO(root 已獨立驗證 patch 01 / 02,見下)。
