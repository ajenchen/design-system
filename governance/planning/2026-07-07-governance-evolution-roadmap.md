...
- **已實作**:`@qijenchen/design-system` 內建 versioned `qijenchen-ds-codemod`，以 exact TypeScript 5.9.3 AST/language service 處理 beta.84 `onClear`、`direction`、`mode="display"`、`DescriptionDirection` 變更。綁定不明、動態值與 `DataTable meta.disabled` 必須 fail-loud/manual blocker。
- **交易邊界**:`plan → check → apply(exact digest)`，proposal 封閉 source inventory/bytes/mode/parser version；合作寫入者 lock、symlink/hard-link/path 防護、已觀測 per-file pathname CAS 與 commit point 前 rollback 有正反測試。不宣稱 parent-directory swap、power-loss 或舊 open descriptor 寫入者保證。

### 7. Baseline accept workflow ✅ 2026-07-22 落地
- **理由**:Chromatic「Baselines only update when changes are accepted by you or your team」;今日手動四輪(dispatch→下載→看圖→cp→commit)。
- **已實作**:`qijenchen-ds-visual-baseline` 以 exact `pngjs@7.0.0` 解析完整 CRC-valid PNG，封閉 governed Git origin/HEAD commit/tree、candidate/baseline 清單、bytes/mode、explicit non-placeholder 差異陳述、proposal/review digest，並提供 `plan → check → apply-reviewed`。
- **信任邊界**:apply 只接受 BOM/sync-all 投影的短效 allowlisted Ed25519 human approval；managed-model-broker 分支在本 tool version 強制 `not-activated`，任意 digest pseudo-activation 無效，未來必須重用 canonical signed broker/model-authority receipt validators。callback、self-report、cross-repo replay、rogue/missing/symlink/concurrent image 均 fail closed。`authorIdentity` 是 audit label，不宣稱作者/審核者獨立；Git 綁定也只是 governed workspace replay boundary。canonical human policy 目前是 `not-activated`，因此如實提供 plan-only；工具不自動 commit/push。
