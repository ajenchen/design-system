...
# Claude instruction/context:fork/native-hook probes invoke a bounded local
# Claude Code model session, so the run must carry explicit bounded execution intent.
node infra/governance/bin/conform-provider-runtime.mjs \
...
- `reconcile-github.mjs` 透過已登入的 `gh api` 呼叫 GitHub REST API，不讀寫 token 檔，也不把 credential 寫入輸出。
...
- `governanceCheckApp.id=null` 或 `governanceWriterApp.id=null`、兩 App 的 integration ID/secret prefix 不 distinct、App check history 不存在、required workflow 可被 PR title/paths skip、mutation environment 未接 Writer App/workflow、protected-default dispatch 未從受保護預設分支載入或 payload exact tag 未綁定當前 protected `main` HEAD，任一情況都會在任何 mutation 前阻擋。
...
6. 建立兩個完全獨立的 GitHub Apps，不可共用 App、private key、integration ID 或 secret prefix：
   - **Governance Check App**：`repositorySelection=selected`，顯式 App permission 只有 `checks: write`（GitHub 的 Metadata read 為 App 內建最小存取）；精確安裝到 DS、published template 與 WM 這些必要 target。它只能發布 `Immutable consumer snapshot`，不得取得 Contents write 或 Pull requests write。
...
7. 將兩 App 的實際且 distinct integration ID 分別填入 `integrations.governanceCheckApp.id` 與 `integrations.governanceWriterApp.id`。兩者必須是不同整數，且都不可以用 GitHub Actions integration `15368` 代替。
8. 只在 exact 受保護 GitHub environment 建立兩組不共用的憑證：DS、published template 與 WM 的 `governance-check-verdict` 只放 `GOVERNANCE_CHECK_APP_ID` 與 `GOVERNANCE_CHECK_APP_PRIVATE_KEY`；DS 的 `governance-mirror`、DS 的 `governance-external-ledger`，以及 published template/WM 的 `governance-upgrade` 只放 `GOVERNANCE_WRITER_APP_ID` 與 `GOVERNANCE_WRITER_APP_PRIVATE_KEY`。Repository/organization secret fallback、模糊的 `GOVERNANCE_APP_*` 別名與任何其他 governance credential 都必須為空。
9. 先觀測至少一筆由 Governance Check App integration ID 產生的成功 check history；Writer App 產生的 check 不可用來滿足 required context。
10. 將 `npm-release` 綁定 exact release workflow（它不使用 Writer App）；DS `governance-mirror` 與 template/consumer `governance-upgrade` 綁定 exact workflow 並指定 `governanceWriterApp`；每個 `governance-check-verdict` 綁定 protected-default anchor 並指定 `governanceCheckApp`。只在第 8 點指定的 environment 配置對應 credential，不得放 PAT 或模糊別名。Consumer writer job 本身的 `permissions` 只保留 Contents read，寫權只來自 late-minted、target-installation-repository-scoped App token。Reconciler 必須讀回 `default_workflow_permissions=read`；published-template/product-consumer 的 `can_approve_pull_request_reviews=false`，authority 因 Changesets built-in version-PR writer 保留 `true`。App ID、installation、environment secrets 或 operational installation-token probe 尚未真實啟用時必須保持 external blocker，不可用 committed placeholder 假裝完成。
...
16. `governance-check-verdict` environment 只能提供 Check App，且限 protected branches；`release-finalize` 不使用 GitHub User/Team reviewer gate，獨立性由 exact quorum-signed finalizer authority 強制，且 authorizer subjects 必須與 stage-only OIDC publisher 分離。任一 authority/readback 未完成時 desired model 故意 fail closed。
