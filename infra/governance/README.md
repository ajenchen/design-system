...
4. **Evidence 執行邊界**:managed-CI executor 叢集(trusted-execution plan、executor supply chain、class adapters、OIDC 啟用鏈)已於 2026-08-04 隨 activation cluster 退役(可達圖與理由 → `governance/planning/2026-08-02-cloud-compat-and-deep-audit-baton.md` §8.1)。Managed broker 執行因此**永久 fail-closed**:任何宣稱 `managed-ci-attested` 的 receipt 一律拒收,本機 receipt 也永遠不會自動升級成 managed evidence。正式 model judgment 的唯一設計路徑仍是 repo-unmounted、tools-omitted、content-only provider API;Claude/Codex CLI 僅保留 runtime conformance 身分。
