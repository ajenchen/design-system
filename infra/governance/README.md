...
- GitHub hard gate 依 repository role 封閉選擇，但三者都使用 GitHub Actions `15368` 的原生 `Verify consumer`／authority checks：published template 由普通 `pull_request` workflow 執行，product consumer 由 protected-base `repository_dispatch` workflow 執行，兩者都不得使用已退休的 `governance-anchor.yml`、`Immutable consumer snapshot` Check App verdict 或 `governance-check-verdict` environment。任何需要寫入的 route 若 `governanceWriterApp.id=null`、Writer App installation/secret placement 未對齊、required workflow 可被 PR title/paths skip、mutation environment 未接 Writer App/workflow、protected-default dispatch 未從受保護預設分支載入或 payload exact tag 未綁定當前 protected `main` HEAD，會在 mutation 前阻擋。
...
9. 對每個啟用的 template-mirror/product-consumer target，觀測 native GitHub Actions `Verify consumer` 的 exact workflow/event/head：template 是 ordinary `pull_request`，product consumer 是 protected-base `repository_dispatch`。Writer App 產生的任何 check 都不可替代 required context。
