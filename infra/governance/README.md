...
5. **Standard release SSOT**：`release-workflow.json` 唯一規定 `pr-checks → merge → publish → readback → consumer`，五步全部 AUTO；只有未決產品/UI/UX SSOT 真取捨 ASK。Required CI、protected-main merge、immutable publish、GitHub/npm exact readback 與 template/WM protected-main readback 是 blocking graph。`staged-rollout-plan.json`、release rings 與以下 receipt machinery 僅屬 legacy/opt-in assurance，不得阻擋 `PRODUCTION_GRADE_SINGLE_OWNER_SMALL_TEAM` standard release。

`candidate-freeze`、offline signatures、72h soak 與 fleet promotion 對 standard profile retired；broad external activation 與 model certification non-blocking。以下 machinery 可在明確啟用的額外 assurance profile 中維持 fail-closed，但缺漏不得插入五步、改寫 completion、要求 user 核准或阻止 standard publish/consumer propagation。
...
External-ledger PR 在合併前仍必須由 protected-base `Governance anchor` 重建 exact handoff transaction。Reserved writer branch prefix 永遠走 writer verifier；prefix 不符才走一般 privileged-closure structural verifier，因此錯誤 bot/type/repository 的 writer-shaped PR 會 hard fail，不會 fallback。此 workflow 不提供 merge、approval 或 bypass；每一筆 PR 必須先合併並獨立 read back，下一筆 handoff 才能開始。尚未填入正整數 App IDs、environment secrets、signed activation 與 required-check readback 時，正確狀態仍是 blocked。

`npm run governance:activation-readiness -- --json` 是 legacy/opt-in assurance 的唯讀診斷，不是 standard release completion gate。它可誠實回報 `externalActivationRequired`，但 `candidateRelease=null`、certification、簽章、rollout 或 mirror 缺漏不得阻擋 `release-workflow.json` 五步。
...
以下 staged/fleet predicates 僅約束明確呼叫 legacy/opt-in assurance 工具的操作；standard release 只由 `release-workflow.json` 判定。
...
7. 在啟用相應 mutation route 前，將 Writer App 的實際 integration ID 填入 `integrations.governanceWriterApp.id`，並以 external-activation evidence 綁定 exact installation repository。`governanceCheckApp` schema/registry record 只保留為未啟用的相容／future-reserved 型別，active profile、plan、workflow 或 environment 不得引用。
8. 只在 exact 受保護的 DS `governance-mirror`、DS `governance-external-ledger` 與 template/consumer `governance-upgrade` environment 放 `GOVERNANCE_WRITER_APP_ID`／`GOVERNANCE_WRITER_APP_PRIVATE_KEY`。不得建立 `governance-check-verdict` environment，也不得放 Check App secrets、PAT、repository/organization fallback 或模糊 `GOVERNANCE_APP_*` 別名。
...
15. Privileged executable closure 的初始啟用是一筆普通 candidate PR，不得把 private key 寫進 repo，也不得要求 OWNER comment、key-enrollment command 或 per-change signature。同一 PR 依 active profile 原子地更新角色合格的 public-key records、privileged/rollout/runtime/release-tag 各政策的 key ID/quorum/digest，並把 `bootstrap.enabled` 永久改為 `false`。Protected-base verifier 會驗證 trusted+candidate 聯集 closure、active-profile bindings 與完整 changed paths。Small-team profile 的 privileged allowlist 精確一把同時具 `privileged-change-authorizer`／`root-rotator` 的 governed key；maximum-assurance profile 才要求 2..5 distinct-subject root quorum。後續 registry/policy rotation 或 revocation 也走同一結構驗證，不用 commit-bound authorization artifact。Registry 必須維持 append-only lineage：既有 issuer 不可刪除、re-key、改 subject/roles/有效期或從 revoked 恢復 active；只允許 active 保持 active 或單向轉成 revoked，並可追加新 issuer。新撤銷的 `revokedAt` 必須是合法 canonical timestamp；沒有 per-change `issuedAt` 錨點。候選設定若留下 revoked/expired/role-ineligible key、stale profile binding 或不足 quorum 會被拒絕。
16. Active profile 不得出現 `governance-check-verdict` environment、Check App required context 或 `release-finalize.yml`。相關 schema/helper 若為歷史 receipt 相容或 future-reserved 而保留，必須保持不可達於 current desired profile、staged-rollout plan、CI evidence plan 與 agent audit prompt；任一 current readback 未完成時仍 fail closed。
