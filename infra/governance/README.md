...
它不是薄包裝：先執行 role-bound `setup:governance` primitive，驗證 Node 下限與 lock 綁定的 npm 11.19.0 tarball／integrity，加上 exact brace-expansion 5.0.8 + tar 7.5.22 security overlays，以隔離的 npm 設定執行 lifecycle-disabled `npm ci`、registry signature audit 與 high-severity audit；DS-author 再 provision lock-matched package-local Chromium（Linux 一定執行系統 dependency setup）、跑 provider-neutral governance check與全部已登錄 local governance Harness。最後由同一入口安裝、驗簽、稽核並 read back provider CLI provisioning registry 中以 canonical lock 綁定的 CLI（目前是 Claude Code 與 Codex）。每個 compatibility provider 都必須在該封閉清單明示選擇 `repo-provisioned-cli` 或 `external-runtime`；新 provider 只有前者需要新增 exact npm lock／SRI／platform package，後者不會被安裝器擅自虛構 CLI。使用者或 hosted image 注入的 npm／Playwright／CLI override 不會取得 authority。DS-author 完整入口可能耗時數小時，因為 All-Harness 的外層期限由同一份 registry 中的所有唯一命令期限總和加固定收尾預留機械推導，不會被一般 bootstrap 的 15 分鐘預設誤殺。
...
3. **Evidence SSOT**：每次 deep audit 先 freeze exact Git tree/inventory，deterministic、hook 與 CI 證據都綁同一個 immutable run。Wrapper 宣稱還必須引用與 lockfile version/integrity 綁定的 dependency source bytes；缺 source 只能是 unverifiable finding。
4. **Evidence 執行邊界**:model-broker 執行層(evidence broker、shard/transcript、model runner、managed-CI executor 叢集與 OIDC 啟用鏈)已於 2026-08-04 全數退役(可達圖與理由 → `governance/planning/2026-08-02-cloud-compat-and-deep-audit-baton.md` §8.1/§8.3)。Deep-audit judgment 與 A1b model envelopes **永久 fail-closed**;唯一 review 路徑是 waived self-review(receipt 記錄 waiver 與 self-attested coverage,`unverifiedModelCoverage` 保持可見)。Entitlement review-lane contract 與 `model-api-transport` 純合約模組保留,供未來明確要求 independent review 的交付物使用;Claude/Codex CLI 僅保留 runtime conformance 身分。
5. **Standard release SSOT**:`release-workflow.json` 唯一規定 `pr-checks → merge → publish → readback → consumer`,五步全部 AUTO;只有未決產品/UI/UX SSOT 真取捨 ASK。Required CI、protected-main merge、immutable publish、GitHub/npm exact readback 與 template/WM protected-main readback 是 blocking graph。

`candidate-freeze`、offline signatures、72h soak、fleet promotion、broad external activation 對 standard profile 一律 retired;model certification non-blocking。repo 無法自行完成的少數真實外部保證(npm trusted publisher、2FA、rollback drill)改列 `infra/governance/external-assurance-checklist.md` 純文件 checklist,不進任何 blocking graph。
...
3. `external-assurance-checklist.md`:repo 無法自行完成的外部保證(npm trusted publisher、2FA、rollback drill)的 documentation-only 清單;機械驗證 inventory 已於 2026-08-04 退役。
4. `trust/issuers.json`:全 control plane 唯一 Ed25519 public-key registry;角色、有效期與撤銷狀態的 SSOT。
...
`audit:*check` 版本同樣唯讀,但比 `*plan` 更嚴格:CI/model check 需要目前 immutable active run。缺少任一真實證據時 non-zero 或明確 `blocked` 才是正確結果;不得把 plan 輸出當成 certification、promotion 或 rollout receipt。
...
對 trusted+candidate 聯集的完整 protected changed-path closure 做結構驗證；若 trust
configuration 改變，再驗證 candidate policy 與 issuer registry append-only lineage。此路徑不接受
per-change Ed25519 簽章、OWNER comment、key-enrollment command 或 commit-bound authorization artifact；
protected required checks 才是合併邊界。不得將本機 proposal acknowledgement 宣稱為授權。
...
Passing checks alone cannot authorize certification or release. `trust/issuers.json` is the only public-key SSOT for rollout receipts, runtime evidence, fleet recovery, and release-tag authorization. The privileged-closure verifier binds the same registry structurally but does not use a key, signature, or comment to authorize each PR. The policies contain only its digest, allowed key IDs, quorum, TTL, and the content-addressed active assurance-profile binding; they never duplicate public keys or quorum maps. The committed registry and allowed-key lists are empty, so every local run remains `attestation.status=pending`, promotion fails closed, and `stage-protected-release` cannot pass preflight. Initial trust configuration is one atomic structurally verified PR that assigns least-privilege roles, updates every policy reference to the same registry digest, and permanently closes bootstrap; it has no OWNER-comment or per-change signature ceremony. Under `PRODUCTION_GRADE_SINGLE_OWNER_SMALL_TEAM`, exactly one honestly governed key carries `privileged-change-authorizer` plus `root-rotator`; a dedicated release-tag-only key is a functional role boundary, not a claim of another custodian. Under `MAXIMUM_ASSURANCE_MULTI_CUSTODIAN_WORM`, the active profile requires a 2..5 distinct-subject root quorum and its external custody evidence. Private keys stay outside the repository and model environments. Every remaining cryptographic authorization/attestation signs `issuerRegistryDigest`; registry changes invalidate older evidence as mutation authority. Historical journal evidence remains usable only for read-only recovery observation after its issuer is rotated or revoked, and only when the current append-only registry still proves the issuer's immutable lineage. Verification rejects unknown roles, future/expired/revoked keys, insufficient profile-bound quorum, digest drift, or an evidence lifetime extending beyond key validity.

### External evidence 邊界(2026-08-04 後)

外部 runtime surface receipt 仍是 Ed25519 簽署、content-addressed 的 CAS artifact(`infra/governance/evidence/external-runtime/<sha256>.json`,lib = `infra/governance/lib/external-surface-evidence.mjs`);runtime certification 的唯一 promotion 入口是 `promote-runtime-evidence.mjs`,簽章、freshness、target binding 全 fail-closed。原 external-operator 儀式 CLI(import/propose/check/handoff、prepare/assemble/check-activation、external-ledger dispatch/writer workflow 與 `governance-external-ledger` environment)已隨 activation cluster 退役;`governance-anchor.yml` 不再有 writer-branch routing,所有 PR 一律走 privileged-closure structural verifier。
...
- `managed-host-effective-readback.schema.json` 是端點管理或 provider effective API 的封閉輸入契約;本 repo 的 verifier 只比對、不佈署。即使兩方都對齊,未經 target-bound readback 也只是 `aligned-unattested` 與 `not-certified`。
...
以下 fleet predicates 約束 `reconcile-github`/`consumerctl` 的 plan/apply 邊界;standard release 只由 `release-workflow.json` 判定。
...
- promotion eligibility 由有型別的 predicates 機械計算;未知 predicate 不能執行。`candidateRelease=null`、BOM/source commit/source tree/npm SRI 不完整、check 非 success、certification/upstream/waiver/plan 任一適用 gate 未過都 fail closed。
...
- Release BOM v5 另外綁定 immutable `product-template-scaffold.lock.json` asset 的 exact bytes。該 lock 封閉 provider/fork generation 後的每個靜態 published path/mode/size/SHA-256,並且只允許 npm 11.19.0 在發布後生成 `package-lock.json`。Mirror 只能由 protected-default `release.yml` 成功完成 exact-tag npm publish、registry provenance readback 與 GitHub Release asset readback 後交接。Mirror tree 由 `validateMirrorRoot` 對 protected-base allowlist policy 驗證:逐檔 path/mode/SHA-256 inventory、拒絕 symlink 與範圍外路徑;provider copy/evidence allowlist 不列 Claude/Codex,必須從 `governance.lock` 已綁定的 fork manifest `consumer.commonInstruction` 與每個 generated provider `managedSurfaces` 推導,未登錄 sibling 仍 fail closed。原 signed evidence envelope(receipt v4 + `mirror-activation-boundary-proof.json`)已隨 activation cluster 退役,無 live producer/consumer。
...
- plan 展示全 fleet 的 `candidateActions`;有 conflict 的 repo 不會 materialize 任何 action,`applyPlan` 再驗證其他 repo 沒有夾帶 action。
...
- GitHub hard gate 依 repository role 封閉選擇，但三者都使用 GitHub Actions `15368` 的原生 `Verify consumer`／authority checks：published template 與 product consumer 的一般 required check 都由 ordinary `pull_request` `audit.yml` 執行；發版驅動的 product receiver 另由 protected-base `repository_dispatch` 完成更新，並把同名 check 的 `details_url`、workflow event 與 base/head 精確綁入 release SSOT。兩條路都不得使用已退休的 `governance-anchor.yml`、`Immutable consumer snapshot` Check App verdict 或 `governance-check-verdict` environment。任何 required workflow 可被 PR title/paths skip、receiver dispatch 未從受保護預設分支載入、check 未綁 Actions run，或 payload exact tag 未綁定當前 protected `main` HEAD，都會在合併前阻擋。
- apply 先完成全 repo GET/preflight;每筆 ruleset/environment mutation 都把 normalized before-image、expected-applied digest、`verifiedReleaseEvidenceDigest` 與明確 compensation 寫入同一個 v7 journal/rollback-plan digest。journal 以 atomic rename 寫入並同步 fsync 檔案與目錄;v7 嵌入 immutable、normalized、digest-bound 的歷史 inventory/desired/attestation-policy/issuer-registry bundle 與 `evidenceDurabilityClass`。唯一 durability class 是 `local-content-addressed-fsync-v1`:事件 hash chain、檔案與目錄 fsync、exact readback。`historical-observation` 只以 journal 已綁定的 embedded bundle digest 重播,不會被日後 canonical digest 更版錯誤封死;`current-apply` 仍必須對齊目前 canonical SSOT。歷史 journal 只能授權後續 read-only observation,不能成為任何 mutation authority;整包替換、未知欄位、self-signed 替換 issuer、action 或 registry lineage 不符,都必須在呼叫 provider client 前失敗。
- `.github/workflows/release.yml` 是唯一 active release authority：protected-default `repository_dispatch` 先驗 tag 格式，並要求 event commit、目前 protected `main` 與 peeled tag commit 三者完全相同；接著一次安裝、build、typecheck、pack 三包，產 deterministic SBOM／scaffold lock／BOM，透過 npm Trusted Publishing 直接發布並驗證 registry provenance，再由 contents-only job下載同一 attempt 的 exact artifact set、發布並讀回 GitHub Release，最後觸發 template mirror。沒有 stage/approve/promote/finalizer 第二條 workflow，也沒有人工 2FA release ceremony；tag、main、package、BOM、artifact 或 registry readback 任一漂移都會中止。
- partial failure 明記 `failed-partial-state-possible`、`rollbackAttempted=false`。`--recover-journal` 永遠只用歷史 embedded bundle 觀測,因此目前 desired/policy 漂移或舊 issuer 在 transaction 開始後被 rotation/revocation 不會造成 recovery deadlock;它會以 journal `startedAt` 驗證歷史 issuer 當時尚未被撤銷,且不授權任何寫入。`--rollback-journal` 只有在 fresh recovery authorization 同時通過目前 release-rings 的 active/unrevoked `apply-authorizer` quorum 與目前 privileged policy 的 active/unrevoked `root-rotator` quorum 後,才會依反序 compensating plan 寫入;兩組 signature 分開計數與驗證。Small-team profile 可由同一把誠實標記且同時具兩角色的 governed key 通過兩個 gate,不冒充 principal/custodian independence。沒有安全 inverse 的 action 必須在其他 drift 對齊後,以新 plan 和新簽章成為唯一 action;跨 repo 或與可回滾 action 混合會在寫 journal 前被拒絕。每一步都會在 provider mutation 緊鄰前重驗 live clock、current SSOT、fresh profile-bound authorization、journal head 與遠端狀態。缺任一適用 gate、授權過期、遠端 drift、rollback plan digest 變更或無安全 inverse(例如 immutable releases enablement)一律在 mutation 前 fail closed 或標成 `rollback-blocked`,不得宣稱 rolled back。rollback 後的 forward recovery 不另建 flow:重新 readback、建立 fresh plan/authorization,並以新的 v7 journal 對同一 exact candidate 執行既有 apply API;舊 terminal journal 不得改寫或冒充新 transaction。本機 fsync journal 仍不是異地耐久證據,不得冒充。
...
以下全是 repo 外部狀態,不能以 committed prose 假裝完成。原 21 項機械驗證 inventory(`external-activation-requirements.json`)已於 2026-08-04 隨 activation cluster 退役;真實項目的人工追蹤清單 → `infra/governance/external-assurance-checklist.md`。仍然成立的結構性要求:

1. 三個 npm package identity 必須已存在,且版本只能由 `packages/design-system/package.json` 的 release SSOT 經 canonical generation 同步;不得用 `npm version`、Changesets 或人工 package bootstrap 改寫正式版本線。
2. 在 npm 為 `@qijenchen/design-system`、`@qijenchen/storybook-config`、`@qijenchen/governance` 三包設定唯一 Trusted Publisher:綁本 repo 的 `.github/workflows/release.yml` 與 GitHub `npm-release` environment,允許 protected-main workflow 使用 `npm publish` OIDC provenance。
3. npm organization/package 必刪除 classic/granular/bypass publish tokens;release job 不接受 `NPM_TOKEN`/`NODE_AUTH_TOKEN`,也不建立 stage/approve/promote 的人工 2FA ceremony。三包必在同一 exact run 依固定順序直接發布並逐包讀回 provenance。
4. 任何既有或後續版本都不可用人工 direct publish 補洞;一律走 protected-default `repository_dispatch`(payload exact tag 當下等於 protected `main` HEAD)→ build-once/BOM → npm Trusted Publishing + registry readback → 同 attempt GitHub Release asset readback → template mirror。`release-finalize.yml` 已退休,active plan/audit/文件不得再要求它。
...
6. Template mirror/consumer upgrade 的遠端 mutation 只使用 **Governance Writer App**:`repositorySelection=selected`,權限精確為 `contents: write`、`pullRequests: write` 與變更 `.github/workflows/**` 時才需要的 `workflows: write`。它不得取得 Checks write,也不得成為 required-check trust anchor;所有 required checks 都由 GitHub Actions 原生 workflow 產生。
7. 在啟用相應 mutation route 前,將 Writer App 的實際 integration ID 填入 `integrations.governanceWriterApp.id`,並以 desired-state readback 綁定 exact installation repository。`governanceCheckApp` schema/registry record 只保留為未啟用的相容/future-reserved 型別,active profile、plan、workflow 或 environment 不得引用。
8. 只在 exact 受保護的 DS `governance-mirror` 與 template/consumer `governance-upgrade` environment 放 `GOVERNANCE_WRITER_APP_ID`/`GOVERNANCE_WRITER_APP_PRIVATE_KEY`。不得建立 `governance-check-verdict` environment,也不得放 Check App secrets、PAT、repository/organization fallback 或模糊 `GOVERNANCE_APP_*` 別名。
9. 對每個啟用的 template-mirror/product-consumer target,依 `release-workflow.json` 觀測 GitHub Actions `Verify consumer` 的 exact workflow/event/head:template 是 ordinary `audit.yml` `pull_request`;product 的 release receiver 是 protected-base `sync-design-system.yml` `repository_dispatch`,check head 綁 PR head、producer head 綁 PR base。僅名稱相同或沒有 Actions run `details_url` 的 check 一律不可替代。
10. 將 `npm-release` 綁定 exact release workflow;DS `governance-mirror` 與 template/consumer `governance-upgrade` 綁定 exact writer workflow。Consumer writer job 本身的 `permissions` 只保留 Contents read,寫權只來自 late-minted、target-installation-repository-scoped App token。Reconciler 必須讀回 `default_workflow_permissions=read`;各 profile 的 `can_approve_pull_request_reviews` 以 canonical desired state 為準。
11. Required PR contexts 必對所有適用事件無條件出現;不得用 title/message skip、path filter 或 vacuous pass。DS packaging context 必為 `Packaging integrity(dims 84/85/86/88 light checks)`;consumer trust anchor 必為 GitHub Actions 原生 `Verify consumer`。
12. release workflow 產出 immutable BOM 並完成 live readback 後,才可把 exact `candidateRelease` 寫入 `release-rings.json`;`candidateRelease=null` 是刻意的安全鎖,不能用 placeholder digest 解鎖。
13. 為 provider runtime evidence 建立獨立 Ed25519 signing authority,只把 public key、角色與有效期加入 `trust/issuers.json`,再由 `providers/runtime-conformance.json` 引用 key ID;private key 不得進 repo、本機 staging evidence 或模型環境。未啟用前 promotion 必須維持 fail-closed,certification 不得從 `not-certified` 改成 `certified`。
14. Privileged executable closure 的初始啟用是一筆普通 candidate PR,不得把 private key 寫進 repo,也不得要求 OWNER comment、key-enrollment command 或 per-change signature。同一 PR 依單一 production profile 原子地更新角色合格的 public-key records 與 privileged/rollout/runtime 政策的 key ID/quorum/digest,並把 `bootstrap.enabled` 永久改為 `false`。Registry 必須維持 append-only lineage:既有 issuer 不可刪除、re-key、改 subject/roles/有效期或從 revoked 恢復 active。候選設定若留下 revoked/expired/role-ineligible key 或不足 quorum 會被拒絕。
15. Active profile 不得出現 `governance-check-verdict` environment、Check App required context 或 `release-finalize.yml`。相關 schema/helper 若為歷史 receipt 相容而保留,必須保持不可達於 current desired profile 與 agent audit prompt。
16. Rollback readiness 由 machine-verifiable local candidate→exact rollback→fresh-plan/new-journal forward-recovery 測試證明;每季人工 rollback drill 記錄於 `external-assurance-checklist.md`。fixture/local evidence 不冒充真實 GitHub drill 或異地耐久性。只有平台不可代理 login/MFA/OAuth/owner、缺 credential reference 或 plan 外付費等 human-only boundary 才停。
