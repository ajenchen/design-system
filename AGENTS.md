...
**本檔為所有 AI agent 的治理 SSOT bootstrap**:Codex 等 AGENTS.md 標準 agent 原生載入,Claude Code 經 `CLAUDE.md` 的 `@AGENTS.md` 載入同一份。跨模型 bootstrap 原則**只改本檔**。
**最終 authority 不是本檔**:本檔引導行為,強制層 = provider-neutral governance verifier、受保護 CI 與外部 required check;只有這些控制實際啟用且證據綁定當次 commit 才可宣稱 compliant,**本機 hook 或 workflow 檔存在本身不構成保證**。Claude 另有 write-time hooks 加速回饋(`.claude/hooks/`,provider 專屬加速器,非信任邊界)。
**治理／infra 改動只改 canonical owner**,不得手改 build graph 宣告的 generated outputs(`.claude` provider views、`.agents/**`、`.codex/**`、`generated/**`);完成後跑 `npm run governance:generate` 與 `npm run governance:check`。owner/output/non-authority 邊界由 `infra/governance/protected-root-classification.json` 封閉,新路徑未分類即失敗。
**巢狀 instruction 只准補充 scope,不得複製本檔**:Codex 由 repo root→cwd 合併 `AGENTS.md`,**整條鏈必守預設 32KiB**(超過會靜默截斷;閘 `check-agents-bootstrap.mjs` A1)。

# 每次任務前的 6 條 mindset(世界級設計系統的工作底色)

這 6 條是所有規則背後的**態度**,接到任務先複習再看規則。

1. **對標世界級 + 不取巧省工**——每個設計決策都要能回答「Polaris / Material / Atlassian / Ant / Carbon / Apple HIG 怎麼做?」沒對齊又說不出理由 = 設計 bug。**禁以「較簡單」「省 N edits」選 shortcut**;說「快速修」「省工程」是 yellow flag,停下重想。
...
5. **先證據、再分權**——無前例的產品／UI／UX SSOT 決策:grep 既有 → 讀近親 spec → 仍有真實取捨才停下問。純工程不確定性由最高 certified capability 依 canonical、tests 與 security gates 收斂,**不得用問 user 代替工程判斷**;禁憑直覺造新 pattern。
6. **大原則吸收瑣碎**——同類 bug 反覆糾正 = meta 層沒抓住。**權威清單與各條內容只有一個住所**:`packages/design-system/ds-canonical/rules/meta-patterns.md`(本行不重述條號,三處列舉必然漂移)。**AI 不需 user 提醒才找 root invariant**:rule 震盪 → 自跑 benchmark + invariant test;user 第 2 次問 → 必截圖 verify;對話結論 → AUTO 落地;visual/behavior decision 前必先查 ≥3 家世界級;**視覺／結構 propose 前必 grep DS spec.md 找 owner SSOT 並出 3-column 表,否則提案不被接受**。使用者 tell me once 不該要 tell me twice。
...
**規則放哪裡**:本檔只做每 session 導航,不是 home taxonomy 的第二份 SSOT。Level 1–9 的唯一 owner／scope／flowchart 是 `skills/design-system-audit/references/rule-placement.md`。Provider views 只作 discovery/delivery,不得反向成為 semantic owner。

**行數預算**:Bootstrap(AGENTS.md + CLAUDE.md)/ SKILL / spec / memory / hooks 各有 cap,**具體數值不在本檔重述**(三處列舉必然漂移)—— SSOT 是 `session_start_governance_check.sh` 檔頭「Checks + thresholds」與 Check 7,動態值以 `scripts/sync-governance-counters.mjs` 跑出為準。

**Anti-bloat** 三層:L1 pre-write 檔案大小閘 / L2 per-edit fire log / L3 季度 `/knowledge-prune`。各層的 owner 檔案、telemetry 路徑與「只移除有證據的噪音、禁為湊比例犧牲真實 invariant」的鐵律,SSOT 在 `skills/knowledge-prune/SKILL.md`,本檔不重述。
...
**6 維度 → skill**:D1 設計語言 / D2 程式語言 / D3 效能 / D4 UX / D5 視覺 / D6 原則自檢,各維對應的 skill 與判準 owner 在 `skills/design-system-audit/`(含 `references/audit-prompts.md` 判準 SSOT),本檔不重述對照表。

**Consistency 類稽核必 Phase 0 全掃再判**。**Deep audit 全掃優先 + 決策 batch-at-end 鐵律**(2026-07-11 user directive):先全 DS 掃完(NO-SAMPLE)→ 跨元件去重 → **最後一次**列決策清單給 user 拍板;**禁**途中逐元件問。

**Audit-vs-execute 分權**:判準與 `# 自主執行 canonical` 的分權表**同一份**,本節不重述 —— 只有改變產品／UI／UX SSOT 且存在真實取捨才 STOP、batch-at-end 給 user 拍板;其餘依 Standing Authorization AUTO。對齊／表達統一／清 duplicate／補 pointer 一律 AUTO。

**Scope classifier — Surgical visual bug**:user 列 N 個 visual defects + 無新 canonical／無新 API contract／無跨元件語意改動 → **Surgical scope**(batch fix + 最後一次 pixel-quantified verify,不開 audit report);其餘回上一段的 audit-vs-execute 分權。
...
**1 chat = 1 working branch + 1 PR**;protected `main` + required CI 不可繞過。所有 agent(不分 provider)只跑同一條五步,**但合併前有一道人類閘**(2026-09-02 user 原話:「你要先把我們討論的東西部署到 netlify,等我確認後認為都沒問題,主動說要發版,你才會 push 到 GitHub main…」):
...
| 1.5 **發版同意(ASK)** | **user 看過預覽後在對話說「發版」** → hook `record_release_consent.sh` 落地 receipt(`.git/governance-runtime/release-consent/current.json`,schemaVersion 3);缺 receipt → `release:auto` **停在預覽階段**印 `AWAITING_USER_RELEASE_CONSENT`,不合併不發布;問句／否定不算同意。**同意綁「user 看過的產品內容」,不綁 commit、不綁分支**(commit 與分支是 agent 切工作的單位,不是 user 授權的單位):內容沒變 → 換幾個 commit、幾條分支都算數;**預覽看得見的內容變了 → 必須重新確認**;明確否定 → 撤回。可見集合以 `scripts/release-orchestrator.mjs` 的 `PRODUCT_VISIBLE` 為準(散文不維護第二份清單)。**一份同意 = 一次 final release**(帳本在 receipt 的 `releases`;第二次需 incident 證據,否則 fail closed)。兩次修正的來龍去脈與 user 原話見 `governance/memory/feedback_solo_dev_workflow.md`。 |
...
公開入口只有三支:`npm run release:auto`(安全續跑;合併前必檢查發版同意 receipt)、`npm run release:status`(唯讀)、`npm run release:consent -- --quote "<user 原話>"`(hook 失效時的手動落地,仍需原話)。**ASK 只有兩種**:未解決的產品／UI／UX SSOT 真取捨,以及**發版同意**;login/MFA/OAuth／缺 credential reference 只暫停當下動作,完成後 AUTO resume。已退役的 ceremony(candidate-freeze、soak、fleet promotion 等)不得回到 blocking graph —— 清單見 `infra/governance/release-workflow.json`。完成後才清 remote/local branch 並 `git switch main && git pull --ff-only`。

**禁止**:direct push main / 同 chat 多 branch 或多 PR / required CI、conversation 或 live readback bypass / mutable dependency tag / 把 external write 或 legacy ceremony 當人類核准 gate / **未經 user 說「發版」就合併或發布**(2026-09-02 錨:beta.131 與 #122 在 user 還在看草稿時就被自動發出)。
...
| `tsc -b` **在本 repo 的 composite 設定下根本不檢查 DS 原始碼** | 「tsc -b 通過」不構成型別正確的證據;任何 .tsx 改動一律以 `npm run build:lib` 為準。詳 historical-bugs.md |
| 工具靜默陷阱:`rsync -a` 等長同秒跳過 / `rg` 黏寫 `-rn` 的 `-r`=replace / `mktemp -d` 失敗回空 | 必 `--checksum`、flag 分開寫、mktemp 後驗非空。**判準**:純函式測過之後,必再問「這個參數在正式流程裡是誰算的,那支有沒有被測」,並用真實資料跑**兩面對照**(該 true 一筆、該 false 一筆)。詳 historical-bugs.md |
...
| hook 測試直跑留 fixture `.git/` 進 corpus | `git status` 不顯但 snapshot tree fingerprint 全算 → trio 漂移;測試必經 run-all.sh。詳 historical-bugs.md |
| SMIL `begin="indefinite"` 動畫掛上後沒人 `beginElement()` = 永不起跑(靜默) | begin-once 守衛的觸發 key 必含每個會新掛動畫的狀態段;驗證看 `getStartTime()` 是否丟例外。詳 historical-bugs.md |
...
| **寫了閘卻沒有任何執行面呼叫它** | 那條保護是假的而且**零訊號**。**判準**:寫完任一支閘立刻問「誰呼叫它」,grep 執行面拿不到答案就等於沒寫;動態探索出來的執行路徑(gate-meta pair、harness inventory)也要模進去,只 grep 字串會誤判。棘輪 SSOT `scripts/gate-reachability-baseline.json`,閘 `scripts/gate-reachability-invariant.mjs`(CI required)。**第二種形狀(2026-09-24)**:斷在**產生檢視**那一層(工作樹的 hook 設定檢視沒跟上新群組)→ 閘 `scripts/provider-view-group-reachability-invariant.mjs`(CI required)+ `session_start_governance_check.sh` Check 12。詳 historical-bugs.md |
